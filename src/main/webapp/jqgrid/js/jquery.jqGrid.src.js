...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this._compareValues=function(func,f,v,how,t){
			...
			switch(swst) {
				...
				case 'datetime':
					...
					fld = '$.jgrid.parseDate("'+t.srcfmt+'",'+fld+').getTime()';
					break;
				default :
		...
		this.startsWith=function(f,v){
			...
			if(_useProperties){
				self._append(self._getStr('$.jgrid.getAccessor(this,\''+f+'\')')+'.substr(0,'+length+') == '+self._getStr('"'+self._toStr(v)+'"'));
			}else{
		...
		this.endsWith=function(f,v){
			...
			length=_trim ? $.trim(val.toString()).length:val.toString().length;
			if(_useProperties){
				self._append(self._getStr('$.jgrid.getAccessor(this,\''+f+'\')')+'.substr('+self._getStr('$.jgrid.getAccessor(this,\''+f+'\')')+'.length-'+length+','+length+') == "'+self._toStr(v)+'"');
			} else {
		...
		this.contains=function(f,v){
			if(_useProperties){
				self._append(self._getStr('$.jgrid.getAccessor(this,\''+f+'\')')+'.indexOf("'+self._toStr(v)+'",0) > -1');
			}else{
