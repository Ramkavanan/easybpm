...
}get ,perform:function(){this.confirmed_petrinettimeandprobability_extension_missing=this.confirmed_petrinettimeandprobability_extension_missing||false;
...
}}get .bind(this),animEl:"mb4",icon:Ext.MessageBox.QUESTION})
}else{this.checkTokens()
}}else{this.checkTokens()
}}get ,checkTokens:function(){var a=this.getPlaces().any(function(b){var c=parseInt(b.properties["oryx-numberoftokens"]);
return !(isNaN(c)||c==0)
}get );
...
}}get ,showDialog:function(){var a=this.createCompletenessSelector(),d=this.createNoiseField(),c=this.createTraceCountField(),b=this.createDialog();
...
}get ,createCompletenessSelector:function(){var b=[["None"],["Trace"],["Ordering"]],a=new Ext.data.SimpleStore({fields:["value"],data:b});
...
e.add(a)
}get ,generateLog:function(a){var c=this.getRDFFromDOM(),b=JSON.stringify(a);
...
}get ,getChildShapesByResourceIds:function(b){var a=[];
Ext.each(b,function(c){a.push(this.facade.getCanvas().getChildShapeByResourceId(c))
}get .bind(this));
return a
}get ,showOverlay:function(a,b,e,c){if(!this.overlayIds){this.overlayIds=[]
...
}get ,construct:function(a){arguments.callee.$.construct.apply(this,arguments);
...
}get ,showCheckerWindow:function(){var d=this;
...
}}get ,setIcon:function(g){this.ui.getIconEl().src=g
}get ,getIcon:function(g){return this.ui.getIconEl().src
}get ,reset:function(){d.hideOverlays();
this.hideMarking();
d.facade.raiseEvent({type:ORYX.Plugins.SyntaxChecker.RESET_ERRORS_EVENT})
}get ,hideMarking:function(){if(!d.marking){return
...
d.marking=undefined
}get ,showMarking:function(h){d.marking=h;
for(place in h){var g=d.facade.getCanvas().getChildShapeByResourceId(place);
g.setProperty("oryx-numberoftokens",h[place])
}d.facade.getCanvas().update()
}get ,showErrors:function(g){Ext.each(this.childNodes,function(h){if(h&&h.itemCls==="error"){h.remove()
}}get );
...
}get ,showOverlayWithStep:function(g){Ext.each(g,function(j,h){d.showOverlay(d.facade.getCanvas().getChildShapeByResourceId(j),{fill:"#FB7E02"},ORYX.Editor.graft("http://www.w3.org/2000/svg",null,["text",{style:"font-size: 16px; font-weight: bold;"},(h+1)+"."]),"SE")
}get )
}get ,showOverlay:function(g){if(g.length===0){return
...
this.showOverlayWithStep(g.path);
this.showMarking(g.marking)
}get ,update:function(g){this.improperTerminatings=g;
...
}get ,clickHandler:function(g){g.reset();
this.showOverlay(this.deadTransitions)
}get ,update:function(g){this.deadTransitions=g;
...
}get ,update:function(g){this.notParticipatingTransitions=g;
...
}get .bind(this)},{text:"Hide Errors",handler:function(){this.checkerWindow.getTree().getRootNode().reset()
}get .bind(this)},"->",{text:"Close",handler:function(){this.checkerWindow.close()
}get .bind(this)}],getTree:function(){return this.items.get(0)
}get ,check:function(g){this.prepareCheck(g);
this.checkSyntax(this.checkSoundness.bind(this))
}get ,prepareCheck:function(h){var g=this.getTree().getRootNode();
g.reset();
Ext.each(g.childNodes,function(j){if(h){j.expand(true)
}j.collapse(true);
j.setIcon(b.LOADING_STATUS)
}get )
}get ,checkSyntax:function(g){d.facade.raiseEvent({type:ORYX.Plugins.SyntaxChecker.CHECK_FOR_ERRORS_EVENT,onErrors:function(){Ext.Msg.alert("Syntax Check","Some syntax errors have been found, please correct them!");
this.turnLoadingIntoUnknownStatus()
}get .bind(this),onNoErrors:function(){g()
}get })
}get ,turnLoadingIntoUnknownStatus:function(){Ext.each(this.getTree().getRootNode().childNodes,function(g){if(g.getIcon().search(b.LOADING_STATUS)>-1){g.setIcon(b.UNKNOWN_STATUS)
}}get )
}get ,checkSoundness:function(){var g=this.getTree().getRootNode();
...
}}get ,failure:function(){}get ,params:{data:d.getSerializedDOM()}})
}get ,items:[new Ext.tree.TreePanel({useArrows:true,autoScroll:true,rootVisible:false,animate:true,containerScroll:true,root:new b({text:"Checks",id:"source",expanded:true}),listeners:{render:function(k){var h=new b({text:"Structural Sound (Workflow Net)",id:"structuralSound",check:function(){this.checkInitialNode.update();
...
return true
}}get ,checkInitialNode:new b({qtip:"There must be exactly one initial place, which is the only place without any incoming edges.",update:function(){this.initialNodes=[];
...
}get ,clickHandler:function(m){m.reset();
this.showOverlay(this.initialNodes)
}get ,hasErrors:function(){return this.initialNodes.length!==1
}get }),checkFinalNode:new b({qtip:"There must be exactly one final place, which is the only place without any outgoing edges.",update:function(){this.finalNodes=[];
...
}get ,hasErrors:function(){return this.finalNodes.length!==1
}get }),checkConnectedNode:new b({qtip:"Each node in the process model is on the path from the initial node to the final node.",update:function(n,m){if(n.length!==1||m.length!==1){this.setText("There must be exactly one initial and final place to perform further checks!");
...
}get ,findNotParticipatingNodes:function(m){this.passedNodes.push(m);
...
}get ,hasErrors:function(){return this.notParticipatingNodes.length!==0
}get })});
...
}get ,deadTransitionsNode:new a({}),improperTerminatingsNode:new f({}),deadLocksNode:new c({})});
...
}get ,notParticipatingTransitionsNode:new e({})});
j.appendChild([j.notParticipatingTransitionsNode]);
k.getRootNode().appendChild([h,g,l,j])
}get }})],listeners:{close:function(g){this.checkerWindow.getTree().getRootNode().reset()
}get .bind(this)}});
...
Ext.apply(this,h);
if(this.clickHandler){this.on("click",this.clickHandler.bind(this))
}}get ,setIcon:function(h){this.ui.getIconEl().src=h
}get ,getIcon:function(h){return this.ui.getIconEl().src
}get ,reset:function(){d.hideOverlays();
this.hideMarking();
d.facade.raiseEvent({type:ORYX.Plugins.SyntaxChecker.RESET_ERRORS_EVENT})
}get ,hideMarking:function(){if(!d.marking){return
...
d.marking=undefined
}get ,showMarking:function(j){d.marking=j;
for(place in j){var h=d.facade.getCanvas().getChildShapeByResourceId(place);
h.setProperty("oryx-numberoftokens",j[place])
}d.facade.getCanvas().update()
}get ,showErrors:function(h){Ext.each(this.childNodes,function(j){if(j&&j.itemCls==="error"){j.remove()
}}get );
...
}get ,showOverlayWithStep:function(h){Ext.each(h,function(k,j){d.showOverlay(d.facade.getCanvas().getChildShapeByResourceId(k),{fill:"#FB7E02"},ORYX.Editor.graft("http://www.w3.org/2000/svg",null,["text",{style:"font-size: 16px; font-weight: bold;"},(j+1)+"."]),"SE")
}get )
}get ,showOverlayMarking:function(h){Ext.each(h,function(l,j){var k=l.split(":");
...
}get ,showOverlay:function(h){if(h.length===0){return
...
}get ,update:function(h){this.marking=h.counter?h.counter.split(","):[];
...
}get ,update:function(h){this.unboundedplaces=h.unboundedplaces;
...
this.showOverlay(this.deadTransitions)
}get ,update:function(h){this.deadTransitions=h.deadtransitions;
...
this.showOverlay(this.notParticipatingTransitions)
}get ,update:function(h){this.notParticipatingTransitions=h.uncoveredtransitions;
this.setIcon(this.notParticipatingTransitions.length==0?b.OK_STATUS:b.ERROR_STATUS);
this.setText("There are "+this.notParticipatingTransitions.length+" transitions that cannot participate in a properly terminating firing sequence.")
}get });
var c=new Object;
...
}get ,check:function(h){this.prepareCheck(h);
this.checkSyntax(this.checkSoundness.bind(this),this.reRender.bind(this))
}get ,reRender:function(){window.setTimeout(function(){this.getResizeEl().beforeAction();
this.getResizeEl().sync(true)
}get .bind(this),70)
}get ,prepareCheck:function(j){var h=this.getTree().getRootNode();
h.reset();
Ext.each(h.childNodes,function(k){if(j){k.expand(true)
}k.collapse(true);
k.setIcon(b.LOADING_STATUS)
}get )
}get ,checkSyntax:function(j,h){d.facade.raiseEvent({type:ORYX.Plugins.SyntaxChecker.CHECK_FOR_ERRORS_EVENT,onErrors:function(){Ext.Msg.alert("Syntax Check","Some syntax errors have been found, please correct them!");
this.turnLoadingIntoUnknownStatus();
h()
}get .bind(this),onNoErrors:function(){j();
h()
}get })
}get ,turnLoadingIntoUnknownStatus:function(){Ext.each(this.getTree().getRootNode().childNodes,function(h){if(h.getIcon().search(b.LOADING_STATUS)>-1){h.setIcon(b.UNKNOWN_STATUS)
}}get )
}get ,checkSoundness:function(){var h=this.getTree().getRootNode();
...
}}get .bind(this),failure:function(){}get .bind(this),params:{data:j}})
}get ,items:[new Ext.tree.TreePanel({useArrows:true,autoScroll:true,rootVisible:false,animate:true,containerScroll:true,root:new b({text:"Checks",id:"source",expanded:true}),listeners:{render:function(l){var j=new b({text:"Structural Sound (Workflow Net)",id:"structuralSound",check:function(){this.checkInitialNode.update();
this.checkFinalNode.update();
this.checkConnectedNode.update(this.checkInitialNode.initialNodes,this.checkFinalNode.finalNodes);
if(this.checkInitialNode.hasErrors()||this.checkFinalNode.hasErrors()||this.checkConnectedNode.hasErrors()){this.setIcon(b.ERROR_STATUS);
this.expand();
...
this.setText(this.initialNodes.length+" initial places found.");
this.setIcon(!this.hasErrors()?b.OK_STATUS:b.ERROR_STATUS)
}get ,clickHandler:function(n){n.reset();
this.showOverlay(this.initialNodes)
}get ,hasErrors:function(){return this.initialNodes.length!==1
}get }),checkFinalNode:new b({qtip:"There must be exactly one final place, which is the only place without any outgoing edges.",update:function(){this.finalNodes=[];
...
}get }),checkConnectedNode:new b({qtip:"Each node in the process model is on the path from the initial node to the final node.",update:function(o,n){if(o.length!==1||n.length!==1){this.setText("There must be exactly one initial and final place to perform further checks!");
this.setIcon(b.UNKNOWN_STATUS);
return
}this.notParticipatingNodes=[];
...
this.showOverlay(this.notParticipatingNodes)
}get ,findNotParticipatingNodes:function(n){this.passedNodes.push(n);
...
}get ,deadTransitionsNode:new a({}),boundednessNode:new g({}),livenessNode:new f({}),});
...
this.livenessNode.update(n)
}get ,boundednessNode:new g({}),livenessNode:new f({}),});
...
}get ,deadTransitionsNode:new a({}),notParticipatingTransitionsNode:new e({})});
k.appendChild([k.notParticipatingTransitionsNode,k.deadTransitionsNode]);
l.getRootNode().appendChild([j,h,m,k])
}get }}),c],listeners:{close:function(h){this.checkerWindow.getTree().getRootNode().reset()
}get .bind(this)}});
this.checkerWindow.show();
this.checkerWindow.check(true)
}get });
...
h.each((function(k){if(!g[k]){g[k]=new Ext.tree.TreeNode({text:k,allowDrag:false,icon:j.palette(),allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true});
b.appendChild(g[k]);
g[k].render()
...
var b=formStore.findBy(function(h,j){if(h.get("formName")==formIdMap[f]){return h.get("formName")
}}get );
...
var b=newFormStore.findBy(function(h,j){if(h.get("formName")==formIdMap[f]){return h.get("formName")
}}get );
...
this.popularProperties=[];
if(this.shapeSelection.commonProperties){this.shapeSelection.commonProperties.each((function(z,n){var I=z.prefix()+"-"+z.id();
var J=z.title();
var F=[];
var t=this.shapeSelection.commonPropertiesValues[I];
var a=undefined;
var c=null;
var v=false;
var s=false;
if(!z.readonly()){var m=this.shapeSelection.shapes[0];
if(m.incoming!=undefined&&m.incoming[0]!=undefined&&m.incoming[0].incoming!=undefined&&m.incoming[0].incoming[0]!=undefined){if(m.incoming[0].incoming[0].toJSON().stencil.id=="StartNoneEvent"){var b=checkFirstNodeTaskProperty(z.type(),z.title());
if(b!=true){v=true
}}}switch(z.type()){case ORYX.CONFIG.TYPE_STRING:if(z.wrapLines()){var j=new Ext.form.TextArea({alignment:"tl-tl",allowBlank:z.optional(),msgTarget:"title",maxLength:z.length()});
j.on("keyup",function(L,K){this.editDirectly(I,L.getValue())
}get .bind(this));
a=new Ext.Editor(j)
}else{var p=new Ext.form.TextField({allowBlank:z.optional(),msgTarget:"title",maxLength:z.length()});
p.on("keyup",function(K,L){this.editDirectly(I,K.getValue())
}get .bind(this));
p.on("blur",function(K){if(!K.isValid(false)){this.updateAfterInvalid(I)
}}get .bind(this));
p.on("specialkey",function(K,L){if(!K.isValid(false)){this.updateAfterInvalid(I)
}}get .bind(this));
a=new Ext.Editor(p)
}break;
case ORYX.CONFIG.TYPE_BOOLEAN:var H=new Ext.form.Checkbox({disabled:v});
H.on("check",function(L,K){this.editDirectly(I,K)
}get .bind(this));
a=new Ext.Editor(H);
break;
case ORYX.CONFIG.TYPE_INTEGER:var e=new Ext.form.NumberField({allowBlank:z.optional(),allowDecimals:false,msgTarget:"title",minValue:z.min(),maxValue:z.max(),disabled:v});
e.on("keyup",function(K,L){this.editDirectly(I,K.getValue())
}get .bind(this));
a=new Ext.Editor(e);
break;
case ORYX.CONFIG.TYPE_FLOAT:var e=new Ext.form.NumberField({allowBlank:z.optional(),allowDecimals:true,msgTarget:"title",minValue:z.min(),maxValue:z.max(),disabled:v});
e.on("keyup",function(K,L){this.editDirectly(I,K.getValue())
}get .bind(this));
a=new Ext.Editor(e);
break;
case ORYX.CONFIG.TYPE_COLOR:var D=new Ext.ux.ColorField({allowBlank:z.optional(),msgTarget:"title",facade:this.facade,disabled:v});
a=new Ext.Editor(D);
break;
case ORYX.CONFIG.TYPE_CHOICE:var y=z.items();
var g=[];
y.each(function(K){if(K.value()==t){t=K.title()
}if(K.refToView()[0]){s=true
}g.push([K.icon(),K.title(),K.value()]);
F.push({name:K.title(),icon:K.icon()})
}get );
var l=new Ext.data.SimpleStore({fields:[{name:"icon"},{name:"title"},{name:"value"}],data:g});
var C=new Ext.form.ComboBox({tpl:'<tpl for="."><div class="x-combo-list-item">{[(values.icon) ? "<img src=\'" + values.icon + "\' />" : ""]} {title}</div></tpl>',store:l,displayField:"title",valueField:"value",typeAhead:true,mode:"local",triggerAction:"all",selectOnFocus:true,disabled:isForStartNodeType,});
C.on("select",function(M,K,L){this.editDirectly(I,M.getValue())
}get .bind(this));
a=new Ext.Editor(C);
break;
case ORYX.CONFIG.TYPE_DATE:var w=ORYX.I18N.PropertyWindow.dateFormat;
if(!(t instanceof Date)){t=Date.parseDate(t,w)
}a=new Ext.Editor(new Ext.form.DateField({allowBlank:z.optional(),format:w,msgTarget:"title",disabled:v}));
break;
case ORYX.CONFIG.TYPE_TEXT:var u=new Ext.form.ComplexTextField({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade,disabled:v});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_COMPLEX:var u=new Ext.form.ComplexListField({allowBlank:z.optional(),disabled:v},z.complexItems(),I,this.facade);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case"CPNString":var p=new Ext.form.TextField({allowBlank:z.optional(),msgTarget:"title",maxLength:z.length(),enableKeyEvents:true,disabled:v});
p.on("keyup",function(K,L){this.editDirectly(I,K.getValue());
alert("huhu")
}get .bind(this));
a=new Ext.Editor(p);
break;
case ORYX.CONFIG.TYPE_RUNTIMECOMBO:var A=new Ext.form.ComboBox({store:formStore,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:baseForm,});
A.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue();
jspForm=true
}get .bind(this));
a=new Ext.Editor(A);
break;
case ORYX.CONFIG.TYPE_ADDPROCESS:var q=new Ext.form.ComboBox({store:processStore,displayField:"processName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"processName",triggerAction:"all",selectOnFocus:true,});
q.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(q);
break;
case ORYX.CONFIG.TYPE_NEWJSPFORM:var A=new Ext.form.ComboBox({store:newFormStore,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:jspForm,});
A.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue();
baseForm=true
}get .bind(this));
a=new Ext.Editor(A);
break;
case ORYX.CONFIG.TYPE_REPEATEDOPERATION:var k=new Ext.data.SimpleStore({fields:["id","formName"],data:[["true","True"],["false","False"]]});
var E=new Ext.form.ComboBox({store:k,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:v,});
E.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(E);
break;
case ORYX.CONFIG.TYPE_EMPTYPERSONNEL:var B=new Ext.data.SimpleStore({fields:["id","formName"],data:[["true","True"],["false","False"]]});
var o=new Ext.form.ComboBox({store:B,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:v,});
o.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(o);
break;
case ORYX.CONFIG.TYPE_ISFORSTARTNODE:var x=new Ext.form.TextField({allowBlank:false,disabled:true});
a=new Ext.Editor(x);
break;
case ORYX.CONFIG.TYPE_ISARCHIVE:var r=new Ext.data.SimpleStore({fields:["id","formName"],data:[["true","True"],["false","False"]]});
var G=new Ext.form.ComboBox({store:r,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:v});
G.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(G);
break;
...
var h=new Ext.form.ComboBox({store:f,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"formName",triggerAction:"all",selectOnFocus:true,disabled:v});
h.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(h);
break;
case ORYX.CONFIG.TYPE_ADDOPINION:var d=new Ext.data.SimpleStore({fields:["id","formName"],data:[["true","True"],["false","False"]]});
var G=new Ext.form.ComboBox({store:d,displayField:"formName",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"true",triggerAction:"all",selectOnFocus:true,disabled:v});
G.on("select",function(M,K,L){this.editDirectly(I,M.getValue());
globalVariableFromValue=M.getValue()
}get .bind(this));
a=new Ext.Editor(G);
break;
case ORYX.CONFIG.TYPE_PROCESSTIMESETTING:var u=new Ext.form.ProcessTimeSetting({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_FORMFIELDAUTOMATIC:var u=new Ext.form.FormFieldAutomatic({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_ONREAD:var u=new Ext.form.OnRead({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade,disabled:v});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_ONUPDATE:var u=new Ext.form.OnUpdate({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade,disabled:v});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_ONCREATE:var u=new Ext.form.OnCreate({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade,});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_CUSTOMTREESELECT:var u=new Ext.form.CustomTreeSelect({allowBlank:false},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_CHOOSECONDITION:var u=new Ext.form.ChooseCondition({allowBlank:false},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_TRANSACTORRELATIONSHIP:var u=new Ext.form.TransactorRelationship({allowBlank:false},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_FORMFIELDPERMISSION:var u=new Ext.form.FormFieldPermission({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_FORMMEMBERPERMISSION:var u=new Ext.form.FormMemberPermission({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_MEMBERPERMISSION:var u=new Ext.form.MemberPermission({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_MEMBERPERMISSIONPROCESS:var u=new Ext.form.MemberPermissionProcess({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_STARTPERMISSION:var u=new Ext.form.StartPermission({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_URGEMESSAGE:var u=new Ext.form.UrgeMessage({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_NOTIFICATIONMESSAGE:var u=new Ext.form.NotificationMessage({allowBlank:z.optional(),dataSource:this.dataSource,grid:this.grid,row:n,facade:this.facade});
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_MEMBERPERMISSIONEND:var u=new Ext.form.MemberPermissionEnd({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_OPERATIONPERMISSION:var u=new Ext.form.OperationPermission({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_STARTNODESCRIPT:var u=new Ext.form.StartNodeScript({allowBlank:false},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_ENDNODESCRIPT:var u=new Ext.form.EndNodeScript({allowBlank:false},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
case ORYX.CONFIG.TYPE_CREATECUSTOMOPERATION:var u=new Ext.form.CreateCustomOperation({allowBlank:false,disabled:v},z.complexItems(),I,this.facade,this.grid);
u.on("dialogClosed",this.dialogClosed,{scope:this,row:n,col:1,field:u});
a=new Ext.Editor(u);
break;
default:var p=new Ext.form.TextField({allowBlank:z.optional(),msgTarget:"title",maxLength:z.length(),enableKeyEvents:true,disabled:v});
p.on("keyup",function(K,L){this.editDirectly(I,K.getValue())
}get .bind(this));
a=new Ext.Editor(p)
}a.on("beforehide",this.facade.enableEvent.bind(this,ORYX.CONFIG.EVENT_KEYDOWN));
a.on("specialkey",this.specialKeyDown.bind(this))
}else{if(z.type()===ORYX.CONFIG.TYPE_URL||z.type()===ORYX.CONFIG.TYPE_DIAGRAM_LINK){t=String(t).search("http")!==0?("http://"+t):t;
t="<a href='"+t+"' target='_blank'>"+t.split("://")[1]+"</a>"
}}if(z.visible()){if(z.popular()){z.setPopular()
}if(z.popular()){this.popularProperties.push([z.popular(),J,t,F,{editor:a,propId:I,type:z.type(),tooltip:z.description(),renderer:c}])
}else{this.properties.push([z.popular(),J,t,F,{editor:a,propId:I,type:z.type(),tooltip:z.description(),renderer:c}])
}}}get ).bind(this))
}this.setProperties()
...
return new d(b)
}get ,buildColumnModel:function(m){var j=[];
for(var d=0;
d<this.items.length;
...
g.each(function(n){k.append(l,{value:n.value()})
}get );
f=new Ext.form.ComboBox({typeAhead:true,triggerAction:"all",transform:l,lazyRender:true,msgTarget:"title",width:b,listeners:{blur:function(){var n=this.getValue();
var o=this.getRawValue()
}get ,select:function(r,n,o){var p=this.getValue();
...
f=new Ext.form.ComboBox({store:c,displayField:"name",valueField:"id",typeAhead:true,mode:"local",lazyRender:true,value:"name",triggerAction:"all",selectOnFocus:true,listeners:{focus:function(n){if(selectedUser.toLowerCase()=="humanperformer"){Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/admin/getAllUsers",failure:function(o){alert(ORYX.I18N.Feedback.failure)
}get ,success:function(p){var o=Ext.util.JSON.decode(p.responseText);
c.loadData(o)
}get })
}else{if(selectedUser.toLowerCase()=="potentialowner"){Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/admin/getAllGroups",failure:function(o){alert(ORYX.I18N.Feedback.failure)
}get ,success:function(p){var o=Ext.util.JSON.decode(p.responseText);
c.loadData(o)
}get })
}}}get }})
}}}}j.push({id:a,header:e,dataIndex:a,resizable:true,editor:f,width:b})
}return new Ext.grid.ColumnModel(j)
...
if(k!=""){reInitializeObjectObj(k,a,"operationPermission","")
}else{clearObjects(k,a,"operationPermission")
...
var j=new Ext.form.ComboBox({store:c,displayField:"functionName",valueField:"id",lazyRender:true,typeAhead:true,mode:"local",forceSelection:true,triggerAction:"all",emptyText:"Add custom operation...",selectOnFocus:true,listeners:{select:function(m,n,l){addCustomOperationResources(this.getValue(),this.getRawValue(),a)
}get }});
...
}get }},{id:"custom_operation",bodyBorder:false,autoScroll:true,border:false,width:436,height:205,items:[{id:"customPermissionItem",items:[j],bodyBorder:false,border:false,autoScroll:true,bodyStyle:"padding-left:120px",width:436,height:24},{id:"custom_operation_permission",html:"<div id='customOperationPermission' class='padding5 overflow'></div>",bodyBorder:false,border:false,autoScroll:true,width:436,height:149}]}]});
...
}if(m!=""){reInitializeObjectObj(m,a,permissionType,j)
}else{clearObjects(m,a,permissionType)
}var l='<div id="resources_select" class="pad-T5 pad-L40">';
l+='<select id = "resources_options" name = "resources_options" onchange="loadTree(null,\'fieldPermission\',\''+a+'\')" class="fontSize13">';
l+='<option value="none"> </option>';
if(a==ORYX.I18N.PropertyWindow.reader){l+='<option value="potentialOwner">Group</option>';
l+='<option value="potentialOwner">Role</option>';
l+='<option value="potentialOwner">Department</option>'
}l+='<option value="humanPerformer" selected="selected">User</option>';
l+="</select>";
l+='</div><div id="tree2" class="pad-L40 pad-T10"></div>';
var e=new Ext.Panel({title:ORYX.I18N.PropertyWindow.cts,layout:"table",width:447,height:300,header:false,items:[{id:"tree_select",html:l,bodyBorder:false,autoScroll:true,width:225,height:300,listeners:{render:function(){loadTree("humanPerformer","fieldPermission",a)
...
var j="";
if(h!=""&&h.length>0){j=h
}else{var n=this.grid.store.getAt(2);
var m=n.get("value");
j=m
}if(l!=""){reInitializeObjectObj(l,a,"startPermission",j)
}else{clearObjects(l,a,"startPermission")
}var b=new Ext.TabPanel({renderTo:document.body,activeTab:0,height:329,width:450,defaults:{autoScroll:true},items:[{title:ORYX.I18N.PropertyWindow.premission,html:'<div id="startFieldPermissionDiv" class="padding10"></div>',listeners:{activate:function(){setStartPropertyFieldPermission(d,j,a,l)
}get }},{title:ORYX.I18N.PropertyWindow.dynamicTransactor,html:'<div id="startDynamicTransactorDiv" class="padding10"></div>',listeners:{activate:function(){setStartDynamicTransactorContent(j,a,l)
}get }}]});
...
}get .bind(this)}],items:[b],bodyStyle:"background-color:#FFFFFF",buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){this.data=this.buildValue(a,j);
this.dialog.hide()
}get .bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.dialog.hide()
}get .bind(this)}]});
this.dialog.on(Ext.apply({},this.dialogListeners,{scope:this}));
if(a==ORYX.I18N.PropertyWindow.startPermission){this.dialog.show()
}else{if(j==null||j==""||j==undefined){Ext.MessageBox.alert("Information",ORYX.I18N.PropertyWindow.formError)
...
l+='<select id = "resources_options" name = "resources_options" onchange="loadTree(null,\'fieldPermission\',\''+a+'\')" class="fontSize13">';
l+='<option value="none"> </option>';
l+='<option value="potentialOwner">Group</option>';
l+='<option value="potentialOwner">Role</option>';
l+='<option value="potentialOwner">Department</option>';
l+='<option value="humanPerformer" selected="selected">User</option>';
l+="</select>";
...
var n=o.get("value");
k=n
}if(m!=""){reInitializeObjectObj(m,a,"membersFields",k)
}else{clearObjects(m,a,"membersFields")
...
var b=new Ext.TabPanel({renderTo:document.body,activeTab:0,height:368,width:506,defaults:{autoScroll:true},items:[{title:ORYX.I18N.PropertyWindow.members,items:[f],},{title:ORYX.I18N.PropertyWindow.premission,html:'<div id="fieldPermissionDiv" class="padding10"></div>',listeners:{activate:function(){setFieldPermissionContent(d,k,a,m)
}get }},{title:ORYX.I18N.PropertyWindow.dynamicTransactor,html:'<div id="dynamicTransactorDiv" class="padding10"></div>',listeners:{activate:function(){setDynamicTransactorContent(k,a,m)
}get }}]});
...
}get .bind(this)}],items:[b],bodyStyle:"background-color:#FFFFFF",buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){var q=getExtjsJsonObjWithName(a);
if(q.getCount()>0){this.data=this.buildValue(a,k);
this.dialog.hide()
}else{var p=getFormCountLength(k);
if(p==0){this.data=this.buildValue(a,k);
this.dialog.hide()
...
var b=f.get("name");
var k=f.get("value");
var j=this.grid.store.getAt(0);
var h=j.get("value");
var a=h;
var c=new Ext.TabPanel({renderTo:document.body,activeTab:0,height:330,width:580,defaults:{autoScroll:true},items:[{title:ORYX.I18N.PropertyWindow.timeSetting+a,html:'<div id="processTimeSetting"> </div>',listeners:{activate:function(){setProcessTimeSetting(k,b,a)
}get }}]});
...
var j=this.grid.store.getAt(k[0]);
var a=j.get("name");
var p=j.get("value");
...
}get .bind(this)}],items:[d],bodyStyle:"background-color:#FFFFFF",buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){this.data=this.buildValue(n);
this.dialog.hide()
}get .bind(this)},{text:ORYX.I18N.PropertyWindow.cancel,handler:function(){this.dialog.hide()
}get .bind(this)}]});
this.dialog.on(Ext.apply({},this.dialogListeners,{scope:this}));
...
var l=f.get("value");
var d=getExtjsJsonObjWithName(a);
var k=this.grid.store.getAt(3);
var h=k.get("value");
var j="";
if(h!=""&&h.length>0){j=h
}else{var n=this.grid.store.getAt(4);
var m=n.get("value");
j=m
}if(l!=""){reInitializeObjectObj(l,a,"FieldPermission",j)
}else{clearObjects(l,a,"FieldPermission")
}if(j==null||j==""||j==undefined){Ext.MessageBox.alert("Information",ORYX.I18N.PropertyWindow.formError)
}else{var b=new Ext.TabPanel({renderTo:document.body,activeTab:0,height:329,width:450,defaults:{autoScroll:true},items:[{title:ORYX.I18N.PropertyWindow.premission,html:'<div id="fieldPermissionDiv" class="padding10"></div>',listeners:{activate:function(){setFieldPermissionContent(d,j,a,l)
}get }}]});
this.dialog=new Ext.Window({autoScroll:true,autoCreate:true,title:ORYX.I18N.PropertyWindow.premission,height:420,width:464,modal:true,collapsible:false,fixedcenter:true,shadow:true,proxyDrag:true,keys:[{key:27,fn:function(){this.dialog.hide
}get .bind(this)}],items:[b],bodyStyle:"background-color:#FFFFFF",buttons:[{text:ORYX.I18N.PropertyWindow.ok,handler:function(){this.data=this.buildValue(a,j);
this.dialog.hide()
...
var m="";
var q=new Ext.form.TextField({readOnly:true,value:"",listeners:{render:function(j){if(h!=undefined){h.load({callback:function(){j.setValue(h.reader.jsonData.forms[0].formName)
}get })
}m=g;
conditionExpression(g,c)
}get }});
var x=new Ext.form.ComboBox({store:h,displayField:"formName",valueField:"id",lazyRender:true,typeAhead:true,mode:"local",forceSelection:true,triggerAction:"all",emptyText:"Select a form...",selectOnFocus:true,listeners:{select:function(y,z,j){m=this.getValue();
...
var c=[];
var j=this.grid.getSelectionModel().getSelectedCell();
var h=this.grid.store.getAt(j[0]);
var a=h.get("name");
var o=h.get("value");
var n=this.grid.store.getAt(3);
var k=n.get("value");
var l="";
if(k!=""&&k.length>0){l=k
}else{var q=this.grid.store.getAt(4);
var p=q.get("value");
l=p
}var m=this.grid.store.getAt(24);
var e=m.get("value");
if(o!=""){reInitializeObjectObj(o,a,"membersFields",l)
}else{clearObjects(o,a,"membersFields")
}var d=getExtjsJsonObjWithName(a);
...
var b=new Ext.TabPanel({renderTo:document.body,activeTab:0,height:335,width:450,defaults:{autoScroll:true},items:[{title:ORYX.I18N.PropertyWindow.members,items:[g],},{title:ORYX.I18N.PropertyWindow.premission,html:'<div id="fieldPermissionDiv" class="padding10"></div>',listeners:{activate:function(){setFieldPermissionContent(d,l,a,o)
}get }},{title:ORYX.I18N.PropertyWindow.dynamicTransactor,html:'<div id="dynamicTransactorDiv" class="padding10"></div>',listeners:{activate:function(){setDynamicTransactorContent(l,a,o)
}get }}]});
...
}get .bind(this)}],items:[b],bodyStyle:"background-color:#FFFFFF",buttons:[{text:ORYX.I18N.PropertyWindow.submit,handler:function(){var u=getExtjsJsonObjWithName(a);
if(u.getCount()>0){if(isResourceOrderDivVisible()){var t=addOrderToResources();
if(t){this.data=assignOrderAndGetResourceData(a,l);
this.dialog.hide()
}}else{showResourceOrderDiv();
renderResourceOrderView(a,e)
}}else{if(isResourceOrderDivVisible()){this.data=assignOrderAndGetResourceData(a,l)
}var r=this.dialog;
var s=0;
Ext.Ajax.request({url:"/bpm/form/getAllFormFields",params:{formId:l},asynchronous:false,method:"GET",failure:function(v,w){}get ,success:function(w,y){var x=w.responseText;
var v={};
v=JSON.parse(x.toString());
var A=v.forms;
s=A.length;
if(s==0){if(isResourceOrderDivVisible()){var z=addOrderToResources();
if(z){r.hide()
}}else{showResourceOrderDiv();
renderResourceOrderView(a,e)
}}else{Ext.MessageBox.alert("Information",ORYX.I18N.PropertyWindow.permissionError)
...
}}get ,saveSynchronously:function(k){toggleFullscreenProcessEditor("save_process");
this.changeDifference=0;
var h="";
var g=document.URL;
var o=g.split("?");
...
}if(k){var n=this.facade.getStencilSets();
var a=n[n.keys()[0]].source().split("stencilsets")[1];
...
if(!o[1]){if(h.include(ORYX.CONFIG.ORYX_NEW_URL)){var n=this.facade.getStencilSets().values()[0];
var e={title:ORYX.I18N.Save.newProcess,summary:"",type:n.title(),url:h,namespace:n.namespace()};
...
var c=new Ext.Panel({labelAlign:"right",title:ORYX.I18N.Save.dialogDesciption,width:450,bodyStyle:"padding:5px 5px 0 0",items:[{xtype:"form",monitorValid:true,id:"processForm",name:"processForm",layout:"form",border:false,items:[{xtype:"textfield",fieldLabel:ORYX.I18N.Save.dialogLabelName+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"title",allowBlank:false,labelStyle:"font-weight:bold;",regex:/^[A-Za-z\u4e00-\u9eff\.][A-Za-z\' \'\w\u4e00-\u9eff\.][\-a-zA-Z\_\u4e00-\u9eff\s]{1,100}$/,anchor:"90%"},{xtype:"textarea",grow:true,fieldLabel:ORYX.I18N.Save.dialogLabelDesc+" ",name:"summary",labelStyle:"font-weight:bold;",anchor:"90%"},{xtype:"hidden",grow:true,name:"namespace",value:n.namespace()},{xtype:"combo",store:d,id:"classification_type",name:"classification_type",fieldLabel:'Classification <span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',labelStyle:"font-weight:bold;",displayField:"name",valueField:"id",allowBlank:false,typeAhead:true,mode:"local",lazyRender:true,triggerAction:"all",overflow:"auto",emptyText:"Select a Classification...",selectOnFocus:true,anchor:"90%",regex:new RegExp("^[A-Za-z一-黿.][A-Za-z w一-黿.]{1,40}$")}],buttons:[{text:ORYX.I18N.Save.saveBtn,formBind:true,handler:function(){callback()
}get },{text:ORYX.I18N.Save.close,handler:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE});
win.destroy()
}get .bind(this)}]}]});
callback=function(t){this.mask=new Ext.LoadMask(Ext.getBody(),{msg:ORYX.I18N.Oryx.pleaseWait});
this.mask.show();
var w=Ext.getCmp("processForm").getForm();
var v=w.findField("title").getValue().strip();
v=v.length==0?e.title:v;
window.document.title=v+" - Oryx";
var q=w.findField("summary").getValue().strip();
q=q.length==0?e.summary:q;
var p=w.findField("classification_type").getRawValue().strip();
var u=w.findField("classification_type").getValue().strip();
if(u.length==0){u=p
}else{if(u.length>0&&p.length>0){var s=p.toLowerCase().replace(/[" "]/g,"_");
if(u==s){p=""
}}}u=p.length==0?u:p;
var r=w.findField("namespace").getValue().strip();
r=r.length==0?e.namespace:r;
win.destroy();
this.sendSaveRequest(h,{data:this.serializedDOM,svg:b,title:v,summary:q,type:r,classification_type:u,designMode:"create"},k,this.mask)
}get .bind(this);
win=new Ext.Window({id:"Propertie_Window",width:"auto",height:"auto",title:k?ORYX.I18N.Save.saveAsTitle:ORYX.I18N.Save.save,modal:true,bodyStyle:"background:#FFFFFF",items:c,});
win.show()
...
d.load();
var n=this.facade.getStencilSets().values()[0];
var e={title:ORYX.I18N.Save.newProcess,summary:"",type:n.title(),url:h,namespace:n.namespace()};
var l=decodeURIComponent(o[1].split("&")[2].split("=")[1]);
var j=decodeURIComponent(o[1].split("&")[3].split("=")[1]);
var m=decodeURIComponent(o[1].split("&")[4].split("=")[1]).replace("#","");
var c=new Ext.Panel({labelAlign:"right",title:ORYX.I18N.Save.dialogDesciption,width:450,bodyStyle:"padding:5px 5px 0 0",items:[{xtype:"form",monitorValid:true,id:"processForm",name:"processForm",layout:"form",border:false,items:[{xtype:"textfield",fieldLabel:ORYX.I18N.Save.dialogLabelName+'<span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',name:"title",allowBlank:false,disabled:true,value:l,labelStyle:"font-weight:bold;",regex:/^[A-Za-z\u4e00-\u9eff\.][A-Za-z\' \'\w\u4e00-\u9eff\.][\-a-zA-Z\_\u4e00-\u9eff\s]{1,100}$/,anchor:"90%"},{xtype:"textarea",grow:true,fieldLabel:ORYX.I18N.Save.dialogLabelDesc+" ",name:"summary",value:j,labelStyle:"font-weight:bold;",anchor:"90%"},{xtype:"hidden",grow:true,name:"namespace",value:n.namespace()},{xtype:"combo",store:d,id:"classification_type",name:"classification_type",fieldLabel:'Classification <span style="color: rgb(255, 0, 0); padding-left: 2px;">* </span>',labelStyle:"font-weight:bold;",displayField:"name",valueField:"id",allowBlank:false,typeAhead:true,value:m,mode:"local",lazyRender:true,triggerAction:"all",overflow:"auto",emptyText:"Select a Classification...",selectOnFocus:true,anchor:"90%",regex:new RegExp("^[A-Za-z一-黿.][A-Za-z w一-黿.]{1,40}$")}],buttons:[{text:ORYX.I18N.Save.saveBtn,formBind:true,handler:function(){callback()
}get },{text:ORYX.I18N.Save.close,handler:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE});
win.destroy()
}get .bind(this)}]}]});
callback=function(t){this.mask=new Ext.LoadMask(Ext.getBody(),{msg:ORYX.I18N.Oryx.pleaseWait});
this.mask.show();
var v=Ext.getCmp("processForm").getForm();
window.document.title=l+" - Oryx";
var q=v.findField("summary").getValue().strip();
q=q.length==0?e.summary:q;
var p=v.findField("classification_type").getRawValue().strip();
var u=v.findField("classification_type").getValue().strip();
if(u.length==0){u=p
}else{if(u.length>0&&p.length>0){var s=p.toLowerCase().replace(/[" "]/g,"_");
if(u==s){p=""
}}}u=p.length==0?u:p;
var r=e.namespace;
win.destroy();
this.sendSaveRequest(h,{data:this.serializedDOM,svg:b,title:l,summary:q,type:r,classification_type:u,designMode:"edit"},k,this.mask)
}get .bind(this);
win=new Ext.Window({id:"Propertie_Window",width:"auto",height:"auto",title:k?ORYX.I18N.Save.saveAsTitle:ORYX.I18N.Save.save,modal:true,bodyStyle:"background:#FFFFFF",items:c,});
win.show()
...
Ext.Ajax.on("beforerequest",function(h,j){Ext.getBody().mask("Loading...")
}get );
...
Ext.Ajax.on("requestcomplete",function(h,j){Ext.getBody().unmask()
}get );
Ext.Ajax.on("requestexception",function(h,j){Ext.getBody().unmask()
}get )
}get ,sourceSaveRequest:function(a,c,b){Ext.Ajax.request({url:a,params:c,method:"POST",asynchronous:false,failure:function(d,e){}get ,success:function(d,e){var j=d.responseText;
var h={};
h=JSON.parse(j.toString());
var g=h.xml;
...
var k=(f-54)+"px";
document.getElementById("view-source").style.height=k;
...
}if(f.length>0&&isFormChanged(f,k)){Ext.Msg.show({title:"Information",msg:"The Form Version of Task "+changedTaskForm+" has been changed as new Version.Are you sure to change form version in Tasks ?",buttons:Ext.Msg.OKCANCEL,fn:function(y){if(y=="ok"){if(f.length>1){for(var j=0;
j<f.length;
j++){Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/form/getFormById",params:{formId:f[j].toString()},failure:function(z){Ext.Msg.show({title:"Message",msg:"failure. ",buttons:Ext.Msg.OK,animEl:"elId",icon:Ext.MessageBox.ERROR})
}get ,success:function(B){var A=Ext.util.JSON.decode(B.responseText);
var D=formStore.recordType;
var C;
C=new D({id:A.forms[0].id,formName:A.forms[0].formName,});
formIdMap[A.forms[0].id]=A.forms[0].formName;
var z=formStore.findBy(function(E,F){if(E.get("formName")==A.forms[0].formName){return true
}return false
}get );
if(z==-1){formStore.add(C);
formStore.commitChanges()
}}get })
}}else{Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/form/getFormById",params:{formId:f.toString()},failure:function(z){Ext.Msg.show({title:"Message",msg:"failure. ",buttons:Ext.Msg.OK,animEl:"elId",icon:Ext.MessageBox.ERROR})
}get ,success:function(B){var A=Ext.util.JSON.decode(B.responseText);
var D=formStore.recordType;
var C;
C=new D({id:A.forms[0].id,formName:A.forms[0].formName,});
formIdMap[f.toString()]=A.forms[0].formName;
var z=formStore.findBy(function(E,F){if(E.get("formName")==A.forms[0].formName){return true
}return false
}get );
if(z==-1){formStore.add(C);
...
}}else{if(f.length>1){for(var j=0;
j<f.length;
j++){Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/form/getLatestFormVersion",params:{formId:f[j].toString()},failure:function(z){Ext.Msg.show({title:"Message",msg:"failure. ",buttons:Ext.Msg.OK,animEl:"elId",icon:Ext.MessageBox.ERROR})
}get ,success:function(B){var A=Ext.util.JSON.decode(B.responseText);
var D=formStore.recordType;
var C=new D({id:A.forms.id,formName:A.forms.formName,});
var z=formStore.findBy(function(E,F){if(E.get("id")==A.forms.id){return true
}return false
}get );
if(z==-1){formStore.add(C);
formStore.commitChanges()
}}get })
}}else{Ext.Ajax.request({waitMsg:"Saving changes...",method:"GET",headers:{"Content-Type":"application/json"},url:"/form/getLatestFormVersion",params:{formId:f.toString()},failure:function(z){Ext.Msg.show({title:"Message",msg:"failure. ",buttons:Ext.Msg.OK,animEl:"elId",icon:Ext.MessageBox.ERROR})
}get ,success:function(B){var A=Ext.util.JSON.decode(B.responseText);
var D=formStore.recordType;
var C=new D({id:A.forms.id,formName:A.forms.formName,});
var z=formStore.findBy(function(E,F){if(E.get("id")==f.toString()){return true
}return false
}get );
if(z==-1){formStore.add(C);
formStore.commitChanges()
