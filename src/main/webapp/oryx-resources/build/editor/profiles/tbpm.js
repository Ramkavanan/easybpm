...
if(!ORYX.Plugins){ORYX.Plugins=new Object()
}ORYX.Plugins.TBPMShapeConnector=Clazz.extend({construct:function(a){this.facade=a;
this.active=false;
this.sourceNode=null;
this.facade.offer({name:ORYX.I18N.TBPMShapeconnctor.name,functionality:this.enableConnector.bind(this),group:ORYX.I18N.TBPMShapeconnctor.group,icon:ORYX.PATH+"images/pencil_go.png",description:ORYX.I18N.TBPMShapeconnctor.desc,index:1,toggle:true,minShape:0,maxShape:0});
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_MOUSEDOWN,this.handleMouseDown.bind(this))
}get ,enableConnector:function(a,b){this.connectButton=a;
if(!b){this.active=false;
this.sourceNode=null
}else{this.active=true;
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_STATUS,text:ORYX.I18N.TBPMShapeconnctor.usage})
}}get ,handleMouseDown:function(b,a){if(this.active&&a instanceof ORYX.Core.Node){if(this.sourceNode){if(!this.createEdge(this.sourceNode,a)){return
}this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE})
}this.sourceNode=a
}else{if(this.active){if(this.connectButton){this.connectButton.toggle()
}}}}get ,createEdge:function(c,e){var a=this.facade.getStencilSets().keys()[0];
var b=ORYX.Core.StencilSet.stencil(a+"SequenceFlow");
var d=this.facade.getRules().canConnect({sourceShape:c,edgeStencil:b,targetShape:e});
var f=new ORYX.Plugins.TBPMShapeConnector.CreateEdge(c,e,b,this.facade);
this.facade.executeCommands([f]);
return f.edge
}get ,});
ORYX.Plugins.TBPMShapeConnector.CreateEdge=ORYX.Core.Command.extend({construct:function(c,d,b,a){this.source=c;
this.target=d;
this.stencil=b;
this.facade=a
}get ,execute:function(){console.log("execute");
var a=new ORYX.Core.Edge({eventHandlerCallback:this.facade.raiseEvent},this.stencil);
a.dockers.first().setDockedShape(this.source);
a.dockers.first().setReferencePoint({x:this.source.bounds.width()/2,y:this.source.bounds.height()/2});
a.dockers.last().setDockedShape(this.target);
a.dockers.last().setReferencePoint({x:this.target.bounds.width()/2,y:this.target.bounds.height()/2});
this.facade.getCanvas().add(a);
this.facade.getCanvas().update();
this.edge=a
}get ,rollback:function(){this.facade.deleteShape(this.edge);
this.facade.getCanvas().update();
this.facade.updateSelection()
}get });
if(!ORYX.Plugins){ORYX.Plugins=new Object()
}ORYX.Plugins.TBPMCanvasMode=ORYX.Plugins.AbstractPlugin.extend({facade:undefined,canvasId:"ext-gen56",fill:"#ffffcc",edited:"RGBA(255,255,255,0.8)",shapeTransform:false,construct:function(){arguments.callee.$.construct.apply(this,arguments);
this.facade.offer({name:ORYX.I18N.TBPMCanvasMode.name,functionality:this.enableToggle.bind(this),group:ORYX.I18N.TBPMCanvasMode.group,icon:ORYX.PATH+"images/shape_move_backwards.png",description:ORYX.I18N.TBPMCanvasMode.desc,index:1,toggle:true,minShape:0,maxShape:0});
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_TBPM_BACKGROUND_UPDATE,this.enableShapeTransform.bind(this))
}get ,enableShapeTransform:function(){if(!this.shapeTransform){this.shapeTransform=true;
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_PROPERTY_CHANGED,this.updateShapeSVG.bind(this))
}}get ,updateShapeSVG:function(b){if(this.shapeTransform){var a=b.elements[0];
if(a&&b.name=="oryx-name"&&!a.isEdited){a._svgShapes.each(function(c){if(c.element.id&&c.element.id.endsWith("bg_frame")){c.element.isEdited=true;
a.isEdited=true;
c.element.setAttributeNS(null,"fill",this.edited)
}}get .bind(this))
}a.refresh();
this.facade.getCanvas().update()
}}get ,enableToggle:function(a,b){if(b&&$(this.canvasId).getStyle("backgroundImage")){$(this.canvasId).style.background="";
...
}else{if(!b&&this.facade.getCanvas().properties["oryx-photo"]){$(this.canvasId).style.background="url("+this.facade.getCanvas().properties["oryx-photo"]+") no-repeat scroll 0% 0%";
$A($$("rect")).each(function(c){if(c.id.endsWith("bg_frame")){if(c.isEdited){c.setAttributeNS(null,"fill",this.edited)
}else{c.setAttributeNS(null,"fill","None")
}c.setAttributeNS(null,"stroke-width","4")
}}get .bind(this));
this.shapeTransform=true
}}}get });
if(!ORYX.Plugins){ORYX.Plugins=new Object()
}ORYX.Plugins.TBPMSupport=ORYX.Plugins.AbstractPlugin.extend({facade:undefined,canvasId:"ext-gen56",construct:function(b,a){arguments.callee.$.construct.apply(this,arguments);
this.facade.offer({name:ORYX.I18N.TBPMSupport.imp.name,functionality:this.showImportDialog.bind(this),group:ORYX.I18N.TBPMSupport.imp.group,icon:ORYX.PATH+"images/page_white_picture.png",description:ORYX.I18N.TBPMSupport.imp.desc,index:3,minShape:0,maxShape:0});
a.properties.each(function(c){if(c.name=="tbpm_recognition_service"){this.tbpmImportServletURL=c.value
}}get .bind(this))
}get ,showImportDialog:function(a){this.form=new Ext.form.FormPanel({baseCls:"x-plain",labelWidth:50,defaultType:"textfield",fileUpload:true,enctype:"multipart/form-data",items:[{text:ORYX.I18N.TBPMSupport.imp.selectFile,style:"font-size:12px;margin-bottom:10px;display:block;",anchor:"100%",xtype:"label"},{fieldLabel:ORYX.I18N.TBPMSupport.imp.file,name:"subject",inputType:"file",style:"margin-bottom:10px;display:block;width:95%",itemCls:"ext_specific_window_overflow"}]});
...
this.dialog.show()
}get ,uploadImage:function(a,b){this.form.form.submit({url:this.tbpmImportServletURL,clientValidation:true,waitMsg:"Processing picture...",method:"POST",success:function(c,d){this.dialog.hide();
this.showConfirmDialog(d.response.responseText)
}get .bind(this),failure:function(c,d){this.dialog.hide();
this.showConfirmDialog(d.response.responseText)
}get .bind(this)})
}get ,showConfirmDialog:function(a){var c=Ext.util.JSON.decode(a);
imgUri=c.uri;
var b=new Ext.Window({autoCreate:true,layout:"fit",width:600,height:500,bodyStyle:"padding:5px;",autoScroll:true,modal:true,fixedcenter:true,shadow:true,proxyDrag:true,resizable:true,title:ORYX.I18N.TBPMSupport.imp.confirm,html:'<div style="width:100%;"><img src="'+imgUri+'" style="width:550px;"></img></div>',buttons:[{text:ORYX.I18N.TBPMSupport.imp.btnImp,handler:function(){b.close();
this.processImport(imgUri,c.model,c.width,c.height)
}get .bind(this)},{text:ORYX.I18N.TBPMSupport.imp.btnClose,handler:function(){b.close()
}get .bind(this)}]});
b.show()
}get ,processImport:function(b,d,e,a){var c=this.facade.getCanvas();
this.addImageLayer(b);
this.importShapes(d);
if(c.bounds.width()<e){c.setSize({width:e+50,height:c.bounds.height()})
}if(c.bounds.height()<a){c.setSize({height:a+50,width:c.bounds.width()})
}c.update()
}get ,addImageLayer:function(a){this.facade.getCanvas().properties["oryx-photo"]=a;
$(this.canvasId).style.background="url("+a+") no-repeat scroll 0% 0%"
}get ,importShapes:function(a){this.facade.importJSON(a,true);
$A($$("rect")).each(function(b){if(b.id.endsWith("bg_frame")){b.setAttributeNS(null,"fill","None");
b.setAttributeNS(null,"stroke-width","4")
}}get .bind(this));
