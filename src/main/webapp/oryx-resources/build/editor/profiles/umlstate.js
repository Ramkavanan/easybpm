...
}ORYX.Plugins.UMLState=Clazz.extend({facade:undefined,construct:function(a){this.facade=a;
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_REGISTER_LABEL_TEMPLATE,edit_template:this.templatizeValue.bind(this),render_template:this.untemplatizeValue.bind(this)});
this.facade.registerOnEvent("layout.uml.state.seperator",this.centerSeperatorOnParent.bind(this))
}get ,templatizeValue:function templatizeValue(d,c,b){var a=b._stencil.id();
if(a==="http://b3mn.org/stencilset/umlstate#controlFlow"){return this.templatizeEdgeValue(d)
}else{if((a==="http://b3mn.org/stencilset/umlstate#stateWithActions")&&(c.indexOf("actions")!==-1)){return this.templatizeStateWithActionsValue(d)
}else{return d
}}}get ,templatizeEdgeValue:function templatizeEdgeValue(a){var d=a.indexOf("[");
var c=a.indexOf("/");
if(a===""){return"Event [Guard] /Action"
}if((d===-1)&&(c===-1)){return a+" [Guard] /Action"
}if((d===0)&&(c===-1)){return"Event "+a+" /Action"
}if(c===0){return"Event [Guard] "+a
}if((d!==-1)&&(c===-1)){return a+" /Action"
}if((d===-1)&&(c>0)){var b=a.split("/");
return b[0]+"[Guard] /"+b[1]
}if((d===0)&&(c!==-1)){return"Event "+a
}if((d>0)&&(c>0)){return a
}return a
}get ,templatizeStateWithActionsValue:function templatizeStateWithActionsValue(a){var d=a.indexOf("entry /");
var b=a.indexOf("do /");
var c=a.indexOf("exit /");
...
do / action
exit / action"
}if((d===0)&&(b===-1)&&(c===-1)){return a+"do / action
exit / action"
}if((d===-1)&&(b===0)&&(c===-1)){return"entry / action
"+a+"exit / action"
}if((d===-1)&&(b===-1)&&(c===0)){return"entry / action
do / action
"+a
}if((d===0)&&(b!==-1)&&(c===-1)){return a+"exit / action"
}if((d===0)&&(b===-1)&&(c!==-1)){return a.slice(0,c)+"do / action
"+a.slice(c)
}if((d===-1)&&(b===0)&&(c!==-1)){return"entry / action
"+a
}if((d===0)&&(b!==-1)&&(c!==-1)){return a
}return a
}get ,untemplatizeValue:function untemplatizeValue(d,c,b){var a=b._stencil.id();
if(a==="http://b3mn.org/stencilset/umlstate#controlFlow"){return this.untemplatizeEdgeValue(d)
}else{if((a==="http://b3mn.org/stencilset/umlstate#stateWithActions")&&(c.indexOf("actions")!==-1)){return this.untemplatizeStateWithActionsValue(d)
}else{return d
}}}get ,untemplatizeEdgeValue:function untemplatizeEdgeValue(a){var b=a.replace("Event ","").replace("[Guard]","").replace(" /Action","");
return b
}get ,untemplatizeStateWithActionsValue:function untemplatizeStateWithActionsValue(a){var b=a.replace("entry / action
","").replace("do / action
","").replace("exit / action","");
return b
}get ,centerSeperatorOnParent:function centerOnParent(c){var a=c.shape;
var b=a.getParentShape();
var d=b.absoluteCenterXY().x-a.absoluteCenterXY().x;
a.bounds.moveBy(d,0)
}get });
if(!ORYX.Plugins){ORYX.Plugins=new Object()
