...
}ORYX.Plugins.UMLUseCase={construct:function(a){this.facade=a;
...
}get ,handleDiagramOnLoad:function(a){this.layoutAllSystems(this.facade.getCanvas())
}get ,layoutAllSystems:function(b){this.layoutAllUseCaseExtended(b);
var a=b.getChildNodes().findAll(function(c){if(this.isSystemNode(c)){return c
}}get .bind(this));
a.each(function(c){this.layoutStereotype(c);
this.layoutAllSystems(c)
}get .bind(this))
}get ,layoutAllUseCaseExtended:function(a){var b=a.getChildNodes().findAll(function(c){if(this.isUseCaseExtendedNode(c)){return c
}}get .bind(this));
b.each(function(c){this.layoutUseCaseExtended(c)
}get .bind(this))
}get ,handleUseCaseExtendedLayout:function(b){var a=b.shape;
if(a.isResized){this.SelectionEventFunction=this.handleUseCaseExtendedSelection.bind(this);
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED,this.SelectionEventFunction)
}}get ,handleUseCaseExtendedSelection:function(b){var a=b.elements.first();
if(!this.isUseCaseExtendedNode(a)){return
}this.facade.unregisterOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED,this.SelectionEventFunction);
this.layoutUseCaseExtended(a)
}get ,layoutUseCaseExtended:function(e){var d=e._svgShapes.find(function(f){return f.element.id==(e.id+"extension_point_text_frame")
}get ).element;
var c=e.getLabels().find(function(f){return f.id==(e.id+"extensions")
}get );
var b=e.getLabels().find(function(f){return f.id==(e.id+"extensionpoint")
}get );
var a=d.y.baseVal.value-b.y-16;
d.y.baseVal.value=b.y+16;
c.y=b.y+16;
d.height.baseVal.value+=a
}get ,handlePropertyChanged:function(b){var a=b.elements.first();
if(!this.isSystemNode(a)){return
}if(!(this.isStereotypeKeyEvent(b)||this.isNameKeyEvent(b))){return
}this.layoutStereotype(a)
}get ,layoutStereotype:function(c){var a=c.getLabels().find(function(d){return d.id==(c.id+"stereotype")
}get );
var b=c.getLabels().find(function(d){return d.id==(c.id+"text")
}get );
if(a.text().empty()){b.y=a.y;
a.hide()
}else{b.y=a.y+14;
a.show();
a.text("≪"+c.properties["oryx-stereotype"]+"≫")
}b.update();
a.update()
}get ,isUseCaseExtendedNode:function(a){return"http://b3mn.org/stencilset/umlusecase#usecaseextended"==a.getStencil().id().toLowerCase()
}get ,isUseCaseNode:function(a){return"http://b3mn.org/stencilset/umlusecase#usecase"==a.getStencil().id().toLowerCase()
}get ,isSystemNode:function(a){return"http://b3mn.org/stencilset/umlusecase#system"==a.getStencil().id().toLowerCase()
}get ,isIncludeEdge:function(a){return"http://b3mn.org/stencilset/umlusecase#include"==a.getStencil().id().toLowerCase()
}get ,isExtendEdge:function(a){return"http://b3mn.org/stencilset/umlusecase#extend"==a.getStencil().id().toLowerCase()
}get ,isStereotypeKeyEvent:function(a){return a.key=="oryx-stereotype"
}get ,isNameKeyEvent:function(a){return a.key=="oryx-name"
}get };
