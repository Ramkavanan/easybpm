...
this.facade.offer({name:"Import from CPN Tools",functionality:this.importCPN.bind(this),group:ORYX.I18N.cpntoolsSupport.group,dropDownGroupIcon:ORYX.PATH+"images/import.png",icon:ORYX.PATH+"images/cpn/cpn_import.png",description:ORYX.I18N.cpntoolsSupport.importDescription,index:1,minShape:0,maxShape:0});
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_RESIZE_END,this.resetTokenPosition.bind(this))
}get ,importCPN:function(){this._showImportDialog()
}get ,exportCPN:function(){var a=this.facade.getSerializedJSON();
this._doExportToCPNTools(a)
}get ,resetTokenPosition:function(){var a=this.facade.getSelection().findAll(function(b){return(b.getStencil().id()==="http://b3mn.org/stencilset/coloredpetrinet#Place")
}get );
...
var m=0;
var l=0;
n.each(function(q){var s=q.absoluteBounds();
var p=s.center();
var o=f.x-p.x;
var c=f.y-p.y;
var r=o*o+c*c;
if(j*j<=r){l=Math.round(Math.sin((Math.PI/6)*h)*k);
m=Math.round(Math.cos((Math.PI/6)*h)*k);
q.bounds.centerMoveTo(g.bounds.width()/2+m,g.bounds.height()/2+l);
q.update();
h=h+1
}}get )
}}get )
...
return c
}get ,_doExportToCPNTools:function(a){this._sendRequest(ORYX.CONFIG.CPNTOOLSEXPORTER,"POST",{data:a},function(b){if(b.startsWith("error:")){this._showErrorMessageBox(ORYX.I18N.Oryx.title,b)
}else{this.openDownloadWindow(window.document.title+".cpn",b)
}}get .bind(this),function(){this._showErrorMessageBox(ORYX.I18N.Oryx.title,ORYX.I18N.cpntoolsSupport.serverConnectionFailed)
}get .bind(this))
...
b.hide()
}get .bind(this)},{text:ORYX.I18N.cpntoolsSupport.close,handler:function(){b.hide()
}get .bind(this)}]});
...
}get ,_getAllPages:function(c,e){var h=new DOMParser();
...
this._sendRequest(ORYX.CONFIG.CPNTOOLSIMPORTER,"POST",{pagesToImport:a,data:c},function(j){if(j.startsWith("error:")){this._showErrorMessageBox(ORYX.I18N.Oryx.title,j);
e.hide()
}else{this.facade.importJSON(j);
e.hide()
}}get .bind(this),function(){e.hide();
...
}get ,showPageDialog:function(f,c){var a=new Ext.data.ArrayReader({},[{name:"name"}]);
...
g.getSelections().each(function(l){h=l.data.name
}get .bind(this));
var k=h.length;
if(h.length==0){alert(ORYX.I18N.cpntoolsSupport.noPageSelection);
return
}var j=new Ext.LoadMask(Ext.getBody(),{msg:ORYX.I18N.cpntoolsSupport.importProgress});
j.show();
e.hide();
pageName=h;
this._sendRequest(ORYX.CONFIG.CPNTOOLSIMPORTER,"POST",{pagesToImport:pageName,data:c},function(l){if(l.startsWith("error:")){this._showErrorMessageBox(ORYX.I18N.Oryx.title,l);
j.hide()
}else{this.facade.importJSON(l);
j.hide()
}}get .bind(this),function(){j.hide();
this._showErrorMessageBox(ORYX.I18N.Oryx.title,ORYX.I18N.cpntoolsSupport.serverConnectionFailed)
}get .bind(this))
}get .bind(this)},{text:ORYX.I18N.cpntoolsSupport.close,handler:function(){e.hide()
}get .bind(this)}]});
e.show()
}get });
if(!ORYX.Plugins){ORYX.Plugins=new Object()
