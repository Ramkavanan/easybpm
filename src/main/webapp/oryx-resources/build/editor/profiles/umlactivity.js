...
a=a||g;
if(this.isActivityPartitionNode(g.parent)){return
}if(!this.isActivityPartitionNode(g)){return
}if(!this.isActivityPartitionNode(a)){return
}if(b.length<=0){return
}this.currentRootPartition=g;
if(!this.hashedBounds[g.resourceId]){this.hashedBounds[g.resourceId]={}
}var f=this.getChildActivityPartitions(g,true);
var d=this.getAllAddedPartitions(f);
var c=this.getAllDeletedPartitions(f);
if(d.length>0){a=d.first()
}if(c.length>0||d.length>0){this.resizeAfterAddorDeleteOfPartition(g,b)
