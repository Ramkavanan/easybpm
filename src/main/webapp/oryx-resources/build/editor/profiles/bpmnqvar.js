...
}ORYX.Plugins.QueryVariant=ORYX.Plugins.AbstractPlugin.extend({facade:undefined,construct:function(a){this.facade=a;
this.active=false;
this.raisedEventIds=[];
this.facade.offer({name:ORYX.I18N.QueryVariant.name,functionality:this.showOverlay.bind(this),group:ORYX.I18N.QueryVariant.group,icon:ORYX.PATH+"stencilsets/bpmnqvar/bpmnqvar.png",description:ORYX.I18N.QueryVariant.tooltip,index:0,toggle:false,minShape:0,maxShape:0})
...
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_ENABLE,text:"Processing partial process model"});
new Ajax.Request(ORYX.CONFIG.QUERYVARIANTEVAL_URL,{method:"POST",asynchronous:true,parameters:{resource:location.href,command:b.command,modelID:b.modelID,stopAtFirstMatch:b.stopAtFirstMatch,data:location.href},onSuccess:function(e){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE});
var f=e.responseXML;
var d=f.firstChild;
var j=d.getElementsByTagName("ProcessGraph");
var h=location.href.substring(0,location.href.substring(8).indexOf("/")+8);
var g=h+"/backend/poem/new?stencilset=/stencilsets/bpmn1.1/bpmn1.1.json";
editor=window.open(g);
editor.oryxCreator1283772618640=self;
editor.oryxCreator1283772618640.variant=f
...
if(!(c instanceof Text)){if(c.hasAttribute("id")){Ext.Msg.alert(c.getAttributeNode("id").nodeValue);
d.push({nodeType:c.getAttributeNode("type").nodeValue,nodeId:c.getAttributeNode("id").nodeValue,nodeLabel:c.getAttributeNode("label").nodeValue,nodeType2:c.getAttributeNode("type2").nodeValue})
}else{if((c.hasAttribute("from"))&&c.hasAttribute("to")){d.push({edgeType:c.getAttributeNode("type").nodeValue,from:c.getAttributeNode("from").nodeValue.substring(3),to:c.getAttributeNode("to").nodeValue.substring(3)})
}}}}return d
