...
}get ,showOverlay:function(){if(this.active){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_OVERLAY_HIDE,id:"desynchronizability"});
...
this.el=undefined;
this.callback=undefined;
this.facade.offer({name:"Enforceability",functionality:this.showOverlay.bind(this),group:"Enforceability",icon:ORYX.PATH+"images/checker_validation.png",description:"enforce?",index:3,minShape:0,maxShape:0})
}get ,showOverlay:function(){if(this.active){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_OVERLAY_HIDE,id:"enforceability"});
this.active=!this.active
}else{try{var b=this.getRDFFromDOM();
