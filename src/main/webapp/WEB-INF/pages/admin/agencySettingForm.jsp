...
<script type="text/javascript">
	...
	function checkIsLoginUser(){
		...
		if(agentIds==""){
			validateMessageBox(form.title.msgBoxTitleValidation,form.error.agentValidation, "info");
			$("#agentFullName").val("");
			return false;
		}else{
			...
			for(var index=0;index<ids.length;index++){
				...
				if(currentUserId==ids[index]){
					...
					validateMessageBox(form.title.msgBoxTitleValidation, form.msg.loginUser+" "+usersFullName+" "+form.error.cantBeAgent , "info");
					$("#agentFullName").val("");
					$("#agent").val("");
					return false;
				}
		...
		if(startDate==""){
			validateMessageBox(form.title.msgBoxTitleValidation, form.error.startDateEmptyValidation , "info");
			return false;
		}
		
		if(endDate==""){
			validateMessageBox(form.title.msgBoxTitleValidation, form.error.endDateEmptyValidation , "info");
			return false;
		}
		...
		if(startDate>endDate){
			$("#startDate").val("");
			$("#endDate").val("");
			validateMessageBox(form.title.msgBoxTitleValidation, form.error.startDateLesserValidation , "info");
			return false;
		}
		...
		if(count==0){
			validateMessageBox(form.title.msgBoxTitleValidation, form.error.selectNotificationType , "info");
			$("#isMail").focus();
			return false;
		}
		return true;
