...
<head>
    ...
    $(function () {
      ...
      if(isNewTable){
		   var htmlContent = "<TABLE id='defaultValueTable' cellpadding='1' cellspacing='1'  width='100%'>";
	   		var defaultList = $("#tableDefaultValues").val();
			var defaultFields = defaultList.split('-');
			 for(var index = 0; index < defaultFields.length; index++){
					...
					htmlContent = htmlContent + "<tr><td style='padding: 3px;'> <input style='width:125px;'  type='text' readonly='true'  value='"+columnField[0]+"' /></td>"
					+"<td style='padding: 3px;'> <input  style='width:125px;' readonly='true' type='text' value='"+columnField[1]+"' /></td>"
					+"<td style='padding: 3px;'> <select style='width:135px;margin-bottom:0px;'  readonly='true' value='"+columnField[2]+"'><option value='"+columnField[2]+"'>"+columnField[2]+"</option></select></td>"
					+"<td style='padding: 3px;'> <input style='width:30px;' readonly='true' type='text' value='"+columnField[3]+"' /></td>"
					+"<td style='padding: 3px;'> <input style='width:75px;' readonly='true' type='text' value='"+columnField[4]+"' /></td>"
					+"<td style='padding: 3px;'> <input  style='width:125px;' readonly='true' type='text' /></td>"
					+"<td style='padding: 3px;'> <input  style='width:15px;' disabled='disabled' type='checkbox' "+columnField[5];
					if(columnField[5]=='true'){
    ...
    function addRow(tableID) {
    	...
    	var isUniqueKey = row.insertCell(6);
        ...
        uniqueKeyElement.setAttribute("onClick", "handleClick(this);");
        ...
        element6.setAttribute("onClick", "addRow('dataTable')");
        ...
        element7.setAttribute("onClick", "deleteTableRow(this)");
    ...
    function activateAutoId(typeCombo,rowCount,tableID){
        ...
        if(typeCombo.value=="VARCHAR"){
        	 row.cells[5].childNodes[0].removeAttribute("readonly");
    	}else{
    		 row.cells[5].childNodes[0].setAttribute("readonly", true);
   	 ...
   	 <c:forEach items="${metaTable.metaTableColumns}" var="columnsField" varStatus="status">
		   ...
		   var defaultList = $("#tableDefaultValues").val();
			var defaultFields = defaultList.split('-');
			 for(var index = 0; index < defaultFields.length; index++){
					 var columnField=defaultFields[index].split(',');
					 if(columnField[0]=="${columnsField.name}"){
						 isReadOnly=true;
					 }
			}
    ...
    function constructTableForAlterTable(editableList,tableID){
    	...
    	for(var index=0;index<editableList.length;index++){
	        ...
	        cell6.style.padding="3px";
	        if(editableList[index].isReadOnly){
	        	cell6.style.width="20px";
	        }else{
	        	element6.setAttribute("src", "/images/delete.gif");
		        element6.setAttribute("onClick", "deleteExistingRow(this)");
		        cell6.appendChild(element6);
	        }
    ...
    function deleteExistingRow(tabObj) {
    	...
    	try {
							...
							if(table_name.length<=56){
								$.ajax({	
									...
									success : function(response) {
										...
										if(tableObj=="true"){
											$.msgBox({
											    title: form.title.msgBoxTitleTableName,
		 ...
		 function checkTableModuleName(){
			 ...
			 if(modulename==""){
				 $.msgBox({
					    title: form.title.msgBoxTitleModuleName,
        ...
        function getData(tableID) {
        	try {
	            ...
	            for(var index=0; index < rowCount; index++) {
	                ...
	                var columnName = row.cells[0].childNodes[0];
	                var columnChineseName = row.cells[1].childNodes[0];
	                var columnType = row.cells[2].childNodes[0];
	                var columnSize = row.cells[3].childNodes[0];
	                var defaultValue = row.cells[4].childNodes[0];
	                var autoGenerationId = row.cells[5].childNodes[0];
	                var isUniqueKey = row.cells[6].childNodes[0];
	                ...
	                if(defaultValue.value==""){
	                	...
	                	if(columnType.value == "INT" || columnType.value == "DECIMAL" || columnType.value == "DOUBLE" || columnType.value == "TINYINT" || columnType.value == "YEAR" || columnType.value == "SMALLINT" || columnType.value == "MEDIUMINT" || columnType.value == "FLOAT" || columnType.value == "NUMERIC" || columnType.value == "BOOLEAN" ) {
        	 ...
        	 if(isNewTable){
     			...
     			var defaultFields = defaultList.split('-');
     			 for(var temp = 0; temp < defaultFields.length; temp++){
     				 ...
     				 mapObj['columnName'] = columnField[0];
     	                mapObj['columnType'] = columnField[2];
     	                mapObj['columnSize'] = columnField[3];
     	                mapObj['columnChineseName'] = columnField[1];
     	                mapObj['defaultValue'] = columnField[4];
     	                mapObj['autoGenerationId'] = "";
     	              	mapObj['isUniqueKey'] = columnField[5];
     	                jsonArray[rowCount]=mapObj;
     	                rowCount++;
     			}
            	...
            	for(var index=0; index < alterTableRowCount; index++) {
	                ...
	                if(checkTableIsAlter(columnName.getAttribute('id'),columnName.value,columnChineseName.value,columnType.value,columnSize.value,assignDefaultValue,autoGenerationId.value,isUniqueKey.checked)){
	...
	function isRelationshipAltered(selectedTableList,currentTableList){
        ...
        function checkTableIsAlter(id,fieldName,ChineseName,type,size,defaultValue,autoGenerationId,isUnique){
        	<c:forEach items="${metaTable.metaTableColumns}" var="columnsField" varStatus="status">
        		if(id=="${columnsField.id}"){
	        		if("${columnsField.name}"!=fieldName || "${columnsField.chineseName}"!=ChineseName || "${columnsField.type}"!=type || "${columnsField.length}"!=size || "${columnsField.defaultValue}"!=defaultValue || "${columnsField.autoGenerationId}"!=autoGenerationId || ${columnsField.isUniquekey}!=isUnique){
	    				return true;
	    			}else{
	        			return false;
	        		}	
        ...
        function checkAllFieldsSaved(){
    		 ...
    		 if(columnNames.length !=0){
    			 validateMessageBox(form.title.msgBoxTitleMessage,form.error.relationPropertiesValidation,"info",false);
    			setTimeout(function() {
 		...
 		break;
                 ...
                 if(!isNaN(columnSizeValue) && columnTypeValue!="DECIMAL" && columnTypeValue!="NUMERIC" && columnTypeValue!="DOUBLE"){
                	 ...
                	 if(defaultValue.value!="" && defaultValue.value!="NULL" && (columnTypeValue=="YEAR" || columnTypeValue=="BOOLEAN" || columnTypeValue=="VARCHAR" || columnTypeValue=="INT" || columnTypeValue=="CHAR" || columnTypeValue=="TINYINT" || columnTypeValue=="SMALLINT" || columnTypeValue=="MEDIUMINT" || columnTypeValue=="TIME" ||  columnTypeValue=="DATE" || columnTypeValue=="DATETIME")){
                    	 ...
                    	 row.cells[4].childNodes[0].value="00:00:00";
                     }
             		 ...
             		 if(columnTypeValue=="VARCHAR" || columnTypeValue=="INT" || columnTypeValue=="CHAR" || columnTypeValue=="TINYINT" || columnTypeValue=="SMALLINT" || columnTypeValue=="MEDIUMINT"){
                 		 if(columnSizeValue<=0 || columnSizeValue>255){
                    			 $.msgBox({
                          		    title:form.title.msgBoxTitleCheck,
        ...
        function setDefaultValueInTable(typeCombo,rowCount,tableID){
        	var defaultValueMap={"VARCHAR":"100","DECIMAL":"19,2","CHAR":"10","INT":"10","YEAR":"4"};
        	var table = document.getElementById(tableID);
        	var row = table.rows[rowCount];
        	
        	for(key in defaultValueMap ){
        		if(typeCombo.value==key){
        			row.cells[3].childNodes[0].value=defaultValueMap[key];
        			break;
        		}else{
        			if(typeCombo.value == "LONGTEXT" || typeCombo.value == "DATE" || typeCombo.value == "DATETIME") {
               		 	row.cells[3].childNodes[0].setAttribute("readonly", true);
        			}
        			row.cells[3].childNodes[0].value="";
        		}
        	}
...
<%-- 					<div align="center" id="wizardTab_div-1" class="active-step"><a href="javascript:void(0);" id="wizardTab-1" class="fontSize13">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="table.properties"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></div> --%>
<%-- 					<div align="center" id="wizardTab_div-2" class="completed-step"><a href="javascript:void(0);" id="wizardTab-2" class="fontSize13" onclick="javascript:checkAllFieldsSaved();">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="table.relation.properties"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></div> --%>
<%-- 				</c:if> --%>
<!-- 				</ul> -->
<!-- 		</div> -->
 ...
 <div class="span7 scroll">
		<div>
			  ...
			  <div id="wizardTab-content">
			    ...
			    <tr id="tableNameRow">
			      <%--   <td width="271" height="40"><eazytec:label styleClass="control-label style3 style18 " key="metaTable.tableName"/></td> 
			     <td width="271" height="40"> <span id="tableNameLable" class="style18 style3"><label><fmt:message key="table.field.name"/><span class="required">*</span></label></span></td>
		        ...
		        <%-- <tr id="moduleRow">
       			 	 <td width="271" height="40" ><span id="moduleNameLable" class="style18 style3"><label><fmt:message key="metaTable.moduleName"/><span class="required">*</span></label></span></td>
			         <td width="436" class="uneditable-input1"><input type="hidden" autocomplete="off" list="searchModuleresults" id="modules" class="medium" name="_moduleName" onblur="setFormModuleDetails('${moduleId}','false')">
