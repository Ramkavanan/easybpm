...
<script type="text/javascript">
				...
				if(/^[\-a-zA-Z0-9\_\u4e00-\u9eff\s]{1,100}$/.test(documentFormName)) {
						$.ajax({	
							...
							success : function(response) {
								...
								if(isDuplicate) {
									$('#name').val('');
									openMessageBox(form.title.msgBoxTitleError,form.error.documentFormNameExist,"error", true);
							        event.preventDefault();
								}else {
									return true;
								}
					...
					$('#name').val('');
					openMessageBox(form.title.msgBoxTitleError,form.error.documentFormNameSpecialCharacterValidation,"error", true);
					event.preventDefault();
				}
			}else {
				openMessageBox(form.title.msgBoxTitleError,form.error.documentFormNameEmptyValidation,"error", true);
		        event.preventDefault();
			}
	});
   ...
   /*  function deleteRow(row){
        if(count<=0){
        	$("#uploadDocuments td").each(function () {
        	...
        	$.msgBox({
        		...
        		content: form.error.noPrivilege
    		...
    		return false;
		     ...
		     document.getElementById('uploadDocuments').deleteRow(delRow);
		     count--;
    	 }
     }
	...
	<spring:bind path="documentForm.name">
  	...
  	<%-- <div id="user_permission_create_links" align="right" class="displayNone">
          <a class="padding10" href="#bpm/dms/showDmsUserPermission" onClick="showDmsUserPermission('document');" ><fmt:message key="button.createNew"/></a>
          <a class="padding10" href="#" onClick="deletePermission('user');" ><fmt:message key="button.delete"/></a>
     </div>
      
     <div id="role_permission_create_links" align="right"  class="displayNone">
          <a class="padding10" href="#bpm/dms/showDmsRolePermission" onClick="showDmsRolePermission('document');" ><fmt:message key="button.createNew"/></a>
          <a class="padding10" href="#" onClick="deletePermission('role');" ><fmt:message key="button.delete"/></a>
     </div>
      
     <div id="group_permission_create_links" align="right"  class="displayNone">
          <a class="padding10" href="#bpm/dms/showDmsGroupPermission" onClick="showDmsGroupPermission('document');" ><fmt:message key="button.createNew"/></a>
          <a class="padding10" href="#" onClick="deletePermission('group');" ><fmt:message key="button.delete"/></a>
     </div> --%>
