...
function printWorkflowInstanceGrid(obj){
	...
	if(rowid.length > 2){
		validateMessageBox(form.title.message, form.msg.selectAtleastSingleProcessToRead, false);
	} else {
		showPrintTemplate(gridObj.jqGrid('getCell', item, 'id'),gridObj.jqGrid('getCell', item, 'formkey'),gridObj.jqGrid('getCell', item, 'formId'));
	} */
}
...
function readAllTask() {
	...
	var rowid =  gridObj.getGridParam('selarrrow'); 
	if(rowid.length ==0){
		validateMessageBox(form.title.message, form.msg.selectSingleRowToPrint, false);
	} else {
			...
			<div class="page-header">
				<div class="pull-left">
					<c:if test="${gridType == 'mybucket'}">
						<h2><fmt:message key="task.myBuckets"/></h2>
					</c:if>
					<c:if test="${gridType == 'myOwnedTasks'}">
						<h2><fmt:message key="task.myOwnedTasks"/></h2>
					</c:if>
					<c:if test="${gridType == 'toRead'}">
						<h2><fmt:message key="task.toRead"/></h2>
					</c:if>
					<c:if test="${gridType == 'doneList'}">
						<h2><fmt:message key="task.myBuckets"/></h2>
					</c:if>
					<c:if test="${gridType == 'readedList'}">
						<h2><fmt:message key="task.readList"/></h2>
					</c:if>
					<c:if test="${gridType == 'waitinglist'}">
						<h2><fmt:message key="task.waitingList"/></h2>
					</c:if>
				</div>
				<div id="header_links" align="right" style="padding: 5px 0">
					<c:if test="${gridType == 'mybucket'}">
      						<c:if test="${not empty isSecretaryExist}">
							<c:if test="${isSecretaryExist == 'true'}">
								<a class="padding10" id="substitute" href="javascript:void(0);" data-role="button" data-theme="b"  onClick="assignSubstitute('${substitute}')"  data-icon=""><button class="btn">Substitute Conduction</button></a>
							</c:if>
						</c:if>	 								
					</c:if>
					<c:if test="${gridType == 'toRead'}">
						<a class="padding10" href="javascript:void(0);" onclick="readAllTask();"><button class="btn">Bulk Read</button></a>
					</c:if>					
					<c:if test="${gridType == 'myOwnedTasks'}">
						<!-- <input type="text" name="searchBox" id="searchBox" onkeyup="_triggerSimpleSearch(this);"/> -->
						<a class="padding10" href="javascript:void(0);" id="suspendTasks" onclick="operateBulkTasks('suspend')"><button class="btn"><fmt:message key="task.suspend"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="restoreTasks" onclick="operateBulkTasks('restore')"><button class="btn"><fmt:message key="task.restore"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="terminateTasks" onclick="operateBulkTasks('terminate')"><button class="btn"><fmt:message key="task.terminate"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="backTasks" onclick="operateBulkTasks('back')"><button class="btn"><fmt:message key="task.back"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="replaceTasks" onclick="operateBulkTasks('replace')"><button class="btn"><fmt:message key="task.replace"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="deleteTasks" onclick="operateBulkTasks('delete')"><button class="btn"><fmt:message key="task.delete"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="exportTasks" onclick="selectWorkFlowExportType()"><button class="btn"><fmt:message key="task.exportTasks"/></button></a>
						<a class="padding10" href="javascript:void(0);" id="print" onclick="printWorkflowInstanceGrid(this)"><button class="btn"><fmt:message key="task.print"/></button></a>						
					</c:if>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
 ...
 $('#_MY_BUCKET').setGridParam({
	 onPaging: function(pgButton) {
			var currentPage = $(".ui-pg-input").val();
			if(pgButton == 'next__MY_BUCKET_PAGER') {
				currentPage = parseInt(currentPage)+1;
			} else if(pgButton == 'prev__MY_BUCKET_PAGER') {
				currentPage = parseInt(currentPage)-1;				
			} else if(pgButton == 'last__MY_BUCKET_PAGER') {
				currentPage = parseInt(1);
			} else if(pgButton == 'first__MY_BUCKET_PAGER') {
				currentPage = parseInt(1);
			}
			// get offset 
			var totalRecordCount = '${totalRecordCount}';
			//alert("==currentPage=="+currentPage+"===totalRecordCount==="+totalRecordCount);

			var totalpages = 1, offset = 0 , limit = 16;
			if(currentPage > 1) {
			    if (totalRecordCount > 0) {
				      totalpages = Math.ceil(totalRecordCount / limit );
						//alert("==totalpages=="+totalpages+"===="+totalRecordCount / limit);
				     } else {
				      totalpages = 0;		    	  
				    }
			...
			$.ajax({
			        ...
			        dataType: 'json',
			        async: false,
			        data: 'type=mybucket&currentPage='+currentPage+'&offset='+offset,
					success : function(response) {
						if(response){
							$("#_MY_BUCKET").jqGrid('clearGridData');
							$("#_MY_BUCKET").jqGrid('setGridParam', {page: 1, data: response}).trigger('reloadGrid');
...
$(document).ready(function() {
	...
	//$('#next__MY_BUCKET_PAGER').removeClass('ui-state-disabled'); 
