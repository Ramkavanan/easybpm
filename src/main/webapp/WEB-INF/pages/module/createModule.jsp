...
<script type="text/javascript">
	function validateModuleForm() {	
		...
		if (!name){
			openMessageBox(form.title.msgBoxTitleError,form.error.moduleNameRequired,"error", true);
			return false;				
		...
		<%--if (!($('#isParent').is(":checked"))){
			var parentModule = document.getElementById("parentModule").value;
			if(!parentModule){
				openMessageBox("Error","Parent Module is required filed","error", true);
				changeWizardByButton('prev');
				return false;				
			}
		}
		
		if($('#private').is(":checked")){
			var administrator = document.getElementById("administrators").value;
			if(!administrator){
				openMessageBox("Error","Administrator is required filed","error", true);
				changeWizardByButton('prev');
				return false;				
			}
		}
		if($('#public').is(":checked")){
			...
			if(!departments){
				openMessageBox("Error","Department is required filed","error", true);
				changeWizardByButton('prev');
				return false;				
			}
		}	--%>
