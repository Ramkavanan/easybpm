...
function getCkEditorElem(mode){
	...
	if(formName != ""){
		...
		if(htmlSourceText.indexOf("<form") == -1 && htmlSourceText.indexOf(formName) == -1){
			validateMessageBox("Validation", "Form is not present." , "info");
			return false;
		}
	    		...
	    		if(tempFormname!="" ){
	    			if(!(tempFormname.indexOf(' ') > -1)){
			    		...
			    		if(tempTablename!=""){
			    			...
			    			setTimeout(function() {
				    			openMessage("Error",form.error.emptytablename,"error",false,tempFormname,tempTablename,tempIsTemplate,tempDefault);
				    		}, 500);
			    		}
	    			}else{
							//alert(tempFormname);
	    				setTimeout(function() {
			    			openMessage("Error",form.error.formnamespace,"error",false,tempFormname,tempTablename,tempIsTemplate,tempDefault);
			    		}, 500);
	    			}
		    	}else{
		    		setTimeout(function() {
		    			openMessage("Error",form.error.emptyformname,"error",false,tempFormname,tempTablename,tempIsTemplate,tempDefault);
		    		}, 500);
		    	}
...
function getFormNameWithValues(enteredFormName, enteredTableValue,isTemplateValue,isDefaultValue){
	...
	$.msgBox({ type: "prompt",
	    ...
	    { header: form.title.moduleName, type: "hidden", id: "_moduleName", name: "_moduleName", autocomplete: "true", datalistId : "searchModuleresults", onblur: "setFormModuleDetails('_moduleId','true')"},
	      { header: "Description &nbsp;&nbsp;", type: "text", name: "_description", id:"_description"},
	   /* { header: form.title.ispublic +" &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", type: "checkbox", name: "_isPublic", value:'public', onclick:'setDefaultModuleDetails(this)'},*/
...
function setFormName(tempFormname, isSave, tempTablename){
	if(/^[\-a-zA-Z\u4e00-\u9eff\s][\-a-zA-Z\u4e00-\u9eff\s0-9]*$/.test(tempFormname) !=false){
		...
		$("#formName").val(""); 
		setTimeout(function() {
			openMessage(form.title.msgBoxTitleError,form.error.specialcharformname,form.title.msgBoxTitleerror, false);
		}, 500);
	}
}
...
function copyForms(){
	...
	if(selectedRowId!=""){
		if(selectedRowId.length>1){
			validateMessageBox(form.title.validation, form.title.copyFormError  , form.title.msgBoxTitleinfo);
		}else{
			$.msgBox({
				...
				success : function(result, values) {
					if (result == "Yes") {
						...
						if(selectedModuleId!=""){
							if(newFormName==""){
								setTimeout(function() {
									openModuleMessage(form.title.validation, form.msg.mandatoryName , form.title.msgBoxTitleinfo,false,moduleId);
								}, 500);
							...
							setTimeout(function() {
								openModuleMessage(form.title.validation, form.msg.moduleCopy , form.title.msgBoxTitleinfo,false,moduleId);
							}, 500);
						}
					}
				}
		...
		validateMessageBox(form.title.validation, form.msg.copyErrorSelect , form.title.msgBoxTitleinfo);
	}
...
function checkCopyFormName(newFormName,moduleId){
	...
	if(!(newFormName.indexOf(' ') > -1)){
		if(/^[\-a-zA-Z\u4e00-\u9eff\s][\-a-zA-Z\u4e00-\u9eff\s0-9]*$/.test(newFormName) !=false){
			...
			if(da.isPresent == true){
				setTimeout(function() {
					openMessageCopyForm(form.error.formexists,form.msg.differentformname,form.title.msgBoxTitleinfo, moduleId);
				}, 500);
			}else{
				status=true;
			}
			...
			setTimeout(function() {
				openMessageCopyForm(form.title.msgBoxTitleError,form.error.specialcharformname,form.title.msgBoxTitleerror, moduleId);
			}, 500);
		}
	}else{
		setTimeout(function() {
			openMessageCopyForm(form.title.msgBoxTitleError,form.error.formnamespace,form.title.msgBoxTitleerror,moduleId);
		}, 500);
	}
	return status;
}
...
function setCopyModuleId(moduleId) {
	...
	if (moduleName != "") {
		...
		if (moduleVal.length == 0) {
			...
			//if(copyModuleId!=moduleId){
				$("input[name='_copyModuleId']").val(moduleVal.attr('id'));
			/*}else{
				validateMessageBox("Message", "Cannot copy to same module.","info", false);
			}*/
		}
	}
}
