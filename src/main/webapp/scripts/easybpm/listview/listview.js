...
function columnsPropertyGrid(){
	 	$("#_LIST_VIEW_COLUMNS_PROPERTY").jqGrid({
		    ...
		    colNames:['Id', 'Column Title', 'Data Fields', 'Move Up','Move Down', 'Column Width','Order By','Replace Words','Is Sort','Is Advanced Search','Is Simple Search','Text Align','Is Hidden','Render Event','Render Event Name','Comment','Is Group By','Field Type', 'Data Dictionary', 'Is Range','Column Type'],
		    colModel :[{
				    name: 'id'
				 	,index: 'id'
				 	,width: 100
...
function updateListViewColumnOrderNo(gridArrayList) {
	...
	$.ajax({
       ...
       //jQuery("#list10").setGridParam({rowNum:10,datatype:"json" }).trigger('reloadGrid'); 
  			...
  			//	 jQuery('#_LIST_VIEW_COLUMNS_PROPERTY').setGridParam({sortname:'orderBy', sortorder: 'ASC'}).trigger('reloadGrid');	
					
//	    	validateMessageBox("Success", "List View Column Order is updated successfully.", "success");
        },
			...
			if(!alreadyExist){
				...
				if(!isNameExist(columnTitle,'_LIST_VIEW_COLUMNS_PROPERTY')){
				    ...
				    $("#columnTitle").focus();
						...
						validateMessageBox(form.title.information, form.msg.columnAddedSuccessfully , form.title.msgBoxTitleinfo);
				}else{
					validateMessageBox(form.title.validation, form.msg.listViewColumnTitleErrorNameExist , form.title.msgBoxTitleinfo,"columnTitle");
				}
			}else{
				validateMessageBox(form.title.validation, "Order number is empty or exists already" , form.title.msgBoxTitleinfo,"columnTitle");
				$("#columnsOrder").focus();
			}
			
		}else{
			validateMessageBox(form.title.validation, form.msg.listViewColumnTitleErrorRestrictedCharacters , form.title.msgBoxTitleinfo,"columnTitle");
		}
	}else{
		validateMessageBox(form.title.validation, form.msg.listViewColumnTitleErrorNotEmpty , form.title.msgBoxTitleinfo,"columnTitle");
	}
	$("#_LIST_VIEW_COLUMNS_PROPERTY").jqGrid('resetSelection');
}
...
function _deleteRecord(){
	...
	/*var data = $("#_LIST_VIEW_COLUMNS_PROPERTY").jqGrid('getRowData', sel_id); 
		...
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewNoRecords , form.title.msgBoxTitleinfo);
	}else{
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewSelectRow , form.title.msgBoxTitleinfo);
		$("#columnTitle").focus();
	}
	_clearRecord();
			...
			$.ajax({	
				...
				success : function(response) {
					if(response.successMessage){
						$.msgBox({
							title: form.title.msgBoxTitleSuccess,
						...
						validateMessageBox(form.title.msgBoxTitleError, response.errorMessage , form.title.msgBoxTitleerror);
					}
				}
		  	});
		}else{
			validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewColumnProperty , form.title.msgBoxTitleInfo);
			$("#columnTitle").focus();
		}
	}else{
		validateMessageBox(form.title.msgBoxTitleError, form.msg.listViewEntityError , form.title.msgBoxTitleerror);
	}
}
...
function _checkListViewQuery(){
	...
	if(/^[\-a-zA-Z\_\u4e00-\u9eff]*$/.test(listViewName) == false) {
		$.msgBox({
			      title: form.title.msgBoxTitleError,
...
function isValidFieldValues(){
	...
	if(selectColumn==""){
		$.msgBox({
			title: form.title.msgBoxTitleCheck,
		...
		if(!isSpecialCharacterFound($("#buttonDisplayName").val())){
			if($("#buttonScript").val()!="" || $("#buttonDisplayName").val() == "Create New" ){
				if(!isNaN($("#buttonOrder").val()) && !isOrderNumberExist($("#buttonOrder").val(), '_LIST_VIEW_BUTTONS_PROPERTY')){
					...
					_clearButtonRecord();
					validateMessageBox(form.title.information, form.msg.buttonAddedSuccessfully , "info");
					 $("#buttonDisplayName").focus();
					}else{
						validateMessageBox(form.title.validation, form.msg.listViewErrorDisplayName , form.title.msgBoxTitleinfo,"buttonDisplayName");
					}
				}else{
					validateMessageBox(form.title.validation, form.msg.listViewErrorOrderNumber , form.title.msgBoxTitleinfo,"buttonOrder");
				}
			}else{
				validateMessageBox(form.title.validation, form.msg.listViewErrorButtonScript, form.title.msgBoxTitleinfo,"buttonScript");
			}
		}else{
			validateMessageBox(form.title.validation, form.msg.listViewErrorRestrictedCharacters , form.title.msgBoxTitleinfo,"buttonDisplayName");
		}
	}else{
		validateMessageBox(form.title.validation, form.msg.listViewErrorEmptyName , form.title.msgBoxTitleinfo,"buttonDisplayName");
	}
...
function listViewAdvanceSearch(listViewId){
	...
	openSelectDialogForFixedPosition("popupDialog","","800","260","Advanced Search");//height Changed from 300 to 230
...
function _deleteButtonRecord(){
	...
	/*	var data = $("#_LIST_VIEW_BUTTONS_PROPERTY").jqGrid('getRowData', sel_id);
		if(data.displayName == "Create New" || data.displayName == "Restore" || data.displayName == "Recycle" || data.displayName == "Delete" || data.displayName == "Print"){
			validateMessageBox(form.title.msgBoxTitleMessage, form.msg.cannotDeleteDefaultButtons , form.title.msgBoxTitleinfo);
		}else{
			if(data.id){
	...
	$.msgBox({
		    ...
		    success: function (result) {
		        if (result == "Yes") {
		        	var data = $("#_LIST_VIEW_BUTTONS_PROPERTY").jqGrid('getRowData', sel_id);
		if(data.displayName == "Create New" || data.displayName == "Restore" || data.displayName == "Recycle" || data.displayName == "Delete" || data.displayName == "Print"){
							setTimeout(function(){validateMessageBox(form.title.msgBoxTitleMessage, form.msg.cannotDeleteDefaultButtons , form.title.msgBoxTitleinfo);},500);
	$("#buttonDisplayName").focus();
		}else{
			if(data.id){
				$("#_LIST_VIEW_BUTTONS_PROPERTY").jqGrid('delRowData', sel_id);
...
function showListView(listViewName,title){
	if(checkListViewName(listViewName, false)){
		...
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewErrorListView , form.title.msgBoxTitleinfo);
	}
}
...
function showTimingTaskListViews(listViewName,title){
	if(checkListViewName(listViewName, false)){
		...
		document.location.href = "#bpm/listView/showListViewGrid";
		_execute('target',listViewParam);
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewSuccessTaskDelete , form.title.msgBoxTitlesuccess);
	}else{
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewErrorListView, form.title.msgBoxTitleinfo);
	}
}
							...
							if(groupFieldsName != parentGroup){
								...
								var rowCount = $("#_LIST_VIEW_GROUP_SETTING_PROPERTY").jqGrid('getGridParam', 'records');
								 ...
								 _clearGroupSettingRecord();
									validateMessageBox(form.title.information, form.msg.groupAddedSuccessfully , "info");
							}else{
								validateMessageBox(form.title.validation, form.msg.listViewErrorSameParentName , form.title.msgBoxTitleinfo);
							}
						}else{
							validateMessageBox(form.title.validation, "Group name "+groupName+" is a Parent group its type cannot be Single." , form.title.msgBoxTitleinfo);
						}
					
				}else{
					validateMessageBox(form.title.validation, form.msg.listViewErrorGroupName, form.title.msgBoxTitleinfo);
				}
			}else{
				validateMessageBox(form.title.validation, form.msg.listViewOrderNumber , form.title.msgBoxTitleinfo);
				$("#groupOrder").focus();
			}
		}else{
			validateMessageBox(form.title.validation, form.msg.listViewRestrictedCharacters , form.title.msgBoxTitleinfo);
		}
	}else{
		validateMessageBox(form.title.validation, form.msg.listViewGroupFields , form.title.msgBoxTitleinfo);
	}
...
function _deleteGroupSettingRecord(){
	...
	if(sel_id != null){
		...
		if(!isGroupHavingChildGroup(data)){
			...
			validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewRemoveChild , form.title.msgBoxTitleinfo);
		}
	}else if(rowCount == 0){
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.listViewNoRecords , form.title.msgBoxTitleinfo);
	}else{
...
function _saveOrUpdateAllGroupSettingRecord(listViewId){
	...
	if(listViewId){
		...
		if(groupSettingProperty.length > 0){
			...
			validateMessageBox(form.title.msgBoxTitleMessage , form.msg.listViewGroupSettingChanges , form.title.msgBoxTitleinfo);
		}
	}else{
		validateMessageBox(form.title.msgBoxTitleError , form.msg.listViewEntityError, form.title.msgBoxTitleerror);
	}
...
function getGroupSettingchildNodes(currentNode, rootNode, groupByFields, listViewId, parentGroup, whereParams){ 
	...
	$.ajax({
		...
		error : function(response){
			data=response;
			validateMessageBox(form.title.msgBoxTitleError, form.msg.listViewError , form.title.msgBoxTitleerror);
		}
	});
	return data;
}
...
function selectTableColumnsTree(){
	...
	if(selectedFromQuery!=""){
		...
		$.msgBox({
		    ...
		    type: form.title.msgBoxTitleinfo,
			 afterClose:function (){
				  $("#fromQuery").focus() ;
			}
		});
...
function deleteOrRestoreGridData(listViewId, listViewName, isDelete, tableName, columnName){
	...
	if(!isAdmin){
		validateMessageBox(form.title.validation, form.msg.permissionRestricted , form.title.msgBoxTitleinfo);
		return false;
	}
	if(tableName != "" && columnName != ""){
		...
		if(sel_id.length > 0){
			...
			validateMessageBox(form.title.validation, form.msg.listViewSelectRow , form.title.msgBoxTitleinfo);
		...
		validateMessageBox(form.title.validation, form.msg.listViewSelectColumn , form.title.msgBoxTitleinfo);
	}
}
...
function restoreListViewGrid(listViewId, listViewName, isDelete, tableName, columnName){
	...
	if(isEdit=='false'){
		validateMessageBox(form.title.validation, form.msg.listViewRestrictRestorePermission , form.title.msgBoxTitleinfo);
		return false;
	}
		var sel_id = $('#_'+listViewName).jqGrid('getGridParam', 'selarrrow');
...
function processToExecuteQuery(){
	...
	if(query!=""){
		if(query.indexOf("DROP")!=-1){
			validateMessageBox(form.title.validation, form.msg.queryDropQuery , form.title.msgBoxTitleinfo);
			return false;
		}
			...
			success : function(response) {
				if(response.success){
					if(query.indexOf("SELECT")!=-1){
						...
						validateMessageBox(form.title.validation, form.msg.querySuccess , form.title.msgBoxTitleinfo);
					}
		...
		validateMessageBox(form.title.validation, form.msg.queryValidation , form.title.msgBoxTitleinfo);
	}
}
...
function bulkDeleteListView(){
	...
	if(rowid.length !=0){
		...
		$.msgBox({
   		    title:form.title.msgBoxTitleMessage,
...
function deleteAllListViewConfirm(listViewIds){
	...
	$.msgBox({
	    ...
	    afterClose : function(){
	    	if(isNeededMsg){
	    		if(isSuccess){
		    		validateMessageBox(form.title.msgBoxTitleSuccess, message , success);
				}else{
					validateMessageBox(form.title.msgBoxTitleError, message , form.title.msgBoxTitleerror);
				}
	    	}
	    }
	});
