...
/************************************
	...
	if(rowid.length !=0 ){
		if(departmentTypes.indexOf('root') > -1){
			$.msgBox({
	   		    title:"Message",
	   		    content:"You cannot delete root Department",
	   		});
		}else{
			deleteAllDepartmentsConfirm(deptId);
		}
	}
	if(rowid.length ==0){
		$.msgBox({
   		    title:"Message",
   		    content:"Please select atleast single department to Delete",
   		});
	}
...
function updateDataDictionaryOrderNo(dictionaryList) {
	$.ajax({
        ...
        success : function(response) {
			data=response;
//			validateMessageBox("Success", "Data Dictionary Order is updated successfully.", "success");
        },
...
function updateDepartmentOrderNo(gridArrayList) {
	$.ajax({
        ...
        success : function(response) {
			data=response;
        //	validateMessageBox("Success", "Department Order is updated successfully.", "success");
        },
		error : function(response){
				data=response;
...
function updateGroupOrderNo(gridArrayList) {
	$.ajax({
        ...
        success : function(response) {
			data=response;
    // 		validateMessageBox("Success", "Group Order is updated successfully.", "success");
	    },
		error : function(response){
				data=response;
...
function updateRoleOrderNo(gridArrayList) {
	$.ajax({
        ...
        success : function(response) {
			data=response;
//	    	validateMessageBox("Success", "Role Order is updated successfully.", "success");
        },
		error : function(response){
				data=response;
...
function deleteAllUserOpinion(userOpinionIds){
	...
	$.msgBox({
	    title: form.title.confirm,
	    content: form.msg.confirmToDeleteWord,
	    type: "confirm",
	    buttons: [{ value: "Yes" }, { value: "No" }],
	    success: function (result) {
	        if (result == "Yes") {
