...
/*					if(url){
			    	...
			    	validateMessageBox(form.title.msgBoxTitleError, response.failure, "error");
				}
         }
       });
...
function terminate(executionId,taskId,resourceName,processDefinitionId){
	...
	$.msgBox({
	    ...
	    content: form.msg.confirm+" Terminate",
	    type: "confirm",
...
function suspend(processInstanceId,taskId,resourceName,suspendState,processDefinitionId){
	if(suspendState && suspendState == "1"){
		...
		$.msgBox({
		    content: form.msg.confirm+" Suspend",
		    type: "confirm",
		    buttons: [{ value: "Yes" }, { value: "No" }],
		    success: function (result) {
		        if (result == "Yes") {
...
function withdraw(processInstanceId,taskId,resourceName,processDefinitionId){
	...
	$.msgBox({
	    title: form.title.msgBoxTitleConfirm,
	    content: form.msg.confirm+" Withdraw",
	    type: "confirm",
	    buttons: [{ value: "Yes" }, { value: "No" }],
	    success: function (result) {
	        if (result == "Yes") {
...
function ofJointConductSubmitForm(formId, taskId){
	...
	if ( !isFormValuesValid()) {
		validateMessageBox(form.title.msgBoxTitleMessage, form.msg.mandatoryMsg , false);
	} else {
			    ...
			    success: function(response){
			    	...
			    	if(response != null && response != ""){
			    		...
			    		for(var i in response){
			    	    	if(response[i].error){
						    	validateMessageBox(form.title.msgBoxTitleError, response[i].error, "error");
			    		...
			    		if(isOrganizerExist){
			    			showNextTaskAddMembers(taskId, 'jointConduction', "Select Transactors for Next Task" , formId,JSON.stringify(organizerObj),"","", nodeType,endScriptName,false);
			    		}
			    	}else{
			    		//$( "div" ).remove( "#hidden" );
...
function ofSaveForm(formId){
	...
	if ( !isFormValuesValid()) {
		validateMessageBox("Info", "Fill required fields" , false);
	} else {
			...
			success : function(response) {
				if(response != null) {
					if(response.result == "success") {
						...
						validateMessageBox(form.title.msgBoxTitleError, response.result, "error");
					}
				}
			}
		});
	...
	if(isNextTaskExist){
		if((organizers && coordinators)){
			...
			if((organizerValues!=null && organizerValues!="")){
				...
				messageBoxShow("Warning", "Organizers should not be empty" , "info");
//				messageBoxShow(form.title.msgBoxTitleMessage,form.msg.organizerManadatory);
				return;
			}
		}else{
			messageBoxShow("Warning", "No organizer,co-ordinator element found!" , "info");
//			messageBoxShow(form.msg.noOrganizer);
			return;
		}	
	} else {
		document.getElementsByName('isLastTask')[0].value = "true";
	    ...
	    success: function(response){
	    	if(response != null && response != ""){
	    		...
	    		if(isOrganizerExist){
	    			...
	    			validateMessageBox(form.title.msgBoxTitleMessage,form.msg.formMandatoryForProcessStart,false);
	    		}else if(isError){
	    			validateMessageBox(form.title.msgBoxTitleMessage,form.msg.processStartError,false);
	    		}
	    	}
	    }
	});
		    ...
		    success: function(response){
		    	if(response != null && response != ""){
		    		...
		    		if(isOrganizerExist){
		    			...
		    			validateMessageBox("Message","Problem in starting the process",false);
		    		}*/
		    	}
		    }
		});
...
function backOffJump(deploymentId,resourceName,processName,processDefinitionId,taskId,activityId,jumpType,processInstanceId,canSave,formId){
	...
	$.myDialog.dialog({
		...
		height : 'auto',
		modal : true,
		title : "Current Process Name - " + processName
	});
	$.myDialog.dialog("open");
}
		...
		$.ajax({
		    ...
		    success: function(response){
		    	if(response != null && response != ""){
		    		...
		    		for(var i in response){
		    	    	if(response[i].error){
					    	validateMessageBox("Error", response[i].error, "error");
					    	isOrganizerExist = false;
					    	break;
		    	    	}else if(response[i].isNextTaskExist == "true"){
		    		...
		    		if(isOrganizerExist){
		    			...
		    			showNextTaskAddMembers(taskIdFromJump, 'jump', 'Select Transactors for Jump Task' ,formId,JSON.stringify(organizerObj),activityId,resourceName,nodeType,'',canSave);
		    		}
		    	}
		    }
		});
...
function bulkTasksReplace(){
	...
	if(userId!=null && userId!=""){
		...
		messageBoxShow(form.title.msgBoxTitleMessage,form.msg.addUserMandatory);
	}
}
...
function showAttachmentsForStartNode(isSameTable,isStartForm,instanceId,isForRecall){
	...
	$.ajax({
		...
		success : function(response) {			
			var attachments = "";
			var currentHtmlContent = "<br/><table width='300px' ><th colspan='4'>Attachments</th>";
					for ( var index = 0; index < response.length; index++) {
						isCurrentFileAttachment = true;
						currentHtmlContent = currentHtmlContent
								+ "<tr id="
								...
								+ response[index].id
								+ ")'>Download</a></td><td width='15%'><a href='javascript:;' onclick='viewAttachment("
								+ response[index].id + ")'>View</a></td>;"
								if(isForRecall != "yes") {
									currentHtmlContent = currentHtmlContent + "<td width='15%'><a href='javascript:;' class='removeButton' onclick='deleteAttachment("
									+ response[index].id + ")'>Remove</a></td></tr>";
								}
...
function deleteFileUploadRow(row,tableId){
	if(count>0){
		...
		messageBoxShow(form.title.msgBoxTitleMessage,form.msg.noPrevilage);
		return false;
	}
}
