...
function getFolderChildNodes(currentNode){ 
	...
	$.ajax({
		...
		error : function(response){
				data=response;
				validateMessageBox(form.title.msgBoxTitleMessage, form.msg.errorInGettingChildNodes , false);
		}
	});
	return data;
}
...
function checkPermissionFieldEmpty(form){
