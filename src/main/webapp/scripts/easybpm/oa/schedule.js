	...
	var calendar = $('#'+calenderId).fullCalendar({
		...
		select: function(start, end, allDay) {
			if ( (start < date && start.getDate() != date.getDate()) 
				...
				$.msgBox({
		   		    title:"Message",
		   		    content:"You cannot create event for past days",
		   		});
			} else{
...
function deleteSchedules(){
	...
	if(deletePermission){
		...
		$.msgBox({
   		    title:"Message",
   		    content:"You have no permission to delete this schedule , this is created by administrator.",
   		});
		return false;
	}
	if(rowid.length ==0){
		$.msgBox({
   		    title:"Message",
   		    content:"Please select atleast single event to Delete",
   		});
	}
}
...
function deleteAllSchedulesConfirm(eventIds){
	...
	$.msgBox({
	    title: "Confirm",
	    content: "Are you sure want to delete Schedule(s)?",
	    type: "confirm",
...
function validateEventForm(form){
	...
	if(startDate > endDate){
		...
		$.msgBox({
   		    title:"Message",
   		    content:"End Time should be greated than Start Time",
   		});
		return false;
	}*/else{
		var from = $('#from').val();
