...
function defaultJsonData(){
	var jsonData = '[{ "data" : "All Users",  "attr" : { "id" : "all", "name" : "All","parent" : "All" },"metadata": {"id" : "all", "name" : "All"}}, '
	   	+' {"data":"Organization","attr":{"id" : "Organization","name" : "Organization","parent" : "Organization"},"metadata": {"id" : "Organization", "name" : "Organization"}},'
	    +'{"data":"Group","attr":{"id" : "group","name" : "Group","parent" : "Group"},"metadata": {"id" : "group", "name" : "Group"}},'
	    +'{"data":"Role","attr":{"id" : "role","name" : "Role","parent" : "Role"},"metadata": {"id" : "role", "name" : "Role"}}]';
	return jsonData;
}


function defaultDeptJsonData(){
	var jsonData = '[ {"data":"Organization","attr":{"id" : "Organization","name" : "Organization","parent" : "Organization"},"metadata": {"id" : "Organization", "name" : "Organization"}}]'
	return jsonData;
}
...
function getDepartmentUserGrid(e,data){
	
	var parentName="";
	var id = data.rslt.obj.attr("name");
...
function showUserSelection(title, selectionType, appendTo, selectedValues, callAfter){
	...
	if(appendTo == "to" || appendTo == "ccto" || appendTo == "bccto"){
		...
		_execute('userPopupDialog','selectionType='+selectionType+'&appendTo='+appendTo+'&selectedValues='+selectedValues.value+'&selection=user&rootNodeURL=bpm/admin/getUserRootNodes&childNodeURL=bpm/admin/getUserSelectionChildNodes&callAfter='+callAfter+'&from=&parentId=&currentUser='+currentUser);
		
	}else{	_execute('userPopupDialog','selectionType='+selectionType+'&appendTo='+appendTo+'&selectedValues='+selectedValues.value+'&selection=user&rootNodeURL=bpm/admin/getUserRootNodes&childNodeURL=bpm/admin/getUserSelectionChildNodes&callAfter='+callAfter+'&from=&parentId=&currentUser=');
	}
...
function showRoleSelection(title, selectionType, appendTo, selectedValues, callAfter,isGraduation){
	...
	document.location.href = "#bpm/admin/showOrganizationTree";
	_execute('userPopupDialog','selectionType='+selectionType+'&appendTo='+appendTo+'&isGraduationTree='+isGraduationTree+'&selection=role&rootNodeURL=bpm/admin/getRoleNodes&childNodeURL=&selectedValues='+selectedValues.value+'&callAfter='+callAfter+'&from=&parentId=');
	$myDialog = $("#userPopupDialog");
	$myDialog.dialog({
                       width: 'auto',
                       height: 'auto',
...
function showGroupSelection(title, selectionType, appendTo, selectedValues, callAfter,isGraduation){
	...
	document.location.href = "#bpm/admin/showOrganizationTree";
	_execute('userPopupDialog','selectionType='+selectionType+'&appendTo='+appendTo+'&isGraduationTree='+isGraduationTree+'&selection=group&rootNodeURL=bpm/admin/getGroupNodes&childNodeURL=bpm/admin/getGroupNodes&selectedValues='+selectedValues.value+'&callAfter='+callAfter+'&from=&parentId=');
	$myDialog = $("#userPopupDialog");
	$myDialog.dialog({
                       width: 'auto',
                       height: 'auto',
...
function showDepartmentSelection(title, selectionType, appendTo, selectedValues, callAfter,isGraduation){
	...
	document.location.href = "#bpm/admin/showOrganizationTree";
	_execute('userPopupDialog','selectionType='+selectionType+'&appendTo='+appendTo+'&isGraduationTree='+isGraduationTree+'&selection=department&rootNodeURL=bpm/admin/getDepartmentNodes&childNodeURL=bpm/admin/getDepartmentChildNodes&selectedValues='+selectedValues.value+'&callAfter='+callAfter+'&from=&parentId=');
	$myDialog = $("#userPopupDialog");
	$myDialog.dialog({
                       width: 'auto',
                       height: 'auto',
...
function showQuickNaviURLnew(title, selectionType, appendTo, selectedValues, callAfter){
	/*openSelectDialogForFixedPosition("popupDialog","","615","480","Select Menu");
	  document.location.href = "#bpm/admin/menuTree?title='Menu'&id="+$("#menuId").val()+"&selectType='single'&selectedMenu=quickNavigationUrl"; */
...
function saveUserHomePage(){
	...
	/*if(selectedWidgets == ""){
		$.ajax({
			url: '/bpm/admin/getSelectedWidgets',
			type: 'GET',
			async: false,
			//alert(response);
			success : function(response) {
			//	console.log("response" +response);
				if(response.length > 0){
					...
					var isRender = true;
					var isSave = true ;
					userHomePagePreview(noOfColumn,response,isRender,isSave);
					
					// selectedWidgets = eval(response);
					 document.location.href = "#bpm/user/saveHomePage";
					_execute('divHomePage','noOfColumn='+noOfColumn+'&selectedWidgets='+response);
	...
	//if(selectedWidgets != ""){
		//alert(" saveuserhomePage  =======   "+selectedWidgets+ "  noOfColumn =====   "+noOfColumn);
	...
	/*}else{
		validateMessageBox("Error", "Please select atleast one widget" , "error");
		return false;
	}*/
}
...
function saveHomePage(){
		...
		$.ajax({
			...
			//	console.log("response" +response);
				if(response.length > 0){
					...
					var isRender = true;
					var isSave = true ;
					homePagePreview(noOfColumn,response,isRender,isSave);
	...
	//	alert("response else" +selectedWidgets);
	//if(selectedWidgets != ""){
		//alert(" savehomePage  =======   "+selectedWidgets+ "  noOfColumn =====   "+noOfColumn);
		document.location.href = "#bpm/user/saveHomePage";
		_execute('divsample','noOfColumn='+noOfColumn+'&selectedWidgets='+selectedWidgets+'&assignedTo='+assignedTo+'&departments='+departments+'&roles='+roles);
		//selectedWidgets = "";
		//noOfColumn = "";
...
//						$.msgBox({
//				   		    title:"Message",
//				   		    content:"The Change will be affect in next login.",
//				   		});
					return false;
				}
		    }
...
function deleteSysConfig(){
	...
	if(rowid.length !=0 ){	
		...
		if(isSystemDefined == true){
			validateMessageBox(form.title.information, form.msg.cantDeleteActiveSysConfig , "info");
		}else{
...
function showUsers(title, selectionType, callAfter){
		...
		var id = selected_nodes.attr("name");
		...
		ID	 =	id.toLowerCase();
		if( id != 'Organization' && parentId != 'all' && id!= undefined && ParentID == ID ){
			validateMessageBox(form.title.message, form.msg.selectValidRoleOrGroup, false);
...
function selectUsers(from,parentId,selectedValues){
	...
	if(selectedValues=='')
		...
		validateMessageBox(form.title.message, form.msg.selectUser, false);
		    ...
		    if(duplicate){
			validateMessageBox(form.title.message, form.msg.form.msg.someUsersAlreadyAdded, false);
...
function checkUserInTree(isOrganizers,appendTo){
	...
	if (userName != "") {
		...
		if (nodeId != '' && nodeId != undefined) {
			...
			/*$.msgBox({
				title : "Message",
				content : "No User is available in this name",
				buttons : [ {
					value : "Ok"
				} ],
				success : function(result) {
	...
	$.ajax({
		...
		success : function(response) {
			if(response.length > 0){
				...
				if(isAdmin){
					$("#divsample").html("<center><h5>Widgets Not Selected</h5></center>");
				}
