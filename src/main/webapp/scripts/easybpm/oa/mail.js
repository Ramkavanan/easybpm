...
function deleteSelectedMails(from){
	...
	if(rowid.length !=0 ){
		...
		$.msgBox({
		    title: "Confirm",
		    content: "Are you sure want to delete the Mail(s)/InternalMessage(s) ?",
		    type: "confirm",
	...
	if(rowid.length ==0){
		$.msgBox({
   		    title:"Message",
   		    content:"Please select atleast single Mail/InternalMessage to Delete",
   		});
	}
}
...
function deleteAllContactsConfirm(contactIds){
	...
	$.msgBox({
	    title: "Confirm",
	    content: "Are you sure want to delete these contacts?",
	    type: "confirm",
	    buttons: [{ value: "Yes" }, { value: "No" }],
	    success: function (result) {
	        if (result == "Yes") {
