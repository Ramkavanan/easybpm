...
function operateBulkTasks(operation){
	...
	if(rowid.length == 0){ // here to terminate instance list in null or not
		validateMessageBox(form.title.msgBoxTitleMessage,form.msg.select+" "+operation,false);
	}
	if(rowid.length != 0 ){
		if(operation == 'replace'){
			...
			if(taskJsonObj.length > 0){
				...
				if(!canReplace) {
					validateMessageBox(form.title.msgBoxTitleMessage,form.msg.replaceError);
				} else {
					validateMessageBox(form.title.msgBoxTitleMessage,form.msg.operationError);
				}
			...
			if(operation == 'delete' || operation == 'terminate'){
				if ( !Array.prototype.forEach ) {
					for(var index=0;index<rowid.length;index++){
						...
						taskJsonObj.push({taskId: gridObj.jqGrid('getCell', item, 'id'),executionId: gridObj.jqGrid('getCell', item, 'executionId'),
			...
			if(taskJsonObj.length == 0){
				...
				validateMessageBox('Message',message);
			}
			if(isValid){
				$.msgBox({
				    title: form.title.msgBoxTitleConfirm,
				    content: form.msg.confirm+""+operation,
				    type: "confirm",
...
/*if ((hasStartFormKey == 'true')	|| (isSystemDefined == 'true')) {
		var params = "nextTaskOrganizers=&nextTaskCoordinators=&nextTaskOrganizerOrders=&processKey="
			+ processKey + "&isSystemDefined=" + isSystemDefined +"&processInstanceId="+processInstanceId+"&isFromDoneList="+isFromDoneList+"&creator=&isFormReadOnly="+isFormReadOnly;
		_execute("target", params);
		document.location.href = "#bpm/manageProcess/startProcessInstance";	
	} else {
		ofJointConductStartProcess(processKey,"");
	}*/
...
function showTaskFormDetailForDoneList(taskId,suspendState,lastOpPerformed,processInstanceId,resourceName,processDefinitionId,canActivate,isFormReadOnly) {
	...
	/*if(lastOpPerformed && lastOpPerformed =='terminate'){
		validateMessageBox("Message", "Already this instance has been terminated", false);
	}else if(lastOpPerformed && (lastOpPerformed =='back' || lastOpPerformed =='jump')){
		validateMessageBox("Message", "Already "+lastOpPerformed+" operation  done on this instance", false);
	}else if(lastOpPerformed && lastOpPerformed =='returnoperation'){
		validateMessageBox("Message", "Already return operation  done on this instance", false);
	}else if(lastOpPerformed && (lastOpPerformed =='suspend' || lastOpPerformed =='withdraw')){
		var operationMesage = "suspended";
		if(lastOpPerformed =='withdraw'){
			operationMesage ="withdrawn";
		}
		validateMessageBox("Message", "Already this instance has been "+operationMesage, false);
		if(canActivate == "true") {
			var confirmMessage = "This task already "+operationMesage+". Do you want to activate?";
			activateTaskByProcessInstanceId(taskId,processInstanceId, "/bpm/manageTasks/doneList",confirmMessage,resourceName);
		} else {
			validateMessageBox("Message", "Already this instance has been suspended", false);
		}
	}else {*/
		...
		if(taskId == processInstanceId){
			...
			//var url = "showTaskFormDetail?taskId="+taskId+"&suspendState="+suspendState+formReadOnly ;
	//remove start node form from process
...
function showTaskFormDetail(taskId,suspendState,lastOpPerformed,processInstanceId,resourceName,processDefinitionId,creator,gridName) {
	 ...
	 if (lastOpPerformed
			&& (lastOpPerformed == 'suspend' || lastOpPerformed == 'withdraw' || suspendState == "3" || suspendState == "2" )) {
		var operationMesage = "suspended";
		if (lastOpPerformed == 'withdraw' || suspendState == "3") {
			operationMesage = "withdrawn";
		}
		var confirmMessage = "This task already " + operationMesage
...
function _restoreProcess(cellValue, options, rawObject) {
	if (rawObject.activeVersion == "false")
		return '<a href="javascript:void(0);" id="restoreProcessVersions" data-role="button" data-theme="b" onclick="restoreProcessDialog(\'Restore Process Version\',\''
				+ rawObject.id
				+ '\',\''
				+ rawObject.key
				+ '\',\''
				+ rawObject.activeProcessId + '\');"><b>Restore</b></a>';
	else
		return 'Active';
}
	...
	$.msgBox({
				...
				success : function(result) {
					if (result == "Yes") {
						...
						//document.location.href = "#bpm/manageProcess/restoreProcessVersion";
						//_execute('target', params);
						$.ajax({
			    			type : 'GET',
			    			async : false,
			    ...
			    afterClose : function(){
			    	if(isNeeded){
				    	if(isSuccess){
				    		validateMessageBox("Success", message , "success");
						}else{
							validateMessageBox("Error", message , "error");
						}
			    	}
			    }
			});
...
function _deleteProcessVersion(cellValue, options, rawObject) {
	if (rawObject.activeVersion == "false") {
		var params = 'processId=' + rawObject.id + '&processKey='
				+ rawObject.key;
		return '<a id="deleteProcessVersion" href="javascript:void(0);" data-role="button" data-theme="b" onclick="javascript:deleteVersionConfirm(\''
				+ params + '\');"><b>Delete</b></a>';
	} else
		return 'Delete';
}
...
function deleteVersionConfirm(params) {
	...
	$.msgBox({
				...
				success : function(result) {
					if (result == "Yes") {
						//document.location.href = "#bpm/manageProcess/deleteProcessVersion";
						//_execute('target', params);
						$.ajaxSetup({ cache:false});
						$.ajax({
			    			type : 'GET',
...
function _showProcessVersions(cellValue, options, rawObject) {
	return '<a id="showProcessVersion" href="#bpm/manageProcess/showAllVersions" data-role="button" data-theme="b" onclick="_execute(\'target\',\'processKey='
			+ rawObject.key + '\');"><b>Show Versions</b></a>';
...
function assignSubstitute(substitute) {
	...
	var content;
	// show the content in dialogue according to screen
	content = "Please select atleast single Task to Substitute Conduction!";
	var rowid = gridObj.getGridParam('selarrrow');
	var taskIds = "";
	var i = 1;
	rowid.forEach(function(item) {
	...
	if (rowid.length != 0) {
		$.msgBox({
					title : "Confirm",
					content : form.msg.wantToSubtitute + substitute	+ form.msg.selectedTask,
					type : "confirm",
					buttons : [ {
						value : "Yes"
					}, {
...
function _deleteProcessInstance(cellValue, options, rawObject) {
	return '<a id="deleteProcessInstance" href="javascript:void(0);" data-role="button" data-theme="b" onclick="deleteProcessInsDialog(\''
			+ rawObject.id + '\' ,\''+ rawObject.processDefinitionId + '\',\''+ rawObject.operationPerformed + '\');"><b>Delete</b></a>';
}
...
/*function deleteProcessInsDialog(id, processDefId, operationPerformed) {
	...
	if (operationPerformed == "false") {
		$.msgBox({
			title: "Confirm",
			content : 'Are you sure you want to delete this process instance?',
			type: "confirm",
		    buttons: [{ value: "Yes" }, { value: "No" }],
			inputs : [ {
				header : "Reason",
			...
			success : function(result, values) {
				if (result == "Yes") {
					  ...
					  "#bpm/process/deleteProcessInstance"; params =
					  'processInsId=' + id + '&reason=' +
					  values[0].value; _execute('target', params);
					 
					params = 'processInsId=' + id ; 
					//'&reason=' + values[0].value;
					$.ajax({
						type : 'GET',
						async : false,
						url : 'bpm/process/deleteProcessInstanceAjax?'+ params,
							success : function(response) {
								if (response.successMsg) {
									...
									document.location.href = "#bpm/process/myInstances";
									_execute('target', '');
									// showListViewsWithContainerAndParam("PROCESS_LIST_MY_INSTANCES",
									// "My Instances", 'target',
									// params);
								} else {
									isFailure = true;
		...
		var message = "Cannot delete Process Instance after operating"
			validateMessageBox("Info", message, "alert");
	}
}*/
...
function _previewProcessPopup(container, processName, requestedparams) {
	...
	setTimeout(function() {
		...
		$.myDialog.dialog({
			...
			height : 'auto',
			modal : true,
			title : "Process Preview - " + decodeURI(processName)
		});
		$.myDialog.dialog("open");
	}, 500);
}
...
function cancelTaskToReadView(taskId) {
	//params = "";
	//document.location.href = "##bpm/manageTasks/toRead";
	//_execute('target', params);
	close();
}

function _operateOnTask(cellValue, options, rawObject) {
	// if(rawObject.taskClaimable == false && rawObject.taskFormApplicable ==
	// true)
		var save = "";
		var operate = "";
		
		if(cellValue == "save") {
					save = '<a  data-role="button" data-theme="b"><b><img src="/images/easybpm/form/save_btn.jpg" /></b></a>';
					operate = "     Operate  "+save; 
		} else if(rawObject.lastOperationPerformed == "return" || rawObject.lastOperationPerformed == "return submit") {  
			operate = "              Operate (Return)";
		} else{
			operate = "Operate";
		}
...
function _operateOnTaskForDoneList(cellValue, options, rawObject) {
	...
	if(cellValue == "complete") {
		...
		+ rawObject.id + '\',\''+ rawObject.suspensionState + '\',\''+ rawObject.lastOperationPerformed + '\',\''+ rawObject.executionId + '\',\''
		+ rawObject.resourceName + '\',\''+ rawObject.processDefinitionId + '\',\''+ rawObject.canActivate + '\',\''+ rawObject.isFormReadOnly + '\');">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Operate&nbsp;<img src="/images/easybpm/form/complete.gif"/></b></a>';
	} else {
		return '<a id="taskInfo" href="javascript:void(0);" data-role="button" data-theme="b" onclick="showTaskFormDetailForDoneList(\''
		+ rawObject.id + '\',\''+ rawObject.suspensionState + '\',\''+ rawObject.lastOperationPerformed + '\',\''+ rawObject.executionId + '\',\''
		+ rawObject.resourceName + '\',\''+ rawObject.processDefinitionId + '\',\''+ rawObject.canActivate + '\',\''+ rawObject.isFormReadOnly + '\');"><b>Operate</b></a>';
	}
...
function _operateOnTaskForReader(cellValue, options, rawObject) {
	...
	/*return '<a id="taskInfo" href="javascript:void(0);" data-role="button" data-theme="b" onclick="showTaskFormDetailForToRead(\''
			+ rawObject.id + '\',\''+ false + '\');"><b>Operate</b></a>';*/

	return '<a id="taskInfo" href="javascript:void(0);" data-role="button"  data-theme="b" onclick="showTaskFormDetailForToRead(\''
			+ rawObject.id + '\',\''+ false + '\',\''+ rawObject.suspensionState + '\');"><b>Operate</b></a>';
	// else
	// return '';
}
...
function _operateOnTaskForReadedUser(cellValue, options, rawObject) {
	...
	// true)
/*	return '<a id="taskInfo" href="javascript:void(0);" data-role="button" data-theme="b" onclick="showTaskFormDetailForToRead(\''
			+ rawObject.id + '\',\''+ true + '\');"><b>Operate</b></a>';*/
	return '<a id="taskInfo" href="javascript:void(0);" data-role="button" data-theme="b" onclick="showTaskFormDetailForToRead(\''
			+ rawObject.id + '\',\''+ true + '\',\''+ rawObject.suspensionState + '\');"><b>Operate</b></a>';
	// else
	// return '';
}
...
function importXml() {
	openSelectDialogForFixedPosition("importPopupDialog", "", "750", "260","Import Process");
...
function showPrintTemplate(taskId, formId, formName){
	...
	$.msgBox({
		...
		title : "Print Options",
		inputs: [
		{ header: "Print Form&nbsp;&nbsp;&nbsp;&nbsp;", type: "checkbox", name: "isPrintForm", value:'isPrintForm', checked:true, onclick:'clickPrintForm()'},  
		{ header: "Print Opinion&nbsp;&nbsp;&nbsp;&nbsp;", type: "checkbox", name: "isPrintOpinion", value:'isPrintOpinion',onclick:'clickPrintOpinion()'}, 
		{ header: "Print Trace&nbsp;&nbsp;&nbsp;&nbsp;", type: "checkbox", name: "isPrintTrace", id: "isPrintTrace", value:'isPrintTrace', onclick:'clickPrintTrace()'},   
		{ header: "Print All&nbsp;&nbsp;&nbsp;&nbsp;", type: "checkbox", name: "isPrintAll", id: "isPrintAll", value:'isPrintAll', onclick:'clickPrintAll()'}   
		],
		buttons : [{
			value : "Ok"			
		}, {
...
function showAddClassification() {
	openSelectDialogForFixedPosition("popupDialog", "", "520", "250","Add Classification");
...
function createBreadCrumb(){
	var breadCrumb = '<div id="breadCrumb" class="row-fluid"><div style="min-height: 10px;" class="span12"><ul class="breadcrumb">';
		 ...
		 if( $.cookie(menus[i]) != undefined && $.cookie(menus[i]) != null && $.cookie(menus[i]) != ''){
			...
			if((($.cookie('rootMenuUrl') != null && $.cookie('rootMenuUrl') != '' && $.cookie('rootMenuUrl') != '#') || $.cookie('rootMenuType') == 'listview') && i == 0){
				if($.cookie('rootMenuType') == 'listview'){
					...
					breadCrumb = breadCrumb + '<a href="javascript:void(0);" onClick="'+onClickMethod+'">'+ $.cookie(menus[i])+'</a>';
				} else {
					...
					breadCrumb = breadCrumb + '<a href="'+backUrl+'" onClick="'+onClickMethod+'">'+ $.cookie(menus[i])+'</a>';
				}
			} else {
...
function systemDefinedFormStart(formId,formAction){
	...
	if ( !isFormValuesValid()) {
		validateMessageBox(form.title.msgBoxTitleMessage, "Fill mandatory fields" , false);
	} else {
		for(var instanceName in CKEDITOR.instances) {
		...
		if(form == "News" || form == "Notice") {
			...
			if(classification == "" || title == "") {
				isSuccess = false;
				validateMessageBox("Message", "Title and classification should not be empty", false);
			}
				...
				if(form['isNotice'] ){
					if($('input[name="status"]:radio:checked').val() == 'formal'){
						if(!$('[name="user"]').val() && !$('[name="role"]').val() && !$('[name="department"]').val() && !$('[name="group"]').val()){
								messageBoxShow("Message","Any one of the field (User,Role,Department,Group) should be fill");
								return false;
							}
					}
...
function validateStartAndEndDate(formId){
	...
	if( startDate > endDate ||  startDate == "" || endDate == "") {
		validateMessageBox("Message", "End Date should be greater than start Date", false);
		return false;
	}
	if(classification == "" || title == "") {
		validateMessageBox("Message", "Title and classification should not be empty", false);
...
function replaceAllValueAsReadOnly(formId){
		$.each($('#'+formId).serializeArray(), function(i, field) {
			...
			var elemStyleTop=element.style.top+";";
			var elemStyleLeft=element.style.left+";";
			...
			if(field.value!="" && element.nodeName.toLowerCase() != "select"){
				if(element.type == "checkbox") {
					...
					$( element ).replaceWith("<span style='left:"+elemStyleLeft+" top:"+elemStyleTop+" position: absolute;font-size: 14px;padding: 4px 6px;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;height: 30px;align:left;'><b>&nbsp;&nbsp;&nbsp;"+field.value+"</b></span>" );
				}
			}else if(element.nodeName.toLowerCase()== "select"){
				if(element.text != undefined){
					$( element ).replaceWith("<span style='left:"+elemStyleLeft+" top:"+elemStyleTop+" position: absolute;font-size: 14px;padding: 4px 6px;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;height: 30px;align:left;'><b>&nbsp;&nbsp;&nbsp;"+element.text+"</b></span>" );
				}else{
					$( element ).replaceWith("<span style='left:"+elemStyleLeft+" top:"+elemStyleTop+" position: absolute;font-size: 14px;padding: 4px 6px;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;height: 30px;align:left;'><b>"+element.options[element.selectedIndex].text+"</b></span>" );
				}
			}else{
...
function loadPermissionsOnCreate(formId, name, isEditable, isRequired,
	...
	if (element) {
		if (element.length && element.length > 1) {
			for ( var i = 0; i < element.length; i++) {
				var el = element[i];
				setElementPermissionForCreate(el, isRequired, isReadOnly,
						isHidden, isNoOutput);
			}
		} else {
			setElementPermissionForCreate(element, isRequired, isReadOnly,
					isHidden, isNoOutput);
		}
...
function setElementPermissionForCreate(el, isRequired, isReadOnly, isHidden,
		isNoOutput) {
	...
	if((el.className == "datepicker" || el.className == "datetimepicker") &&  isReadOnly == "true") {
...
function loadPermissionsOnEdit(formId, name, isEditable, isRequired,
		isReadOnly, isHidden, isNoOutput) {
	var form = document.getElementById(formId);
	var element = form[name];
	if (element) {
		if (element.length && element.length > 1) {
			for (i = 0; i < element.length; i++) {
				...
				setElementPermissionForCreate(element[i], isRequired,
						isReadOnly, isHidden, isNoOutput);
			}
		} else {
...
function showIcons(iconPath){
	$.ajax({
		...
		success : function(response) {
			...
			openSelectDialogForFixedPosition("icon_dialog","",300,300,"Select icon");
		} 
	});
}
...
function _showInteriorDictionaries(cellValue, options, rawObject) {
	return '<span id="list_view_'
			...
			+ rawObject.dictionaryId
			+ '\')"><b>Interior Dictionary</b></a></span>';
}
...
function prepareSubmitTaskFormForSubmit(formId, formAction,
		isOperationfnSubmit, isFinalTransactor,showReadButton) {
	var form = document.getElementById(formId);
	 $("#form_div").css("height",getFormHeight(form));
	if (form) {
		var attachmentDetail = document.getElementById("attachmentDetail");
...
function deleteNewsData(){
	...
	rowid.forEach(function(item) {
		var newsId = gridObj.jqGrid('getCell', item, 'id');
		newsIds[newsIds.length] = newsId;
	});
	
	if(rowid.length == 0){
		validateMessageBox(form.title.message, form.msg.selectAtleastSingleNewsToDelete, "info");
		
	/*	$.msgBox({ 
   		    title:"Delete News",
   		    content:"Please select atleast single news to Delete"
   		});*/
	}
	if(rowid.length !=0 ){
		deleteAllNews(newsIds);
	}
}

function deleteAllNews(newsIds){
	var status=false;
	var message="";
	var deleteResult="";
	var deleteType="";
	...
	$.msgBox({
	    title: form.title.confirm,
	    content: form.msg.confirmToDeleteNews,
	    type: "confirm",
	    buttons: [{ value: "Yes" }, { value: "No" }],
	        	...
	        	success: function (result) {
	    	        if (result == "Yes") {
	    	        	$.ajax({
	    	        		url: 'bpm/table/deleteAllNewsByIds',
	    	                type: 'GET',
	    	                dataType: 'json',
	    	                async: false,
	    	                data: 'newsIds='+JSON.stringify(newsIds),
	    	        		success : function(response) {
	    	        			deleteNewsCB(response,function (res){
	    	        				status=true;
	    	        				if(res){
...
function deleteNewsCB(response,cb) {
	var status=true;
	...
	document.location.href = "#bpm/news/newsList";
	_execute('target','');
...
function deleteNoticeData(){
	...
	rowid.forEach(function(item) {
		var noticeId = gridObj.jqGrid('getCell', item, 'Id');
		noticeIds[noticeIds.length] = noticeId;
	});
	
	if(rowid.length == 0){
		validateMessageBox(form.title.deleteNotice, form.msg.selectAtleastSingleNoticeToDelete, "info");
		
		/*$.msgBox({ 
   		    title:"Delete Notice",
   		    content:"Please select atleast single notice to Delete"
   		});*/
	}
	if(rowid.length !=0 ){
		deleteAllNoticeByIds(noticeIds);
	}
}

function deleteAllNoticeByIds(noticeIds){
	var status=false;
	var message="";
	var deleteResult="";
	var deleteType="";
	...
	$.msgBox({
	    title: "Confirm",
	    content: "Are you sure want to delete these Notice(s)?",
	    type: "confirm",
	    buttons: [{ value: "Yes" }, { value: "No" }],
	        	...
	        	success: function (result) {
	    	        if (result == "Yes") {
	    	        	$.ajax({
	    	        		url: 'bpm/table/deleteAllNoticeByIds',
	    	                type: 'GET',
	    	                dataType: 'json',
	    	                async: false,
	    	                data: 'noticeIds='+JSON.stringify(noticeIds),
	    	        		success : function(response) {
...
function deleteNoticeCB(response,cb) {
	...
	document.location.href = "#bpm/notice/noticeList";
	_execute('target','');
