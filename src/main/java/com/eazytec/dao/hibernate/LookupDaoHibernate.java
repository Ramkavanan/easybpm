...
public class LookupDaoHibernate implements LookupDao {
   ...
   public List<SysConfig> getSysConfigByKey(String key)throws EazyBpmException{
	 ...
	 * {@inheritDoc}
	 */
	public List<LabelValue> getAllUsersAsMap(){
		/*Session session = sessionFactory.getCurrentSession();
		String HQL_QUERY = 
		      "select new map(user.id, user.firstName) from User user";
		    
		    List<Map<String,String>> usersList = session.createQuery(HQL_QUERY).list(); */
		Session session = sessionFactory.getCurrentSession();
		String HQL_QUERY = "select new com.eazytec.model.LabelValue(user.id as id, concat(user.firstName, ' ', user.lastName)) from User as user";
		List<LabelValue> usersList = (List<LabelValue>) session.createQuery(HQL_QUERY).list();
