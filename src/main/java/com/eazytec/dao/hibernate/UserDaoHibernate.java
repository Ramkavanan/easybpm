...
public class UserDaoHibernate extends GenericDaoHibernate<User, String> implements UserDao, UserDetailsService {
    ...
    */
    ...
    public String getInternalMessageId(String username){
    	List<String> users = getSession().createSQLQuery("select USER.USER_NAME FROM JAMES_USER as USER WHERE USER.USER_NAME = '"+username+"'").list();
    	if(users.isEmpty()){
    		return null;
    	} else {
    		return users.get(0);
   ...
   /**
	...
	public List<UserDTO> getAllUsers() {
		List<UserDTO> user = (List<UserDTO>) getSession()
				.createQuery(
						"select new com.eazytec.dto.UserDTO(user.id as id,user.username as username,user.firstName as firstName,user.lastName as lastName,department.name as department,user.email as email,user.employeeNumber as employeeNumber,user.enabled as enabled) from User as user left join user.department as department")
				.list();
		return user;
	}
...
//		return userList;
    ...
    public void deleteInternalUserId(String username){
    	Query query = getSession().createSQLQuery("DELETE FROM JAMES_USER AS USER WHERE USER.USER_NAME = '"+username+"')");
       	query.executeUpdate();
    }
