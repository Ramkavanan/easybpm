...
@WebService(serviceName = "ProcessService", endpointInterface = "com.eazytec.bpm.metadata.process.service")
public class ProcessDefinitionServiceImpl implements ProcessDefinitionService{
	...
	*/	
	 ...
	 * @throws FileNotFoundException 
	 */
	public boolean deployBPMNSourceFileWithJson(String fileName,String filePath,String description,String classification, String jsonString,String svgString) throws	BpmException, FileNotFoundException {
		DeploymentBuilder builder = repositoryService.createDeployment();
		builder.setDescription(description);
		builder.setClassificationId(classification);
	...
	public boolean checkIsNextTaskCompleted(HistoricTaskInstance task){	
		...
		System.out.println("================="+checkIsNextTaskCompleted.size());
...
//																	      .orderByHistoricActivityInstanceStartTime().desc().list();
	...
	public List<Map<String, Object>> resolveProcessDefinitions(List<ProcessDefinition>  processDefinitions) throws BpmException{
		...
		for(Map<String, Object> processDefinitionsMap : processDefinitionsMapAsList){
			...
			instanceCount = (int)historyService.createHistoricProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).count();
			activeCount = (int) runtimeService.createProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).active().count();
			suspendCount = (int) runtimeService.createProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).suspended().count();
			...
			if(instanceCount > 0){
				...
				processDefinitionsMap.put("noOfInstacnes", 0);
			}
			...
			if(processDefinitionsMap.get("activeVersion").toString().equalsIgnoreCase("true")){
				processDefinitionsMap.put("versionStatus", "Active");
				processDefinitionsMap.put("restore", "Active");
				
			}else{
				processDefinitionsMap.put("versionStatus", "InActive");
...
List<Map<String, Object>> resolvedProcessDefinitionsListAsMap = new ArrayList<Map<String,Object>>();
		...
		for(Map<String, Object> processDefinitionsMap : processDefinitionsMapAsList){
			...
			instanceCount = (int)historyService.createHistoricProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).startedBy(loggedInUser).count();
			suspendCount = (int) historyService.createHistoricProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).startedBy(loggedInUser).suspended().count();
			activeCount = (int) historyService.createHistoricProcessInstanceQuery().processDefinitionId(processDefinitionsMap.get("id").toString()).startedBy(loggedInUser).activated().count();
	...
	public String getProcessCreatedDateForDeplymentId(String deploymentId)throws BpmException{
		...
		dateStr = repositoryService.createDeploymentQuery().deploymentId(deploymentId).list().get(0).getDeploymentTime().toString();
		try {
