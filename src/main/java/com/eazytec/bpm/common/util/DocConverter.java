...
import com.eazytec.exceptions.ConversionException;
import com.eazytec.util.PropertyReader;
//import com.openkm.core.DatabaseException;
import com.eazytec.Constants;
import com.eazytec.bpm.common.util.MimeTypeConfig;
...
public class DocConverter {
	...
	private DocConverter() {
		...
		validOpenOffice.add("application/octet-stream");
	...
	public static synchronized DocConverter getInstance() {
		if (instance == null) {
			...
			String openOfficePath = (String) PropertyReader.getInstance()
			.getPropertyFromFile("String", "system.openoffice.path");
			
			int openOfficeTasks = Integer.parseInt(PropertyReader.getInstance()
			.getPropertyFromFile("Integer", "system.openoffice.tasks")); 
			
			int openOfficePort = Integer.parseInt(PropertyReader.getInstance()
			.getPropertyFromFile("Integer", "system.openoffice.port"));
			
			
			if (!StringUtil.isEmptyString(openOfficePath)) {
				log.info("*** Build Office Manager ***");
				log.info("{}={}", Constants.PROPERTY_SYSTEM_OPENOFFICE_PATH, openOfficePath);
				log.info("{}={}", Constants.PROPERTY_SYSTEM_OPENOFFICE_TASKS, openOfficeTasks);
				log.info("{}={}", Constants.PROPERTY_SYSTEM_OPENOFFICE_PORT, openOfficePort);
				
				officeManager = new DefaultOfficeManagerConfiguration()
					.setOfficeHome(openOfficePath)
					.setMaxTasksPerProcess(openOfficeTasks)
					.setPortNumber(openOfficePort)
					.buildOfficeManager();
			} else {
				log.warn("{} not configured", Constants.PROPERTY_SYSTEM_OPENOFFICE_PATH);
			}
		}
	...
	public boolean convertibleToPdf(String from) {
		...
		if (validOpenOffice.contains(from) /*|| validImageMagick.contains(from) || validGhoscript.contains(from)*/) {
			ret = true;
		} else if (!StringUtil.isEmptyString(openOfficePath) && validOpenOffice.contains(from)) {
			ret = true;
		}
		/*} else if (!Constants.SYSTEM_IMAGEMAGICK_CONVERT.equals("") && validImageMagick.contains(from)) {
			ret = true;
		} else if (!Constants.SYSTEM_GHOSTSCRIPT_PS2PDF.equals("") && validGhoscript.contains(from)) {
			ret = true;
	...
	public boolean convertibleToSwf(String from) {
		...
		if (MimeTypeConfig.MIME_PDF.equals(from) || validOpenOffice.contains(from) || 
			...
			ret = true;
		} else if (!Constants.SYSTEM_SWFTOOLS_PDF2SWF.equals("") && (MimeTypeConfig.MIME_PDF.equals(from) || convertibleToPdf(from))) {
			ret = true;
		}
	...
	public void convert(File inputFile, String mimeType, File outputFile) throws ConversionException {
		...
		.getPropertyFromFile("String", "system.openoffice.path");
		if (StringUtil.isEmptyString(openOfficePath)) {
			throw new ConversionException(Constants.PROPERTY_SYSTEM_OPENOFFICE_PATH + " not configured");
		}
	 ...
	 */
	/*public void pdf2img(File input, File output) throws ConversionException, 
			IOException {
