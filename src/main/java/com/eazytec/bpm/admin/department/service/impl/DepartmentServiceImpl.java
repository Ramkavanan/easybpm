...
@WebService(serviceName = "DepartmentService", endpointInterface = "com.eazytec.bpm.admin.department.service")
public class DepartmentServiceImpl extends GenericManagerImpl<Department, String> implements
	...
	public List<Department> search(String searchTerm) {
		return super.search(searchTerm, Department.class);
	}
	...
	public Department saveDepartment(Department department) throws DepartmentExistsException {
		...
		try {
			...
			if(department.getVersion() == 0 || department.getId() == null){
	 ...
	 * {@inheritDoc}
	 */
	public Response removeDepartmentByAPI(String departmentName,String userName){
		//check whether the department exist or not
		Department department = getDepartmentByNameForAPI(departmentName);
		if(department != null){
			try{
				List<String> departmentIds = new ArrayList<String>();
				User user = userManager.getUser(userName);
				boolean isPermitToDelete = false;
				departmentIds.add(departmentName);
				// check whether user have permission to delete the department,if he have delete will be perform 
				isPermitToDelete = removeDepartments(departmentIds,user,isPermitToDelete);
				if(!isPermitToDelete){
					return Response.status(401).entity(null).build();
					//throw new ApiException(401,"User does not have permission to delete");
				}else{
					return Response.status(200).entity(null).build();
				}
			}catch(Exception e){
				e.printStackTrace();
					//throw new ApiException(500,e.getLocalizedMessage());
					return Response.status(401).entity(null).build();
			}
		}else{
	 ...
	 * {@inheritDoc}
	 */
	public Department getDepartmentByNameForAPI(String departmentName){
		Department department =  departmentDao.getDepartmentByName(departmentName);
		if(department == null){
			throw new ApiException("Department "+departmentName+" does not exist");
		}
		return department;	
	}
    /**
