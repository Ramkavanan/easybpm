...
@WebService(serviceName = "LayoutService", endpointInterface = "com.eazytec.bpm.admin.layout.service")
	...
	public String getRolesChildNode(String currentNode, int nodeLevel)
		...
		if (nodeLevel == 1) {
			...
			for(Role role : roleList){
				...
				labelValue.setLabel(role.getId());
				labelValue.setValue(role.getName());
		...
		if (layouts != null) {
			for (Layout newLayout : layouts) {
				List<String> widgets = new ArrayList<String>(
						Arrays.asList(newLayout.getColumnWidget().split(",")));
