...
public class RoleDaoImpl extends GenericDaoHibernate<Role, String> implements RoleDao {
	...
	public List<RoleDTO> getRolesDTO() throws BpmException{
    	List<RoleDTO> roles = (List<RoleDTO>) getSession().createQuery("select new com.eazytec.dto.RoleDTO(role.id as id,role.name as name) from Role as role where role.roleType !='Internal'").list();
    	return roles;
    }
    ...
    public List<LabelValue> getAllRolesAsLabelValue() throws BpmException {
    	List<LabelValue> roleList = (List<LabelValue>) getSession()
				.createQuery(
						"select new com.eazytec.model.LabelValue(role.name as name,role.id as id) from Role as role where role.id != 'ROLE_DEFAULT' and role.roleType !='Internal'")
				.list();
