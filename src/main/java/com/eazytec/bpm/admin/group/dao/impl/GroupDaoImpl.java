...
public class GroupDaoImpl extends GenericDaoHibernate<Group, String> implements
   ...
   public void updateGroupLeader(String userId,String leader){
    ...
    public void updateSuperGroup(String oldSuperGroup,String newSuperGroup){
    	Query query = getSession().createQuery("UPDATE Group set superGroup = '"+newSuperGroup+"', isParent = true WHERE superGroup='"+oldSuperGroup+"'");
    	query.executeUpdate();
    }
	...
	public List<GroupDTO> getGroupsDTO() throws BpmException{
    	List<GroupDTO> groupList = (List<GroupDTO>) getSession().createQuery("select new com.eazytec.dto.GroupDTO(group.id as id,group.name as name,group.superGroup as superGroup,group.isParent as isParent) from Group as group").list();
    	return groupList;
    }
    ...
    public List<LabelValue> getAllParentGroupsAsLabelValue() throws BpmException {
    	List<LabelValue> groupList = (List<LabelValue>) getSession()
				.createQuery(
						"select new com.eazytec.model.LabelValue(group.name as name,group.id as id) from Group as group where group.isParent != '"+true+"'")
				.list();
    ...
    public List<LabelValue> getGroupsAsLabelValueByParent(String parentGroup) throws BpmException {
    	List<LabelValue> groupList = (List<LabelValue>) getSession()
				.createQuery(
						"select new com.eazytec.model.LabelValue(group.viewName as name,group.id as id) from Group as group where group.superGroup = '"+parentGroup+"'")
				.list();
