...
@WebService(serviceName = "ListViewService", endpointInterface = "com.eazytec.bpm.runtime.listView.service")
public class ListViewServiceImpl implements  ListViewService{
				...
				if(isGrouping){
					...
					if(!listView.getListViewButtons().isEmpty()){
						...
						for(ListViewButtons listViewButton:listViewButtonSet){
							if(listViewButton.getDisplayName().equals("Delete") && listViewButton.isActive() && listViewButton.getTableName()!=null && !listViewButton.getTableName().isEmpty() && listViewButton.getColumnName()!=null && !listViewButton.getColumnName().isEmpty() ){
								whereParams=listViewButton.getTableName()+".is_delete=false";
							}
						}
					}
					if(listViewParamsList!=null && !listViewParamsList.isEmpty()){
		...
		if(listViewGroupSettingList != null && !(listViewGroupSettingList.isEmpty())){
			for(Map<String, Object> listViewGroupSetting : listViewGroupSettingList){
				if(String.valueOf(listViewGroupSetting.get("groupType")).equals("Single")  || 
					...
					List<String> listViewObjArray=listViewDao.getListViewDataMap(queryString);
	...
	private String constructQueryGroupSetting(ListView listView,Map<String, Object> viewColumnsList, List<String> groupByFields, List<Map<String,Object>> whereParamsListOfMap,Map<String,Object> listViewParams)throws EazyBpmException{
		...
		if(listView.getWhereQuery()!=null && !listView.getWhereQuery().isEmpty()){
			...
			if(rolesList.contains("ROLE_ADMIN") && whereString.indexOf(":loggedInUserReport") > -1) {
				...
				whereString = replaceUserDetails(fromQueryString);
				...
				strbuf.append(" WHERE "+whereString+" and "+viewColumnsList.get("groupFieldsName")+" != '' ");	
				if(whereParams != null && !(whereParams.isEmpty())){
					strbuf.append(" and "+whereParams);	
				}
				if(listView.getViewName().equalsIgnoreCase("NOTICE")) {
					strbuf.append("AND NOTICE.IS_DELETE =FALSE");	
				}
			}
		}else if(whereParams != null && !(whereParams.isEmpty())){
			strbuf.append(" WHERE "+whereParams+" and "+viewColumnsList.get("groupFieldsName")+" != '' ");	
		}else{
			strbuf.append(" WHERE "+viewColumnsList.get("groupFieldsName")+" != '' ");	
		}
	...
	private List<Map<String,Object>> getAllListViewMapData(ListView listView,List<String> colunmnList,List<Map<String, Object>> viewColumnsList, String whereParams, String sortType, String sortTypeColumn,Map<String, Object> listViewParamsList)throws EazyBpmException{
		...
		//System.out.println(listViewQuery);
		//log.info("Constructed Query :: "+listViewQuery);
		List listViewObjArray = listViewDao.getListViewDataMap(listViewQuery);
	...
	private String constructQueryToSelectListData(ListView listView,List<Map<String, Object>> viewColumnsList, String whereParams, String sortType, String sortTypeColumn,Map<String, Object> listViewParams)throws EazyBpmException{
		...
		for(Map<String, Object> viewColumn:viewColumnsList){
			if(viewColumn.get("dataFields")!="" && viewColumn.get("dataFields")!=null){
	...
	public List<Map<String, Object>> getAllListViewMapDataWithSearchValue(ListView listView, String searchValue,Map<String,Object> namedParameters)throws EazyBpmException{
        ...
        for(Map<String, Object> viewColumns : viewColumnsList){
            ...
            if(String.valueOf(viewColumns.get("isSimpleSearch")).equals("true")){
                ...
                if(viewColumns.get("replaceWords") != "" && (viewColumns.get("replaceWords") != null)) {
					...
					for(int index1 = 0; index1 < replaceWordLength; index1++){
	                    ...
	                    if(rplWord[1].startsWith(searchValue)) {
        ...
        //System.out.println("======="+whereParams);
	...
	public boolean checkListViewQuery(ListView listView)throws EazyBpmException{
		...
		listViewDao.getListViewDataMap(listViewQuery);
		return true;
	}
	 ...
	 * 
	 */
	public String getListViewForInboxMail(Message[] msgs, String sessionType)throws EazyBpmException, MessagingException, UnsupportedEncodingException{
		...
		for(int i = msgs.length-1; i >= 0; i--){
			...
			if(from.equalsIgnoreCase(CommonUtil.getEmailNotificationId()) || from.equalsIgnoreCase(CommonUtil.getInternalMessageNotificationAdminId())){
				mailDetails.put("type", "<img class='header-menu-name' width='14px' height='14px' alt='notification' src='/images/iconInformation.gif' />");
			} else {
		...
		return generateScriptForMail("inbox", mailList, sessionType);
	}
	 ...
	 * 
	 */
	public String getListViewForDraftMail(Message[] msgs, String sessionType)throws EazyBpmException, MessagingException, UnsupportedEncodingException{
		List<Map<String, Object>> mailList = new ArrayList<Map<String, Object>>();
		for(int i = msgs.length-1; i >= 0; i--){
			Map<String, Object> mailDetails=new HashMap<String, Object>();
			if(!msgs[i].isSet(Flags.Flag.SEEN)){
		...
		return generateScriptForMail("draft" ,mailList, sessionType);
	}
	 ...
	 * 
	 */
	public String getListViewForSentMail(Message[] msgs, String sessionType)throws EazyBpmException, MessagingException, UnsupportedEncodingException{
		List<Map<String, Object>> mailList = new ArrayList<Map<String, Object>>();
		for(int i = msgs.length-1; i >= 0; i--){
			Map<String, Object> mailDetails=new HashMap<String, Object>();
			if(!msgs[i].isSet(Flags.Flag.SEEN)){
		...
		return generateScriptForMail("sent-mail", mailList, sessionType);
	}
	 ...
	 * 
	 */
	public String getListViewForSentLaterMessage(Message[] msgs, String sessionType)throws EazyBpmException, MessagingException, UnsupportedEncodingException{
		List<Map<String, Object>> mailList = new ArrayList<Map<String, Object>>();
		for(int i = msgs.length-1; i >= 0; i--){
			Map<String, Object> mailDetails=new HashMap<String, Object>();
			if(!msgs[i].isSet(Flags.Flag.SEEN)){
		...
		return generateScriptForMail("sent-later", mailList, sessionType);
	}
	
	/**
	 * {@inheritDoc}
	 */
	public String getListViewForMailInbox(JwmaFolderImpl folder,String from, String sessionType)throws EazyBpmException{
		...
		return generateScriptForMail(from,mailList, sessionType);
	}
	 ...
	 * @return
	 */
	private String generateScriptForMail(String from, List<Map<String, Object>> mailList, String sessionType){
		Map<String, Object> context = new HashMap<String, Object>();
		List<Map<String, Object>> mailNameList = new ArrayList<Map<String, Object>>();
		String columnNames ="";
		if(from.equalsIgnoreCase("inbox")){
			columnNames = "['Status','From','Subject','Date','isDraft','isSent','messageNumber','folderName', 'Type']";
		}else{
			columnNames = "['Status','To','Subject','Date','isDraft','isSent','messageNumber','folderName']";
		}
		if(from.equalsIgnoreCase("draft")){
			context.put("title", "Draft");
		} else if(from.equalsIgnoreCase("sent-mail")) {
			if(sessionType.equalsIgnoreCase("email")){
				context.put("title", "Sent Mail");
			} else {
				context.put("title", "Sent Message");
			}
		} else if(from.equalsIgnoreCase("sent-later")){
			context.put("title", "Sent Later");
		} else {
			context.put("title", "Inbox");
		}
