...
@WebService(serviceName = "TaskService", endpointInterface = "com.eazytec.bpm.runtime.task.service")
	...
	public Map<String,Object> getTaskCountByUserId(User user) throws Exception{
	 ...
	 /**
		...
		public List<Task> getToDoListByUser(User user) throws BpmException{
			List<Task> loggedInUserTaskEntiyList = new ArrayList<Task>();
			List<Task> taskList = new ArrayList<Task>();
			taskList = getMyBucketGridData(user, 2);
			Set<Task> taskSet = new HashSet<Task>(taskList);
			loggedInUserTaskEntiyList = new ArrayList<Task>(taskSet);
			return loggedInUserTaskEntiyList;
		}
...
/*		if (user.getGroupIds() != null && user.getGroupIds().size() > 0) {
	 ...
	 * @return
	 */
	public String convertDateToSTDString(String dateStr) {
	        ...
	        try{
	            ...
	            for (Map<String, Object> taskMap : taskListAsMap) {
	            	...
	            	taskMap.remove("htmlSourceForSubForm");
	            	 //TaskEntity taskEntity = (TaskEntity)taskService.createTaskQuery().taskId((String)taskMap.get("id")).list().get(0);
	            		...
	            		if(entities != null && entities.size() > 0){
	            			...
	            			processDefinition= processDefinitionService.getProcessDefinitionById(taskEntity.getProcessDefinitionId());
	            			...
	            			processDefinition= processDefinitionService.getProcessDefinitionById(taskMap.get("processDefinitionId").toString()); 	 	
            			...
            			if(taskMap.containsKey("createTime")&& taskMap.get("createTime")!=null){
            				taskMap.put("createTime",convertDateToSTDString(taskMap.get("createTime").toString()));
            			}else {
            			...
            			if(taskMap.containsKey("endTime")&& taskMap.get("endTime")!=null){
            				taskMap.put("endTime",convertDateToSTDString(taskMap.get("endTime").toString()));
            			}else {
	                    ...
	                    if(type.equalsIgnoreCase("myinvolvements") || type.equalsIgnoreCase("waitinglist")){
	                        //if((taskMap.get("taskClaimable") != null) && (taskMap.get("taskClaimable").toString() == "true")){
	                    	...
	                    	/*TaskFormData taskFormData = getHistoricTaskFormData(taskMap.get("id").toString());
		    				String formKey = taskFormData.getFormKey();
		    				if(formKey != null) {
		    					taskMap.put("formId", formKey);
		    				}*/
	                    }
                    	if(taskMap.get("createTime") == null){
                    		if(taskMap.get("startTime") != null){
                    			taskMap.put("createTime",convertDateToSTDString(taskMap.get("startTime").toString()));	
                    		}
                    	}
    ...
    public Map<String,Object> checkAndGetPastValuesForInstance (String processInstanceId,String taskId){
        ...
        for(Map<String,Object> historicDetails : historicDetailsMap){
          if(!elementValuesMap.containsKey(historicDetails.get("columnId"))){
        	  ...
        	  if(historicDetails.get("name").toString().startsWith("subForm_") && !elementValuesMap.containsKey(historicDetails.get("columnId")+"_"+historicDetails.get("name"))) {
              	elementValuesMap.put(historicDetails.get("columnId")+"_"+historicDetails.get("name"), historicDetails.get("columnValue") );
              } else {
              	// replacing line breaks with space (for ckeditor bug fix)
              	String columnValue = (String) historicDetails.get("columnValue");
              	if(!columnValue.isEmpty() && columnValue != null) {
              		columnValue = columnValue.replaceAll("(\r\n|\n)", " ");
              	}
                  elementValuesMap.put(historicDetails.get("columnId").toString(), columnValue );
              }
            }
            if(!isInsert && historicDetails.get("taskId") != null){
                elementValuesMap.put("taskId", historicDetails.get("taskId"));
	...
	public Map<String, Map<String, Object>> setFormFieldAutomatic(List<Map<String, String>> automaticFillFieldInfo,List<Opinion> opinionList,Task currentTask ) throws EazyBpmException{
		...
		if(hisTask != null){
			hisTaskId = hisTask.getId();
			 OrgSignOffTime = DateUtil.convertDateToStringFormat(hisTask.getEndTime(), "yyyy-mm-dd HH:mm");
			 organizer = hisTask.getAssignee();
			 
		}else{
			// Set Start node organizer detail
			OrgSignOffTime =  DateUtil.convertDateToStringFormat(currentTask.getCreateTime(), "yyyy-mm-dd HH:mm");
			...
			if(!defId.equalsIgnoreCase(parentProcessDefinitionId)) {
				...
				JSONObject parentTraceJson = null;
					for(Task traceTask:parentTask){
						...
						parentTraceJson.put("createdtime", convertDateToSTDString(traceTask.getCreateTime().toString()));
	...
	public List<Map<String, Object>> resolveHistoricTaskInstance(String processInstanceId) {
		...
		try{
			...
			for(Map<String, Object> historicTaskInstanceMap : taskListAsMap){
				historicTaskInstanceMap.put("startTime",convertDateToSTDString(historicTaskInstanceMap.get("startTime").toString()));
				if(historicTaskInstanceMap.get("endTime") != null){
					historicTaskInstanceMap.put("endTime",convertDateToSTDString(historicTaskInstanceMap.get("endTime").toString()));
				}
