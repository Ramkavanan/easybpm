...
@WebService(serviceName = "ProcessService", endpointInterface = "com.eazytec.bpm.runtime.process.service")
public class ProcessServiceImpl implements ProcessService{
	...
	public List<HistoricProcessInstance> getProcessIntanceByUser(String loggedInUser) throws BpmException {		
		return historyService.createHistoricProcessInstanceQuery().startedBy(loggedInUser).list();
	}	
	...
	public boolean saveClassification(Classification classification,HttpServletRequest request)  throws BpmException{
		
		String classificationName = classification.getName().trim().replaceAll("\\s+", " ");
   	 boolean isClassificationExist = isClassificationExist(classificationName);
			log.info("Is classi exist ---------- "+isClassificationExist);
			if(!isClassificationExist){
	             ...
	             classification.setId(classification.toString().trim().toLowerCase().replaceAll("\\s+", " ").replaceAll(" ", "_"));
	             classification.setName(classificationName);
	             if(classification.getDescription()!=null){
	            	 classification.setDescription(classification.getDescription().trim().replaceAll("\\s+", " "));
	             }
	             processDao.saveClassification(classification);
	             request.getSession().getServletContext().setAttribute(Constants.PROCESS_CLASSIFICATION, mgr.getAllProcessClassifications());
	             
	             log.info("Classification added successfully");
			}
		return isClassificationExist;
	}
	...
	public void designArchivedProcessForm(TaskEntity task,String resourcePath,String stylePath, String ScriptPath)throws BpmException, EazyBpmException, JDOMException, IOException{
		...
		if(formKey!=null){
			...
			if(form!=null){
				...
				String processName = URLEncoder.encode(processDefinition.getName(), "UTF-8"); // encoding for showing chinease archived process
...
//	        	throw new BpmException("All or selected tasks contains no Form");
	 ...
	 * {@inheritDoc}
	 */
	public Response deleteProcessForAPI(String name){
		try{
			List<String> processDefinitionKeys = new ArrayList<String>();
			processDefinitionKeys.add(name);
		//deleteProcessDefinition(id, true);
			deleteAllProcessDefinition(processDefinitionKeys,true);
	 ...
	 * {@inheritDoc}
	 */
	public Response startProcessForAPI(String processKey) {
		try {
			  Map<String, Object> rtFormValues = new HashMap<String, Object>();
			  rtFormValues = CommonUtil.convertJsonToMap(processKey);
	    	   ProcessInstance proIns = startProcessInstanceById((String)rtFormValues.get("processKey"));
	    	   ...
	    	   operatingFunctionService.setJointConductionByOrder(proIns.getId(), (String)rtFormValues.get("nextTaskOrganizers"), "",(String)rtFormValues.get("organizerOrders"),false, null,false,false);
	    	   Map<String, String> formValues = (Map) rtFormValues;
	    	   formValues.put("taskId", task.getId());
	    	   formValues.put("processInstanceId", proIns.getId());
	    	  // rtFormService.submitTaskForm(task.getId(), formValues, null, null, null, true, proIns.getId());
	    	 	User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
	    	 	OperatingFunctionAudit operFunAudit = new OperatingFunctionAudit(user.getId(),task.getId(),proIns.getProcessDefinitionId().split(":")[0] ,OperatingFunction.SUBMIT,proIns.getId(), (String)rtFormValues.get("processKey"));
