...
public class TaskController extends BaseFormController{
	...
	public ModelAndView showTaskDetail(@RequestParam("taskId") String id, ModelMap model, HttpServletRequest request){
		try{
			...
			String taskCreateTime = rtTaskService.convertDateToSTDString(task.getCreateTime().toString());
	...
	public ModelAndView showTaskFormDetail(@RequestParam("taskId") String taskId,@RequestParam("suspendState") String suspendState,ModelMap model, HttpServletRequest request, @RequestParam("isStartNodeTask") boolean isStartNodeTask,
		...
		try{
			...
			/**
			 * Getting active task to highlight in Instance Trace 
			 */
			...
			model.addAttribute("lastOperationPerformed", lastOperationPerformed);
			...
			if(historicVariables!=null&&historicVariables.size()>0){
				...
				//log.info("Operating Process :"+processDefinitionEntity.getName());
				...
				model = OperatingFunctionUtil.setOperatingFunctionTypes(model);
	   			...
	   			if(currentTask != null){
		   			...
		   			model.addAttribute("nodeType", String.valueOf(currentTask.getTaskDefinition().getNodeTypeExpression()));
		   			// set automatic form fill values
			...
			if(currentTask == null) {
				...
				// for checking if process completed
				...
				Task doneTask =  taskDefinitionService.getTaskByProcessInstanceId(task.getProcessInstanceId());
	   			if(doneTask != null){
//		   				OperatingFunction withdrawOperation = new OperatingFunctionImpl();
//		   				withdrawOperation = rtTaskService.getOperatingFunctionForStartNode(user, doneTask);
			   			model.addAttribute("operatingFunctionForTask", rtTaskService.getOperatingFunctionForStartNode(user, doneTask));
	   			}
				
			/*	boolean isNextTaskEnd = true; 
				boolean isBackOperation = false;
				List<HistoricTaskInstance> historicTaskInstances = rtTaskService.getHistoricTaskInstances(task.getProcessInstanceId());
				for(HistoricTaskInstance historicTaskInstance : historicTaskInstances){
					//For checking the same task has been recalled also in active state
					if(!historicTaskInstance.getTaskDefinitionKey().equalsIgnoreCase((task.getTaskDefinitionKey()))){
						...
						if(historicTaskInstance.getStartTime().after(task.getStartTime()) && historicTaskInstance.getEndTime() == null){
							isNextTaskEnd =false;
							break;
						}
						// recall option should not be shown if current task is backed to previous node
						if(historicTaskInstance.getDeleteReason() != null) {
							if(historicTaskInstance.getDeleteReason().equalsIgnoreCase(Constants.TASK_JUMP) || 
									historicTaskInstance.getDeleteReason().equalsIgnoreCase(Constants.TASK_RETURNED)	) {
				...
				if(!isNextTaskEnd && !isBackOperation){
					model.addAttribute("recallOperatingFunctionForTask",rtTaskService.getRecallFunctionForHistoricTask(user, task));
					// withdraw function for creator
					Task doneTask =  taskDefinitionService.getTaskByProcessInstanceId(task.getProcessInstanceId());
		   			if(doneTask != null){
...
//			   				withdrawOperation = rtTaskService.getOperatingFunctionForStartNode(user, doneTask);
	...
	/*private void setInstanceTraceData(Task currentTask,ModelMap model){
		ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionId(currentTask.getProcessDefinitionId()).active().list().get(0);
		String workFlowTrace = rtProcessService.getSvgString(processDefinition.getDeploymentId(), processDefinition.getResourceName());
		List<Task> tasks = rtTaskService.getWorkFlowTraceData(currentTask.getProcessInstanceId());
		Map<String,Object> elementValuesMap = rtTaskService.checkAndGetPastValuesForForm(currentTask.getProcessInstanceId(), currentTask.getId());
		...
		try {
			...
			throw new BpmException("Error while rendering content");
		}
	...
	private String convertDateToSTDString(String dateStr) {
		...
		try {
			...
			log.info("Date parsed for" +dateStr);
...
//		log.info(" Get Task type:"+type);
		...
		try{
			...
			if(type.equalsIgnoreCase("mybucket")){
				/*Map<String, String> keyAndValues = CommonUtil.getKeyValuePairs(request.getParameterMap());
				String processInstanceId = keyAndValues.get("processInstanceId");
				TaskEntity taskEntity = (TaskEntity) taskDefinitionService.getTaskByProcessInstanceId(processInstanceId);
				if(taskEntity != null){
					startScriptName = taskEntity.getStartScriptName();
            		...
            		if(startScriptName!=null && !startScriptName.isEmpty()){
                         ...
                         model.addAttribute("startScript", java.net.URLDecoder.decode(startScript, "UTF-8"));
            		}else{
			...
			if(type.equalsIgnoreCase("mybucket") || type.equalsIgnoreCase("documentToBeDone")){        
				...
				gridTitle = appResourceBundle.getString("Read List");
