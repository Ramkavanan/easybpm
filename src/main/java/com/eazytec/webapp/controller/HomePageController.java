...
//			log.info(" widget List "+widgetNames);
			...
			if(quickNavigationPosition == assignedWidgetList.size()){
...
//		log.info("getting widgets for id's");
	...
	public @ResponseBody
	List<Map<String, Object>> getHomePageRootNodes(ModelMap model,
			HttpServletRequest request) {
		log.info("Getting homepage org tree root nodes");
		List<Map<String, Object>> nodeListOfMap = new ArrayList<Map<String, Object>>();
		try {
			CommonUtil.createRootTreeNode(nodeListOfMap, "Public", "public",
					"Public");
			CommonUtil.createRootTreeNode(nodeListOfMap, "Roles", "roles",
					"Roles");
			CommonUtil.createRootTreeNode(nodeListOfMap, "Departments",
	...
	public @ResponseBody
	String getUserSelectionChildNodes(ModelMap model,
			@RequestParam("currentNode") String currentNode,
			@RequestParam("rootNode") String rootNode,
			@RequestParam("nodeLevel") String nodeLevel, User user,
			BindingResult errors, HttpServletRequest request,
			HttpServletResponse response) {
		...
		try {
			if (rootNode.equalsIgnoreCase("Roles")) {
				nodes = layoutService.getRolesChildNode(currentNode,
						Integer.parseInt(nodeLevel));
			} else if (rootNode.equalsIgnoreCase("Departments")) {
				nodes = layoutService.getDepartmentsChildNode(currentNode,
						Integer.parseInt(nodeLevel));
			}
