...
public class NoticeController extends BaseFormController{
	...
	@RequestMapping(value = "bpm/notice/showDetailNotice",method = RequestMethod.GET)
	   public ModelAndView showDetailNotice(@RequestParam("noticeInstanceId") String noticeInstanceId, 			  
		...
		try{
			...
			if(properties.containsKey("CREATEDBY") && properties.get("CREATEDBY")!= null && properties.get("CREATEDBY").toString().equalsIgnoreCase(CommonUtil.getLoggedInUserId())){
			...
			//script = TemplateUtil.generateScriptForTemplate(velocityEngine, "com/eazytec/bpm/form/templates/showEditStartHtmlForm.vm", context);
*/			script = designEditProcessScript(noticeDefinition,  noticeInstanceId,context);
	...
	public ModelAndView showNoticeTemplate(@RequestParam("noticeInstanceId") String noticeInstanceId,ModelMap model, HttpServletRequest request) {
		...
		try {
			...
			saveError(request, e.getMessage());
			log.error("Cannot get news details for instance " + noticeInstanceId
					+ e.getMessage(), e);
		  ...
		  try{
			  ...
			  if(noticeInstance!=null ){
				  ...
				  if(rtFormValues.containsKey(STATUS) && rtFormValues.get(STATUS) != null && rtFormValues.get(STATUS).equalsIgnoreCase("formal")){
					  ...
					  /*if(rtFormValues.containsKey("userName") && rtFormValues.get("userName") != null){
						  String users = rtFormValues.get("userName");
						  List<String> userIds = Arrays.asList(users.split("\\s*,\\s*"));
						  publishUser.addAll(getUserManager().getUserByUserIds(userIds));
					  }*/
					 if(rtFormValues.containsKey(IS_NOTIFICATION) && rtFormValues.get(IS_NOTIFICATION) != null){
					  ...
					  /*if(noticeInstance.getId() !=null){
						  endEventScript.put("processInstanceId", noticeInstance.getId());
	...
	private String getStratProcessScript(Map<String, Object> properties,Map<String,Object> permissionVal)
			throws JSONException {
		Map<String, Object> context = new HashMap<String, Object>();
		ProcessDefinition noticeDefinition = noticeService
				.getNoticeDefinition();
		...
		context.put("pastValuesJson", CommonUtil.convertMapToJson(properties));
		// context.put("processDefinition", noticeDefinition);
		return TemplateUtil.generateScriptForTemplate(velocityEngine,
				"com/eazytec/bpm/form/templates/showStartHtmlForm.vm",
				context);
	}
