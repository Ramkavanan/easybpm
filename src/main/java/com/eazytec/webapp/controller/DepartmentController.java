...
public class DepartmentController extends BaseFormController {
	        ...
	        if (validator != null) {
   			...
   			if(originalVersion == 0){
   		        if(departmentService.isDuplicateDepartment(department.getName())){
   		        	errors.rejectValue("name", "errors.existing.department",
   		                    new Object[]{department.getName()}, "duplicate department");
   					department.setVersion(originalVersion);
   					model.addAttribute("administrators", administrators);
   	   				model.addAttribute("interfacePeople", interfacePeople);
	   	     		User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    ...
    /**
	 * Updates Order No of Department when the user click on move up and
	 * move down images in departments  grid.
	 * 
    ...
    public ModelAndView showDepartments(@RequestParam(required = false, value = "q") String query) throws Exception {
        ...
        /*	List<Department> departmentList=departmentService.getDepartments();
        	...
        	String[] fieldNames={"departmentId","name","description","departmentOwner","createdTimeByString","departmentType","orderNo"};
        	String script=GridUtil.generateScriptForDepartmentGrid(CommonUtil.getMapListFromObjectListByFieldNames(departmentList,fieldNames,""), velocityEngine);
	...
	public ModelAndView deleteSelectedDepartments(@RequestParam("departmentIds") String departmentIds, HttpServletRequest request)throws Exception {
		...
		try{
			...
			//check whether the user have permission to delete the department or not 
	...
	/*	if(!department.getDepartmentalRoles().isEmpty()){
  			for(Role role:department.getDepartmentalRoles()){
	...
	/**
	 * method to select users based on the jstree selection ie Roles,Groups,Depts etc
	 * 
	 * @param departmentIds
	 * @param request
	 * 
	...
	public @ResponseBody
	String getDepartmentUserGrid(@RequestParam("id") String id,
		...
		try {
			...
			/*if (users != null) {
				userSet = new HashSet<User>(users);
			}*/
			...
			log.error(e.getMessage(), e);
			saveError(request, getText("error.department.delete", e
					.getMessage(), locale));
		}
		return script;
	}
	 ...
	 */
	 ...
	 @RequestMapping(value="/admin/getDepartmentsBySuperDep", method = RequestMethod.GET)
	   public @ResponseBody List<Map<String, String>> getDepartmentsBySuperDep(@RequestParam("superDep") String superDep) {
		  ...
		  try{
			 /* if(superDep.equals("Departments")) {
				  Map<String,String> departmentDetail = new HashMap<String, String>();
    ...
    public ModelAndView showDepartmentSelection(ModelMap model, @RequestParam("selectionType") String selectionType, 
    ...
    public @ResponseBody String getDepartmentNodes(ModelMap model,@RequestParam("currentNode") String currentNode, @RequestParam("rootNode") String rootNode, @RequestParam("nodeLevel") int nodeLevel, 
    ...
    public @ResponseBody String getDepartmentChildNodes(ModelMap model,@RequestParam("currentNode") String currentNode, 
    		User user, BindingResult errors, HttpServletRequest request, HttpServletResponse response){
		Locale locale = request.getLocale();
		JSONArray nodes = new JSONArray();
    	try {
    ...
    public @ResponseBody String getSuperDepartmentChildNodes(ModelMap model,@RequestParam("currentNode") String currentNode, 
    		User user, BindingResult errors, HttpServletRequest request, HttpServletResponse response){
		Locale locale = request.getLocale();
		JSONArray nodes = new JSONArray();
    	try {
    ...
    public @ResponseBody String getUserAdministrationSuperDepartmentNodes(ModelMap model,@RequestParam("currentNode") String currentNode, @RequestParam("rootNode") String rootNode, @RequestParam("nodeLevel") int nodeLevel, 
    		User user, BindingResult errors, HttpServletRequest request, HttpServletResponse response){
		Locale locale = request.getLocale();
		JSONArray nodes = new JSONArray();
    	try {
    ...
    public @ResponseBody String getUserAdministrationDepartmentNodes(ModelMap model,@RequestParam("currentNode") String currentNode, @RequestParam("rootNode") String rootNode, @RequestParam("nodeLevel") int nodeLevel, 
    		User user, BindingResult errors, HttpServletRequest request, HttpServletResponse response){
		Locale locale = request.getLocale();
		JSONArray nodes = new JSONArray();
    	try {
	...
	public @ResponseBody
	String getDepartmentGrid(@RequestParam("id") String id,
			@RequestParam("parentNode") String parentNode,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		String script = null;
		...
		try {
			...
			//}
			/*if (parentNode.equalsIgnoreCase("organization")) {
				departmentList = userService.getAllUsersByDepartment(id);
			} else if (parentNode.equalsIgnoreCase("group")) {
				users = userService.getAllUsersByGroup(id);
			} else if (parentNode.equalsIgnoreCase("role")) {
				users = userService.getAllUsersByRole(id);
			...
			script = GridUtil.generateScriptForDepartmentGrid(CommonUtil
					.getMapListFromObjectListByFieldNames(department, fieldNames,""), velocityEngine);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			saveError(request, getText("error.department.delete", e
					.getMessage(), locale));
