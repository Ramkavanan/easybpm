...
public class TableController extends BaseFormController{
    ...
    public ModelAndView saveTable(@ModelAttribute("metaTable") MetaTable metaTable,@RequestParam("isModuleChanged") boolean isModuleChanged,@RequestParam("isAutoFormCreate") boolean isAutoFormCreate, ModelMap model,HttpServletRequest request,BindingResult errors){
        ...
        try {
               ...
               if(isAutoFormCreate){
            	   List<MetaTableColumns> tableColumns = tableService.getAllMetaTableColumnsByTableId(tableId, getText("table.default.values",null));
    ...
    @RequestMapping(value="/bpm/table/getAllColumnByTableId", method = RequestMethod.GET)
      public @ResponseBody List<Map<String, String>> getAllColumnByTableId(@RequestParam("tableId") String tableId,@RequestParam("fieldType") String fieldType, HttpServletRequest request) throws BpmException{
        List<Map<String, String>> tableColumnDetailsList = new ArrayList<Map<String, String>>();
         try{
             String defaultColumns=getText("table.default.values",null);
         ...
         String[] executeCmd = new String[]{importCommand+" "+fileNameWithPath};
            ...
            try {
                ...
                if (processComplete >= 0) {
                	System.out.println("moduleQuery "+moduleQuery);
     ...
     * @throws Exception
     */
    @RequestMapping(value="/bpm/table/deleteAllNoticeByIds",method = RequestMethod.GET)
    public @ResponseBody Map<String,Object> deleteAllNoticeByIds(@RequestParam("noticeIds") String noticeIds,HttpServletRequest request,HttpServletResponse response) throws Exception {
    	
    	Locale locale = request.getLocale();
		 Map<String,Object> message = new HashMap<String, Object>();
		try {
			
		    List<String> noticeIdList=CommonUtil.convertJsonToListStrings(noticeIds);
		    tableService.deleteAllNoticeByIds(noticeIdList);
			 message.put("successMessage", getText("notice.deleted",locale));
		
		} catch (EazyBpmException e1) {
			// TODO Auto-generated catch block
			  message.put("errorMessage", getText("error.notice.delete",locale));
    ...
    @RequestMapping(value="/bpm/table/deleteAllNewsByIds",method = RequestMethod.GET)
    public @ResponseBody Map<String,Object> deleteAllNewsByIds(@RequestParam("newsIds") String newsIds,HttpServletRequest request,HttpServletResponse response) throws Exception {
    	
    	Locale locale = request.getLocale();
		 Map<String,Object> message = new HashMap<String, Object>();
		try {
			
		    List<String> newsIdList = CommonUtil.convertJsonToListStrings(newsIds);
		    tableService.deleteAllNewsByIds(newsIdList);
			 message.put("successMessage", getText("news.deleted",locale));
		
		} catch (EazyBpmException e1) {
			// TODO Auto-generated catch block
			  message.put("errorMessage", getText("news.error.deleted",locale));
			//saveError(request,	getText("error.trigger.delete", e1.getMessage(), locale));
			e1.printStackTrace();
		}
		 return message;
