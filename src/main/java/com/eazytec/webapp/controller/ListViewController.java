...
public class ListViewController extends BaseFormController {
	...
	ModelAndView showListViewGrid(
			@RequestParam("listViewName") String listViewName,
			@RequestParam("title") String title,
			@RequestParam("container") String container,
			@RequestParam("listViewParams") String listViewParams,
			@RequestParam(value = "needHeader", required = false) String needPageHeader,
		...
		try {
			List<Map<String, Object>> listViewParamsList = CommonUtil
					.convertJsonToList(listViewParams);
			...
			script = listViewService.showListViewGrid(
					listViewName.toUpperCase(), title, container,
					namedParameter, false, null, needHeader);
			...
			saveError(request,
					getText("list.view.error", e.getMessage(), locale));
			model.addAttribute("script",
					"Problem loading grid. " + e.getMessage());
		} catch (Exception e) {
			model.addAttribute("success", false);
			model.addAttribute("script",
					"Problem loading grid. " + e.getMessage());
		...
		if ((listViewName.equals("PROCESS_LIST")
				|| listViewName.equals("PROCESS_LIST_VERSION")
				|| listViewName.equals("PROCESS_LIST_MY_INSTANCES")
				|| listViewName.equals("AGENCY_LIST")) && needHeader) {
			model.addAttribute("jsonTree",
					ProcessDefinitionUtil.getJsonTreeForProcessList(request));
	...
	public ModelAndView showListView(
			@ModelAttribute("listView") ListView listView,
			ModelMap model,
			@RequestParam(value = "isFromModule", required = false) String isFromModule,
	...
	public ModelAndView showDefaultListView(
			@ModelAttribute("listView") ListView listView, ModelMap model,
			HttpServletRequest request) {
		model.addAttribute("listView", listView);
	...
	public @ResponseBody
	List<Map<String, Object>> columnsProperty(
			@ModelAttribute("listViewId") String listViewId, ModelMap model,
			HttpServletRequest request) {
		...
		try {
			...
			String[] fieldNames = {"id", "columnTitle", "dataFields", "width",
					"otherName", "orderBy", "orderNo", "replaceWords", "isSort","columnType",
					"isAdvancedSearch", "isSimpleSearch", "textAlign",
					"isHidden", "onRenderEvent", "onRenderEventName",
					"comment", "dataFieldType", "dataDictionary", "isRange" };
			mapViewColumnProperty = CommonUtil
					.getMapListFromObjectListByFieldNames(
							listViewColumnProperty, fieldNames, "");
		} catch (BpmException e) {
			log.error(e.getMessage(), e);
	...
	public ModelAndView saveListView(
			@ModelAttribute("listView") ListView listView,
			@RequestParam("isListViewTemplate") boolean isListViewTemplate,
			ModelMap model, HttpServletRequest request, BindingResult errors)
			throws Exception {
		Locale locale = request.getLocale();
		try {
			User user = (User) SecurityContextHolder.getContext()
			...
			if (listView.getId() != null && listView.getId() != ""
					&& !String.valueOf(listView.getId()).isEmpty()) {
				modeType = "update";
			...
			listView = listViewService.saveListView(listView, modeType,
					moduleId);
			...
			if (listViewId != null && !listViewId.isEmpty()
					&& modeType.equalsIgnoreCase("update")) {
				...
				log.info("List View '" + listView.getViewName()
				...
				return showListViewDesignEditScreen(listView,
						isListViewTemplate, moduleId, model);
			}
		} catch (Exception e) {
			log.error("Error while saving a List View ", e);
			saveError(request,
					getText("listview.error", e.getMessage(), locale));
		}
		return new ModelAndView("listview/showListView", model);
	}
	 ...
	 * @return
	 */
	private ModelAndView showListViewDesignEditScreen(ListView listView,
			boolean isListViewTemplate, String moduleId, ModelMap model) {
		try {
			model.addAttribute("newListViewModuleId", moduleId);
			model.addAttribute("showNewListView", "true");
			model.addAttribute("listView", listView);
	...
	public @ResponseBody
	Map<String, Object> checkListViewExist(
			@RequestParam("listViewName") String listViewName, ModelMap model,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		...
		try {
			if (listViewService.checkListViewName(listViewName)) {
				message.put(
						"error",
						getText("listview.name.duplicate", listViewName, locale));
				// log.info("This List View Name ({0}) already exists.  Please try a different List view name.");
			} else {
	...
	public @ResponseBody
	Map<String, Object> saveListViewColumns(
			@ModelAttribute("listViewId") String listViewId,
			@ModelAttribute("listViewColumns") String listViewColumns,
		...
		try {
			...
			List<Map<String, Object>> listViewMetaColumns = CommonUtil
					.convertJsonToList(listViewColumns);
			// List<String>
			...
			if (listViewMetaColumns != null && !listViewMetaColumns.isEmpty()) {
				ListView listViewObj = listViewService
						.getListViewById(listViewId);
				Set<ListViewColumns> columsnSet = listViewObj
						.getListViewColumns();
				...
				listView = listViewService.saveListViewColumnDetails(
						listViewObj, listViewMetaColumns,
						isListViewColumnUpdate, moduleId);
			}
	...
	public @ResponseBody
	List<Map<String, Object>> groupSettingProperty(
			@ModelAttribute("listViewId") String listViewId, ModelMap model,
			HttpServletRequest request) {
		List<Map<String, Object>> mapViewGroupSettingProperty = null;
		try {
			List<ListViewGroupSetting> listViewGroupSettingProperty = listViewService
					.getGroupSettingPropertyByListViewId(listViewId);
			String[] fieldNames = { "id", "groupName", "groupFieldsName",
					"parentGroup", "sortType", "comment", "groupType",
					"orderBy" };
			mapViewGroupSettingProperty = CommonUtil
					.getMapListFromObjectListByFieldNames(
							listViewGroupSettingProperty, fieldNames, "");
		} catch (BpmException e) {
			log.error(e.getMessage(), e);
		} catch (EazyBpmException e) {
			log.error(e.getMessage(), e);
	...
	public @ResponseBody
	Map<String, Object> saveListViewGroupSetting(
			@RequestParam("listViewId") String listViewId,
			@RequestParam("createGroupSetting") String createGroupSetting,
			@ModelAttribute("moduleId") String moduleId, ModelMap model,
			HttpServletRequest request) throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			if (createGroupSetting != null
					&& !createGroupSetting.equals("null")) {
				createGroupSetting = URLDecoder.decode(createGroupSetting, "UTF-8");
				listViewMetaColumns = CommonUtil
						.convertJsonToList(createGroupSetting);
			}
			...
			Set<ListViewGroupSetting> groupSet = listViewObj
					.getListViewGroupSetting();
			if (!groupSet.isEmpty()) {
				isListViewGroupSettingUpdate = true;
			}

			listView = listViewService.saveListViewGroupSettingDetails(
					listViewObj, listViewMetaColumns,
					isListViewGroupSettingUpdate, moduleId);
			// }
	...
	public @ResponseBody
	Map<String, Object> checkListViewQuery(
			@RequestParam("listViewId") String listViewId,
			HttpServletRequest request) throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			if (listViewService.checkListViewQuery(listView)) {
				message.put("success",
						getText("listview.query.checked", locale));
			}
	...
	public @ResponseBody
	Map<String, Object> validateListViewQuery(HttpServletRequest request)
			throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			if(rolesList.contains("ROLE_ADMIN") && constructedQuery.indexOf(":loggedInUserReport") > -1) {
				...
				if (constructedQuery.indexOf(":loggedInUser") > -1) {
					...
					constructedQuery = constructedQuery.replaceAll(":loggedInUser",
							userName);
				} 
			  ...
			  if(constructedQuery.indexOf(":userDepartment") > -1) {
					...
					constructedQuery = constructedQuery.replaceAll(":userDepartment",
							userDepartment);
				}
			...
			message.put("error",
					getText("listview.query.checked.error", locale));
		}
		return message;
	}
	...
	public @ResponseBody
	List<Map<String, String>> getAllSelectColumnsWithoutAlice(
			@RequestParam("constructedQuery") String constructedQuery,
		...
		try {
			...
			List<LabelValueBean> columnNameBean = queryParser
					.getAllSelectColumnsWithoutAlice();
	...
	public ModelAndView advanceSearch(
			@RequestParam("listViewId") String listViewId, ModelMap model,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			List<ListViewColumns> listViewColumnPropertyAdvancedSearch = new ArrayList<ListViewColumns>();
			List<ListViewColumns> listViewColumnProperty = listViewService
					.getColumnsPropertyByListViewId(listViewId);
			...
			model.addAttribute("listViewColumns",
					listViewColumnPropertyAdvancedSearch);
		} catch (EazyBpmException e) {
			message.put("error",
					getText("listview.show.advance.search.error", locale));
	...
	public @ResponseBody
	List<Map<String, Object>> reloadGridWithSearchParams(
			@ModelAttribute("listViewId") String listViewId,
			@RequestParam("searchParams") String searchParams,
			@RequestParam("sortType") String sortType,
			@RequestParam("sortTypeColumn") String sortTypeColumn,
		...
		List<Map<String, Object>> mapViewColumnProperty = null;
		try {
			Map<String, Object> searchParamMap = CommonUtil.convertJsonToList(
					searchParams).get(0);
			List<Map<String, Object>> listViewParamsList = CommonUtil
					.convertJsonToList(listViewParams);
			Map<String, Object> namedParameter = new HashMap<String, Object>();
			...
			ListView listView = listViewService.getListViewById(listViewId);
			mapViewColumnProperty = listViewService
					.getAllListViewMapDataWithSearchParams(listView,
							searchParamMap, sortType, sortTypeColumn,
							searchType, namedParameter);
			// log.info("Getting List View grid data for given parameters Successfully");
		} catch (BpmException e) {
			message.put("error",
					getText("listview.advance.search.data.error", locale));
			log.error(e.getMessage(), e);
		} catch (EazyBpmException e) {
			message.put("error",
					getText("listview.advance.search.data.error", locale));
	...
	public @ResponseBody
	List<Map<String, Object>> reloadGridWithSearchParamsAndConstraints(
			ModelMap model, HttpServletRequest request) {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		List<Map<String, Object>> mapViewColumnProperty = null;
		try {
			List<Map<String, Object>> listViewParamsList = CommonUtil
					.convertJsonToList(request.getParameter("listViewParams"));
			...
			List<Map<String, Object>> searchParamListOfMap = CommonUtil
					.convertJsonToList(request.getParameter("searchParams"));
			ListView listView = listViewService.getListViewById(request
					.getParameter("listViewId"));
			mapViewColumnProperty = listViewService
					.getDataWithSearchParamsAndConstraints(listView,
							searchParamListOfMap,
							request.getParameter("sortType"),
							request.getParameter("sortTypeColumn"),
							request.getParameter("searchType"), namedParameters);
	...
	public @ResponseBody
	ModelAndView reloadListViewGridModuleGroups(
			@ModelAttribute("listViewId") String listViewId,
			@RequestParam("searchParams") String searchParams,
			@RequestParam("sortType") String sortType,
			@RequestParam("sortTypeColumn") String sortTypeColumn,
			@ModelAttribute("searchType") String searchType, ModelMap model,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		List<Map<String, Object>> mapViewColumnProperty = null;
		...
		try {
			...
			mapViewColumnProperty = listViewService
					.getAllListViewMapDataWithSearchParams(listView,
							searchParamMap, sortType, sortTypeColumn,
							searchType, namedParameters);

			script = listViewService.showListViewGrid(listView.getViewName(),
					"List", "rightPanel", null, true, mapViewColumnProperty,
					true);
			model.addAttribute("script", script);
			model.addAttribute("success", true);
	...
	public @ResponseBody
	List<Map<String, Object>> reloadGridWithSimpleSearch(
			@ModelAttribute("listViewId") String listViewId,
		...
		try {
			...
			//System.out.println("========"+searchValue+"========="+URLDecoder.decode(searchValue, "ISO8859_1"));
			ListView listView = listViewService.getListViewById(listViewId);
			mapViewColumnProperty = listViewService
					.getAllListViewMapDataWithSearchValue(listView,
							searchValue, namedParameters);
		} catch (BpmException e) {
			message.put("error", getText("listview.search.data.error", locale));
			log.error(e.getMessage(), e);
		} catch (EazyBpmException e) {
	...
	public @ResponseBody
	Map<String, String> checkListViewName(
			@RequestParam("listViewName") String listViewName,
			HttpServletRequest request) throws Exception {
		Locale locale = request.getLocale();
		try {
	...
	public @ResponseBody
	Map<String, Object> saveListViewButtons(
			@RequestParam("listViewId") String listViewId,
			@RequestParam("listViewButtons") String listViewButtons,
			@ModelAttribute("moduleId") String moduleId, ModelMap model,
			HttpServletRequest request) throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			if (listViewButtons != null && !listViewButtons.equals("null")) {
				...
				listViewMetaButtons = CommonUtil
						.convertJsonToList(listViewButtons);
			}
			...
			listView = listViewService.saveListViewButtonDetails(listViewObj,
					listViewMetaButtons, isListViewButtonUpdate, moduleId);
			...
			message.put("errorMessage",
					getText("listview.buttons.error", locale));
		}
	...
	public @ResponseBody
	List<Map<String, Object>> buttonsProperty(
			@ModelAttribute("listViewId") String listViewId, ModelMap model,
			HttpServletRequest request) {
		List<Map<String, Object>> mapViewColumnProperty = null;
		try {
			List<ListViewButtons> listViewButtonProperty = listViewService
					.getButtonPropertyByListViewId(listViewId);
			
			
			String[] fieldNames = { "id", "displayName", "orderBy",
					"buttonMethod", "iconPath", "tableName", "columnName",
					"redirectValue", "active","listViewButtonUsers","listViewButtonUsersFullName","listViewButtonDeps","listViewButtonRoles","listViewButtonGroups" };
			mapViewColumnProperty = CommonUtil
					.getMapListFromObjectListByFieldNames(
							listViewButtonProperty, fieldNames, "");
		} catch (BpmException e) {
			log.error(e.getMessage(), e);
		} catch (EazyBpmException e) {
			log.error(e.getMessage(), e);
	...
	public @ResponseBody
	ModelAndView showListViewDesignEdit(
			@RequestParam(value = "listViewId", required = false) String listViewId,
			@RequestParam(value = "moduleId", required = false) String moduleId,
			@RequestParam(value = "isListViewTemplate", required = false) boolean isListViewTemplate,
			@RequestParam(value = "isFromModule", required = false) String isFromModule,
	...
	public @ResponseBody
	ModelAndView showListViewGroupGrid(
			@RequestParam("listViewId") String listViewId,
			@RequestParam("moduleId") String moduleId,
			@RequestParam("listViewName") String listViewName,
			@RequestParam("title") String title,
			@RequestParam("container") String container, ModelMap model,
			HttpServletRequest request) {
		String script = null;
		ListView listView = null;
		Locale locale = request.getLocale();
		try {
			...
			if (listView.getListViewColumns().size() != 0) {
				script = listViewService.showListViewGrid(listViewName, title,
						container, null, false, null, true);

				model.addAttribute("listView", listView);
				model.addAttribute("listViewGroup",
						listViewService.constructJsonListViewGroup(listView));
				model.addAttribute("script", script);
				model.addAttribute("moduleId", moduleId);
				model.addAttribute("success", true);
			} else {
				return showListViewDesignEdit(listViewId, moduleId, false,
						"false", "", model, request);
			}
		} catch (BpmException e) {
			log.error(e.getMessage(), e);
	...
	public @ResponseBody
	Map<String, Object> deleteListViewDetails(
			@RequestParam("listViewId") String listViewId, ModelMap model,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		...
		try {
			...
			log.info("List View  '" + listViewObj.getViewName()
					+ "' is deleted successfully.");
			message.put(
					"success",
					getText("listview.deleted", listViewObj.getViewName(),
							locale));
		} catch (Exception e) {
			message.put(
					"error",
					getText("listview.delete.error", listViewObj.getViewName(),
							locale));
	...
	public @ResponseBody
	Map<String, Object> deleteBulkListViewDetails(
			@RequestParam("listViewIds") String listViewIds, ModelMap model,
			HttpServletRequest request) throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			List<String> listOfIds = CommonUtil
					.convertJsonToListStrings(listViewIds);
	...
	public @ResponseBody
	Map<String, Object> softDeleteTableDataAndRestore(
			@RequestParam("deleteTblIds") String deleteTblIds,
			@RequestParam("tableName") String tableName,
			@RequestParam("isDelete") String isDelete,
			@RequestParam("columnName") String columnName, ModelMap model,
			HttpServletRequest request) throws JSONException {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		List<String> deleteTableIds = CommonUtil
				.convertJsonToListStrings(deleteTblIds);
		try {
			int status = isDelete.equalsIgnoreCase("true") ? 1 : 0;
			int result = listViewService.softDeleteTableDataAndRestore(
					deleteTableIds, tableName, columnName, status);
			...
			message.put(
					"success",
					isDelete.equalsIgnoreCase("true") ? getText(
							"softdelete.success", tableName, locale) : getText(
							"restore.success", tableName, locale));
		} catch (Exception e) {
			message.put(
					"error",
					isDelete.equalsIgnoreCase("true") ? getText(
							"softdelete.error", tableName, locale) : getText(
							"restore.error", tableName, locale));
	...
	Map<String, Object> deleteData(
		...
		List<String> deleteTableIds = CommonUtil
				.convertJsonToListStrings(deleteTblIds);
		try {
			int result = listViewService.deleteTableData(deleteTableIds,
					tableName, columnName);
			log.info("'" + tableName + "' data is deleted successfully.");
			message.put("success",
					getText("delete.tabledata.success", tableName, locale));
		} catch (Exception e) {
			message.put("error",
					getText("delete.tabledata.error", tableName, locale));
			log.error("Error while deleting the " + tableName + " data ", e);
		}
		return message;
	}
	...
	public @ResponseBody
	Map<String, Object> deleteAllListViewDetails(
			@RequestParam("listViewName") String listViewName, ModelMap model,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			log.info("List View  '" + listViewName
					+ "' is deleted successfully.");
			message.put("success",
					getText("listview.deleted", listViewName, locale));
		} catch (Exception e) {
			message.put("error",
					getText("listview.delete.error", listViewName, locale));
	...
	public @ResponseBody
	String getUserSelectionChildNodes(
			@RequestParam("currentNode") String currentNode,
			@RequestParam("rootNode") String rootNode,
			@RequestParam("groupByFields") String groupByFields,
			@RequestParam("listViewId") String listViewId,
			@RequestParam("parentGroup") String parentGroup,
			@RequestParam("whereParams") String whereParams,
			HttpServletRequest request) {
		Locale locale = request.getLocale();
		String nodes = "";
		try {
			List<Map<String, Object>> whereParamsListOfMap = CommonUtil
					.convertJsonToList(whereParams);
			List<String> groupByFieldList = CommonUtil
					.convertJsonToListStrings(groupByFields);
			JSONArray childNodeList = listViewService.getChildNodesForTree(
					listViewId, rootNode, parentGroup, groupByFieldList,
					whereParamsListOfMap);
	...
	public @ResponseBody
	List<Map<String, String>> getAllListView(HttpServletRequest request) {
		List<Map<String, String>> listViewList = new ArrayList<Map<String, String>>();
		Locale locale=request.getLocale();
		try {
			List<LabelValue> listViews = listViewService
					.getAllListViewWithoutVersion();
			if (listViews != null) {
	...
	public @ResponseBody
	List<Map<String, String>> getAllListViewTemplate(HttpServletRequest request) {
		List<Map<String, String>> listViewList = new ArrayList<Map<String, String>>();
		Locale locale=request.getLocale();
		try {
			List<LabelValue> listViews = listViewService
					.getAllListViewTemplate();
			if (listViews != null) {
				for (LabelValue listView : listViews) {
					Map<String, String> listViewDetail = new HashMap<String, String>();
					listViewDetail.put("listViewName", listView.getLabel());
			...
			// log.info("List View Retrived All ListView Template Successfully");
	...
	public @ResponseBody
	String showTableSelection(ModelMap model,
			@RequestParam("currentNode") String currentNode,
			@RequestParam("rootNode") String rootNode,
			@RequestParam("nodeLevel") String nodeLevel,
			@RequestParam("selectedValues") String selectedValues, User user,
			@RequestParam("defaultNodes") String defaultNodes,
		...
		try {
			if (rootNode.equalsIgnoreCase("Meta_Tables")) {
				List<LabelValue> allMetaTables = listViewService
						.getAllMetaTable();
				nodes = formTableSelectionNodes(allMetaTables, rootNode, false,
						selectedValues);
			} else if (rootNode.equalsIgnoreCase("Run_Time_Tables")) {
				List<LabelValue> allMetaTables = listViewService.getAllTable();
				nodes = formTableSelectionNodes(allMetaTables, rootNode, true,
						selectedValues);
			} else {
				...
				List<String> nodeNames = CommonUtil
						.convertJsonToListStrings(defaultNodes);
	 ...
	 * @return
	 */
	public String formTableSelectionNodes(List<LabelValue> tableNodeList,
			String root, boolean isRuntimeTable, String selectedValues)
	...
	public ModelAndView showTableSelectFrom(ModelMap model,
		...
		try {
			...
			if (appendTo.equals("allTables")) {
				context.put("childNodeUrl", "bpm/listView/showTableSelection");
				script = VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "TableTree.vm", context);
			} else {
				script = VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "DynamicTree.vm", context);
			}
	...
	public @ResponseBody
	String showColumnSelection(ModelMap model,
			@RequestParam("currentNode") String currentNode,
			@RequestParam("rootNode") String rootNode,
			@RequestParam("nodeLevel") String nodeLevel,
			@RequestParam("selectedValues") String selectedValues, User user,
			BindingResult errors, HttpServletRequest request,
			HttpServletResponse response,
			@RequestParam("defaultNodes") String tableList) {
		...
		try {
			if (rootNode.equalsIgnoreCase("default")) {
				...
				Map<String, Object> attr = new HashMap<String, Object>();
				List<String> nodeNames = CommonUtil
						.convertJsonToListStrings(tableList);
				...
				List<LabelValue> allMetaColumns = listViewService
						.getAllColumnsByTableName(rootNode);
	 ...
	 * @return
	 */
	public String getColumnWhitTableName(List<LabelValue> tableNodeList,
	...
	public void gridDataCSVExport(
			@RequestParam("gridHeader") String gridHeader,
			@RequestParam("gridDatas") String gridDatas,
			@RequestParam("gridName") String gridName,
			@RequestParam("hiddenColumn") String hiddenColumn, ModelMap model,
		...
		try {
			...
			for (String cellValue : colmnName) {
				header.createCell(cellCount).setCellValue(
						cellValue.replace("_", " "));
				cellCount++;
			}
	...
	public @ResponseBody
	Map<String, Object> restoreListView(@RequestParam("viewId") String viewId,
			@RequestParam("viewName") String viewName, ModelMap model,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		Locale locale = request.getLocale();
		Map<String, Object> message = new HashMap<String, Object>();
		try {
			...
			log.error(e.getMessage(), e);
			message.put("errorMessage",
					getText("listView.restored.error", locale));
		}
		return message;
	}
	...
	public @ResponseBody
	List<String> getListViewByName(@RequestParam("viewName") String viewName,
			ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		List<String> listViewWhereParamList = new ArrayList<String>();
		try {
			listViewWhereParamList = listViewService
					.getListViewWhereParamNames(viewName);
	...
	public ModelAndView showListViewSelection(ModelMap model,
			@RequestParam("selectionType") String selectionType,
			@RequestParam("appendTo") String appendTo,
			@RequestParam("selectedValues") String selectedValues,
	...
	public @ResponseBody
	List<Map<String, Object>> getListViewRootNodes(ModelMap model,
			@RequestParam("currentNode") String currentNode,
			@RequestParam("rootNode") String rootNode,
			@RequestParam("nodeLevel") int nodeLevel) {
		List<Map<String, Object>> nodeListOfMap = new ArrayList<Map<String, Object>>();
		List<LabelValue> listViewList = listViewService
				.getAllListViewWithoutVersion();
		try {
			for (LabelValue listView : listViewList) {
				CommonUtil.createListViewRootTreeNode(nodeListOfMap,
						listView.getLabel(), listView.getValue(),
						listView.getLabel());
			}
		} catch (BpmException e) {
			log.warn(e.getMessage(), e);
	...
	public RedirectView copyListViewToModule(
			@RequestParam("listViewId") String listViewId,
			@RequestParam("moduleId") String moduleId,
			@RequestParam("currentModuleName") String currentModuleName,
		...
		Locale locale = request.getLocale();
		try {
			String copyStatus = listViewService.copyListViewToModule(moduleId,
					listViewId);
			if (copyStatus.equals("true")) {
				saveMessage(request,
						getText("listView.copied.successfully", locale));
				log.info("List View '" + listViewName
						+ "' Copied From Module:'" + currentModuleName
						+ "' To Module:'" + moduleName + "' successfully");
				...
				saveError(
						request,
						getText("listView.copied.already.exits", copyStatus,
								locale));
				log.error(getText("listView.copied.already.exits", copyStatus,
						locale));
			}
