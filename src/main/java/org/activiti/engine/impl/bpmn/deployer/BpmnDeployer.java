  ...
  public void deploy(DeploymentEntity deployment) {
    ...
    for (String resourceName : resources.keySet()) {
      ...
      if (isBpmnResource(resourceName)) {
        ...
        bpmnParse.execute();
        if(resource.getJsonString()==null||resource.getSvgString()==null){
        	if(!StringUtil.isEmptyString(bpmnParse.getJsonString())){
            	resource.setJsonString(bpmnParse.getJsonString().getBytes());
            }   
            if(!StringUtil.isEmptyString(bpmnParse.getSvgString())){
            	resource.setSvgString(bpmnParse.getSvgString());
            }
            Context.getCommandContext().getResourceEntityManager().updateResource(resource);
        }        
        //ResourceEntity resourceEntity = Context.getCommandContext().getResourceEntityManager().findResourceByDeploymentIdAndResourceName(deployment.getId(), resourceName);
        ...
        for (ProcessDefinitionEntity processDefinition: bpmnParse.getProcessDefinitions()) {
          ...
          List<ProcessDefinition> existingProcessDef = processDefinitionManager.findProcessDefinitionByKey(processDefinition.getKey());
          
          if ((existingProcessDef != null) && (existingProcessDef.size() > 0) && (deployment.getClassificationId() != null)) {
        	  ProcessDefinition processDef = existingProcessDef.get(0);
        	   if(!processDef.getKey().contentEquals(processDefinition.getKey())){
        		   throw new BpmException("The deployment contains process definitions key mismatch ");
        	   }
          }
    ...
    for (ProcessDefinitionEntity processDefinition : processDefinitions) {
      if (keyList.contains(processDefinition.getKey())) {
        throw new BpmException("The deployment contains process definitions with the same key (process id atrribute), this is not allowed");
      }
    ...
    for (ProcessDefinitionEntity processDefinition : processDefinitions) {
      ...
      if (deployment.isNew()) {
        ...
        processDefinition.setDescription(deployment.getDescription());
        
        processDefinition.setIsSystemDefined(deployment.isSystemDefined());
       
        LOG.info("------------------Make a newly deployed process as Currently Active process------------------");
        //Make a newly deployed process as Currently Active process 
        processDefinition.setActiveVersion(true);
        ...
        processDefinition.setClassificationId(deployment.getClassificationId());
        processDefinition.setModifiedUser(deployment.getModifiedUser());
        ...
        List<ProcessDefinition> existingProcessDefinition = processDefinitionManager.findActiveProcessDefinitionByKey(processDefinition.getKey());
        ProcessDefinitionEntity processDefinitionEntity;
        for(ProcessDefinition oldProcessDefinition : existingProcessDefinition){
        	processDefinitionEntity = (ProcessDefinitionEntity) oldProcessDefinition;
        	LOG.info(" Exist Process Id : "+processDefinitionEntity.getId()+" key : "+processDefinitionEntity.getKey()+ " Status : "+processDefinitionEntity.isActiveVersion());
        	processDefinitionEntity.setActiveVersion(false);
        	dbSqlSession.update(processDefinitionEntity); 
        }
