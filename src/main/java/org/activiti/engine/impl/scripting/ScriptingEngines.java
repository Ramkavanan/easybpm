...
public class ScriptingEngines {
  ...
  public Object evaluate(String script, String language, VariableScope variableScope) {
    ...
    //temp fix to run javascript. faced Binding exceptions
    if(language.equalsIgnoreCase("javascript")){
    	bindings = scriptEngine.createBindings();
    	bindings.putAll(variableScope.getVariablesLocal());
    }
    ...
    try {
    	if(script.indexOf("|") != -1) {
    		UelExpressionCondition uelExpressionCondition = new UelExpressionCondition();
    		StringBuffer scriptExp = new StringBuffer();
    		scriptExp = uelExpressionCondition.replaceWithDefaultValues(script);
    		 return scriptEngine.eval(scriptExp.toString(), bindings);
    	} else {
