...
public class TaskReturnCmd extends NeedsActiveTaskCmd<Object> {
	...
	protected Object execute(CommandContext commandContext, TaskEntity task)
		...
		task.getExecution().setJumpActivityId(getLastOperatedActivity(task.getExecution()));
		if(needToDoReturn){
	...
	protected String getLastOperatedActivity(ExecutionEntity execution){
		  List<HistoricTaskInstance> historicTaskInstances = Context.getCommandContext().getHistoricTaskInstanceEntityManager().
		  findLastCompletedTaskByProcessInstanceId(execution.getId()); // for normal process
		  String lastOperatedTaskDefinionKey = null;
		  if(historicTaskInstances.size() > 0){
			  lastOperatedTaskDefinionKey = historicTaskInstances.get(0).getTaskDefinitionKey();
			  for(PvmActivity activity : execution.getActivity().getProcessDefinition().getActivities()) {
