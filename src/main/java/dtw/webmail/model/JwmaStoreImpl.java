 ...
 * Offers methods and utilities to manage the store.
 *
...
public class JwmaStoreImpl implements JwmaStoreInfo, Serializable {
   ...
   *
   * @param session the <tt>JwmaSession</tt> instance this store
   *        belongs to.
   * @param mstore the mail <tt>Store</tt> this instance should wrap.
   */
   ...
   *
   * @param f the actual folder as <tt>JwmaFolderImpl</tt>.
   *
   *
   * @see dtw.webmail.model.JwmaFolderImpl.
   */
  ...
  private void setInboxFolder(JwmaFolderImpl f) {
	    m_InboxFolder = f;
	  }//setActualFolder
  ...
  private void setInboxDraftFolder(JwmaFolderImpl f) {
	  m_InboxDraftFolder = f;
	  }//setActualFolder
  ...
  private void setSentFolder(JwmaFolderImpl f) {
	  m_SentFolder = f;
	  }//setActualFolder
  ...
  private void setSentLaterFolder(JwmaFolderImpl f) {
	  m_SentLaterFolder = f;
	  }//setActualFolder
  ...
  /**
   * Returns the <tt>JwmaInboxInfo</tt> instance that
  ...
  /**
   * Returns the <tt>JwmaTrashInfo</tt> instance that
   * can  be used to retrieve information about the store's
   ...
   * given as <tt>String</tt>.
   * <p>
   * This method will check and modify the
   * name as follows:
   * <ol>
   *   <li>name is null or an empty string:<br>
   *		 <root folder path>+<folder separator>+ "trash"
   *	 </li>
   *	 <li>name does not start with <root folder path>:<br>
   *       <root folder path>+<folder separator>+name
   *   </li>
   * </ol>
   * The method will finally set the newly created folder name in the
   * associated preferences.
   *
   * @param name the name or full name of the draft folder
   *        as <tt>String</tt>.
   *
   * @throws JwmaException if it fails to set the draft folder and
   *         create a JwmaFolder instance with it.
   */
  public void setTrashFolder(String name)
    ...
    try {
      ...
      if (name == null || name.length() == 0) {
        name = m_RootFolder.getFullName() +
            getFolderSeparator() + "trash";
      } else if (!name.startsWith(m_RootFolder.getFullName())) {
        //Note: causes this special folder to be subfolder of root always.
      ...
      Folder trash = getFolder(name);
      if (!trash.exists()) {
        if (!trash.create(JwmaFolderImpl.TYPE_MAILBOX)) {
          name = m_RootFolder.getFullName() + getFolderSeparator() + "jwma-trash";
          trash = getFolder(name);
          if (!trash.exists() && !trash.create(JwmaFolderImpl.TYPE_MAILBOX)) {
            throw new JwmaException("jwma.store.trashfolder");
          }
        }
        log.debug("setTrashFolder(): Created trash folder=" + name);
      }
      //ensure subscription
      trash.setSubscribed(true);
      m_Session.getPreferences().setTrashFolder(trash.getName());
      m_TrashFolder = JwmaFolderImpl.createLight(trash);
      //ensure always fresh counts
      m_TrashFolder.setOnlineCounting(true);
   ...
   * <tt>JwmaFolderImpl.TYPE_MESSAGE_CONTAINER</tt>.
   *
   * @param f the draft folder as <tt>Folder</tt>.
   *
   * @see javax.mail.Folder
   */
   ...
   * <ol>
   *   <li>name is null or an empty string:<br>
   *		 <root folder path>+<folder separator>+ "draft"
   *	 </li>
   *	 <li>name does not start with <root folder path>:<br>
   *       <root folder path>+<folder separator>+name
   *   </li>
   ...
   *         create a JwmaFolder instance with it.
   */
  public void setDraftFolder(String name)
      throws JwmaException {
    ...
    try {
      ...
      m_DraftFolder = getFolder(name);
      if (!m_DraftFolder.exists()) {
        if (!m_DraftFolder.create(JwmaFolderImpl.TYPE_MAILBOX)) {
          name = m_RootFolder.getFullName() + getFolderSeparator() + "jwma-draft";
          m_DraftFolder = getFolder(name);
          if (!m_DraftFolder.exists() && !m_DraftFolder.create(JwmaFolderImpl.TYPE_MAILBOX)) {
            throw new JwmaException("jwma.store.draftfolder");
          }
        }
        log.debug("setDraftFolder(): Created draft folder=" + name);
      }
      //ensure subscription
      m_DraftFolder.setSubscribed(true);
      m_Session.getPreferences().setDraftFolder(m_DraftFolder.getName());
    } catch (MessagingException ex) {
      throw new JwmaException(ex.getMessage()).setException(ex);
    }
  }//setDraftFolder
   ...
   * <tt>JwmaFolderImpl.TYPE_MESSAGE_CONTAINER</tt>.
   *
   * @param f the sent mail archive folder as <tt>Folder</tt>.
   *
   * @see javax.mail.Folder
   */
   ...
   * <ol>
   *   <li>name is null or an empty string:<br>
   *		 <root folder path>+<folder separator>+ "sent-mail"
   *	 </li>
   *	 <li>name does not start with <root folder path>:<br>
   *       <root folder path>+<folder separator>+name
   *   </li>
   ...
   * associated preferences.
   *
   * @param name the name or full name of the sent-mail archive folder
   *        as <tt>String</tt>.
   *
   * @throws JwmaException if it fails to set the sent-mail folder and
   *         create a JwmaFolder instance with it.
   */
  public void setSentMailFolder(String name)
      throws JwmaException {
    ...
    try {
      if (m_Session.getPreferences().isAutoArchiveSent()) {
        ...
        m_SentMailFolder = getFolder(name);
        if (!m_SentMailFolder.exists()) {
          boolean success = m_SentMailFolder.create(JwmaFolderImpl.TYPE_MAILBOX);
          log.debug("setSentMailFolder(): Created sent-mail folder=" + name + ":" + success);
        } else {
          log.debug("setSentMailFolder(): Set sent-mail folder=" + name);
        }
        m_SentMailFolder.setSubscribed(true);
        m_Session.getPreferences().setSentMailArchive(m_SentMailFolder.getName());
      }
    } catch (MessagingException ex) {
      throw new JwmaException(ex.getMessage()).setException(ex);
    }
  ...
  public void setSentLaterMailFolder(String name)
  throws JwmaException {
	
	try {
	    if (name == null || name.length() == 0) {
	      name = m_RootFolder.getFullName() +
	          getFolderSeparator() + "sent-later";
	    } else if (!name.startsWith(m_RootFolder.getFullName())) {
	      //Note: causes this special folder to be subfolder of root always.
	      int lastindex = name.lastIndexOf(getFolderSeparator());
	      if (lastindex > 0) {
   ...
   * <tt>JwmaFolderImpl.TYPE_MESSAGE_CONTAINER</tt>.
   *
   * @param f the read mail archive folder as <tt>Folder</tt>.
   *
   * @see javax.mail.Folder
   */
   ...
   *        as <tt>String</tt>.
   *
   * @throws JwmaException if it fails to set the read-mail folder and
   *         create a JwmaFolder instance with it.
   */
  public void setReadMailFolder(String name)
      throws JwmaException {

    try {
      if (m_Session.getPreferences().isAutoMoveRead()) {
        if (name == null || name.length() == 0) {
          name = m_RootFolder.getFullName() +
              getFolderSeparator() + "read-mail";
        } else if (!name.startsWith(m_RootFolder.getFullName())) {
        ...
        m_ReadMailFolder = getFolder(name);
        if (!m_ReadMailFolder.exists()) {
          boolean success = m_ReadMailFolder.create(JwmaFolderImpl.TYPE_MAILBOX);
          log.debug("setReadMailFolder(): Created read-mail folder=" + name + ":" + success);
        }
        m_ReadMailFolder.setSubscribed(true);
        m_Session.getPreferences().setReadMailArchive(m_ReadMailFolder.getName());
      }
    } catch (MessagingException ex) {
      throw new JwmaException(ex.getMessage()).setException(ex);
    }
  ...
  /**
   * Updates the root folder, if the path differs
   * from the actual root folder path.
   *
   * @param path the path of the new root folder as <tt>String</tt>.
   */
  public void updateRootFolder(String path)
      throws JwmaException {
    if (!m_RootFolder.getFullName().equals(path)) {
      m_Session.getPreferences().setRootFolder(path);
      prepare();
    }
  }//updateRootFolder
   ...
   *
   * @param message the <tt>Message</tt> to be archived.
   *
   * @throws JwmaException if it fails to archive the message.
   *
  ...
  public int archiveSentLaterMail(Message message)
	  throws JwmaException {
	  int messageNumber = 1;
	  Folder archive = null;
	  try {
   ...
   *
   * @throws JwmaException if it fails to set the new folder and
   *         create a JwmaFolder instance with it.
   */
  public JwmaFolder setActualFolder(String path)
      throws JwmaException {
    setActualFolder(JwmaFolderImpl
        .createJwmaFolderImpl(this, getFolder(path)));
    return getActualFolder();
  }//setActualFolder

  /**
   * Returns a <tt>JwmaFolderImpl</tt> with the given path
   * from the store.
   *
   * @return the folder as <tt>JwmaFolderImpl</tt> and set as actual
   *         folder(!).
   *
   * @throws JwmaException if a folder with the given path does not exist
   *         or an error occurs when creating the wrapper instance.
   */
  private JwmaFolderImpl getJwmaFolder(String fullname)
      throws JwmaException {

    setActualFolder(JwmaFolderImpl
        .createJwmaFolderImpl(this, getFolder(fullname)));
    return getActualFolder();
  }//getJwmaFolder

  /**
   * Returns a <tt>Folder</tt> with a given path
   * from the mail store.
   *
   * @return the folder as <tt>Folder</tt>.
   *
   * @throws JwmaException if a folder with the given path does not exist
   *         on the store or a MessagingException occurs.
   */
  public Folder getFolder(String fullname)
      throws JwmaException {
    try {
      //FIXME: Microsoft Exchange returns "" as Default Folder, but asking for the
   ...
   *
   * @throws JwmaException if the folder already exists, or if it fails
  ...
  public void createFolder(String fullname, int type)
    ...
    try {
      ...
      if (fullname.indexOf(getFolderSeparator()) == -1) {
        //In case of the root folder being "" this can represent a problem
      ...
      if (newfolder.exists()) {
        throw new JwmaException(
            "jwma.store.createfolder.exists", true
        );
      } else {
      ...
      throw new JwmaException(
          "jwma.store.createfolder.failed", true
      ).setException(mex);
    }
  }//createFolder
   ...
   * the array and calling <tt>deleteFolder()</tt>
   *
   * @param folders an array of strings; each <tt>String</tt>
   *        representing the full path of a valid folder of the
   *        actual store.
   *
   * @throws JwmaException if a folder does not exist,
   *         or if an error occurs when deleting.
   *
   ...
   * special folder.
   *
   * @param fullname the folder's path as <tt>String</tt>.
   *
   * @throws JwmaException if a folder does not exist,
   *         or if an error occurs when deleting.
   */
  public void deleteFolder(String fullname)
    ...
    try {
      ...
      throw new JwmaException(
          "jwma.store.deletefolder.failed", true
      ).setException(mex);
    }
  }//deleteFolder
   ...
   * <tt>moveFolders()</tt>.
   *
   * @param foldername the full path of the folder as <tt>String</tt>.
   * @param destfolder the full path of a valid folder on the
   *        actual store.
   *
   * @throws JwmaException if a folder does not exist,
   *         or if an error occurs when deleting.
   ...
   *        representing the full path of a valid folder of the
   *        actual store.
   * @param destfolder the full path of a valid folder of the
   *        actual store.
   *
   * @return the new actual folder as <tt>JwmaFolder</tt>.
   *
   * @throws JwmaException if the source folders or the destination folder
  ...
  public void moveFolders(String[] foldernames, String destfolder)
    ...
    try {
      ...
      if (!(dest.getType() == JwmaFolderImpl.TYPE_FOLDER
          || dest.getType() == JwmaFolderImpl.TYPE_MIXED)) {
      ...
      for (Iterator iter = folders.iterator(); iter.hasNext();) {
        ...
        Folder newfolder = getFolder(
            destfolder + getFolderSeparator() + oldfolder.getName()
        );
        if (newfolder.exists()) {
          throw new JwmaException(
        ...
        // one of its childs
        if (!newfolder.getFullName().regionMatches(
            0, oldfolder.getFullName(),
            0, oldfolder.getFullName().length())) {
      ...
      throw new JwmaException(
          "jwma.store.movefolder.failed", true
      ).setException(mex);
  ...
  public void updateFolderSubscription(String[] foldernames, boolean subscribe)
    ...
    try {
      ...
      throw new JwmaException("jwma.store.updatesubscription.failed"
  ...
  private boolean isSpecialFolder(String fullname) {
    return (
        fullname.equals(m_RootFolder.getFullName())
        || fullname.equals(m_InboxFolder.getPath())
        || fullname.equals(m_TrashFolder.getPath())
        || fullname.equals(m_DraftFolder.getFullName())
        || (m_Session.getPreferences().isAutoArchiveSent() &&
        fullname.equals(m_SentMailFolder.getFullName()))
        || (m_Session.getPreferences().isAutoMoveRead() &&
        fullname.equals(m_ReadMailFolder.getFullName()))
        );
  }//isSpecialFolder
  ...
  public JwmaFolder[] listFolders(int type, boolean subscribed) {
    String[] excludes = {
      m_RootFolder.getFullName(),
      m_InboxFolder.getPath(),
      m_TrashFolder.getPath(),
      m_DraftFolder.getFullName(),
      ((m_Session.getPreferences().isAutoArchiveSent())? m_SentMailFolder.getFullName():""),
      ((m_Session.getPreferences().isAutoMoveRead())? m_ReadMailFolder.getFullName():"")
    };
  ...
  public JwmaFolder[] listFolderMoveTargets() {
    ...
    return m_Folders.createFolderArray(
        m_Folders.sublist(JwmaFolder.TYPE_FOLDER_CONTAINER, m_ActualFolder, c_SubscribedOnly)
    );
  ...
  public JwmaFolder[] listMessageMoveTargets() {
    //sublist with actual folder filtered
    return m_Folders.createFolderArray(
        m_Folders.sublist(JwmaFolder.TYPE_MESSAGE_CONTAINER, m_ActualFolder, c_SubscribedOnly)
    );
   ...
   *
   * @return true if store is supporting mixed mode, false
   *         otherwise.
   */
   ...
   *
   * @param the folder separator as <tt>char</tt>.
   */
   ...
   * Cleans up the store.
   * <p>
   * This method performs some of the do-automatic functions
   * within jwma:
   * <ol>
   *   <li>Move read messages from the inbox to the read messages folder,
   *     if the feature is enabled.
   *	 </li>
   *	 <li>Empty the trashbin.</li>
   * </ol>
   *
   * @throws JwmaException if cleanup routines fail to perform error free.
   */
  public boolean cleanup() throws JwmaException {

    //perform auto features like set in users prefs
    JwmaPreferencesImpl prefs = m_Session.getPreferences();

    //1. automove read from Inbox to readmessagesfolder if set
    if (prefs.isAutoMoveRead()) {
      //move the read messages
      m_InboxFolder.moveMessages(
          m_InboxFolder.getReadMessages(),
          m_ReadMailFolder.getFullName()
      );
    }
  ...
  public void prepare()
    ...
    try {
      ...
      if (!m_RootFolder.exists()) {
        ...
        prefs.setRootFolder(m_RootFolder.getFullName());
      ...
      //Draft
      setDraftFolder(prefs.getDraftFolder());
      
      m_InboxDraftFolder = getJwmaFolder("INBOX"+getFolderSeparator()+"Draft");
      
      //Trash
      setTrashFolder(prefs.getTrashFolder());


      //read-mail archive
      setReadMailFolder(prefs.getReadMailArchive());

      //sent-mail archive
      setSentMailFolder(prefs.getSentMailArchive());

      m_SentFolder = getJwmaFolder("INBOX"+getFolderSeparator()+"sent-mail");
   ...
   */
  public void prepareInboxFolder()
      throws JwmaException {
    log.debug("prepare()");
    try {
    	//Inbox the folder that contains the incoming mail
   ...
   */
  public void refreshInboxFolder(JwmaFolder oldFolder)
      throws JwmaException {
    log.debug("prepare()");
    try {
   ...
   */
  public void prepareDraftFolder()
      throws JwmaException {
   ...
   * pull sent messages from server.
   */
  public void prepareSentLaterFolder()
      throws JwmaException {
   ...
   * pull sent messages from server.
   */
  public void prepareTrashFolder()
      throws JwmaException {
      log.debug("pullSentFromServer()");
      JwmaPreferencesImpl prefs = m_Session.getPreferences();
   ...
   * pull inbox messages from server.
   */
  public void pullInboxFromServer()
      throws JwmaException {
   ...
   * pull draft messages from server.
   */
  public void pullDraftFromServer()
      throws JwmaException {
      log.debug("pullDraftFromServer()");
      JwmaPreferencesImpl prefs = m_Session.getPreferences();
   ...
   * pull sent messages from server.
   */
  public void pullSentFromServer()
      throws JwmaException {
      log.debug("pullSentFromServer()");
      JwmaPreferencesImpl prefs = m_Session.getPreferences();
   ...
   *
   * @return true if a folder with this path exists on the store,
   *         false otherwise.
   *
   * @throws  JwmaException if the folder already exists, or if it fails
   *			to create the folder.
  ...
  public boolean checkFolderExistence(String path)
    ...
    if (path.equals(m_RootFolder.getFullName())
        || path.equals("INBOX")) {
      return true;
    }
   ...
   *
   * @param session the actual <tt>JwmaSession</tt>.
   * @param mstore the mail store that should be wrapped.
   *
   * @return the newly created <tt>JwmaStoreImpl</tt> instance.
   *
   * @throws JwmaException if preparing the store for use fails.
   *
   * @see #prepare()
   * @see dtw.webmail.JwmaSession
   * @see javax.mail.Store
   */
  public static JwmaStoreImpl createJwmaStoreImpl(JwmaSession session, Store mstore)
      throws JwmaException {
