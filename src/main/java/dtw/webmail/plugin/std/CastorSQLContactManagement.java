 ...
 */
public class CastorSQLContactManagement
    implements ContactManagementPlugin {

  //class attributes
  private static Logger log = Logger.getLogger(CastorSQLContactManagement.class);
  ...
  public JwmaContactsImpl loadContacts(String cuid)
    ...
    try {
      ...
      //load contacts
      cts = (CastorContacts) db.load(CastorContacts.class, cuid);
  ...
  public void saveContacts(JwmaContactsImpl contacts)
    ...
    CastorDatabase db = null;
    CastorContacts ctdb = (CastorContacts) contacts;

    try {
      db = m_DBPool.leaseDatabase();
      // Begin a transaction
      log.debug("Persisting contact database...");
      db.begin();

      //check if the preferences are not persistent yet.
      //simplest is if the timestamp is not zero.
      if (ctdb.jdoGetTimeStamp() != 0) {
        log.debug("udpate...");
        ctdb.updateContacts(db.getDatabase());
      } else {
        log.debug("create...");
        ctdb.persistContacts(db.getDatabase());
      }

      // Commit the transaction
      db.commit();
    } catch (Exception ex) {
      log.error("saveContacts()", ex);
      throw new JwmaException("").setException(ex);
    } finally {
      m_DBPool.releaseDatabase(db);
    }
  ...
  public boolean isPersistent(String cuid)
    ...
    try {
      ...
      if (db.load(CastorContacts.class, cuid) != null) {
        exists = true;
      }
