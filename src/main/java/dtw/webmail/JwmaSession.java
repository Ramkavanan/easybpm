 ...
 * <p>
 * Each instance stores the state and provides accessor and
 * mutator operations for retrieving and changing.
 * <p>
 * The reference to the instance itself is stored in the
 * <tt>HttpSession</tt> as session attribute.
 * <p>
 * Note that this class is actually the bridge between the web
 * session and the mail client session, and that instances have
 * the responsibility to manage the state of both.
 *
...
public class JwmaSession
  ...
  transient private JwmaPreferencesImpl m_Preferences;
  transient private JwmaHtmlHelper m_HtmlHelper = new JwmaHtmlHelperImpl();
   ...
   *
   * @param session reference to the actual <tt>HttpSession</tt> instance.
   */
  public JwmaSession(HttpSession session, String sessionType) {
	  if(sessionType.equalsIgnoreCase("email")){
		  JwmaKernel.getReference().setConfiguration();
	  } else {
		  JwmaKernel.getReference().setInternalMessageConfiguration();
	  }
	  ...
	  m_ViewHistory = new String[2];
	  m_ViewHistory[0] = JwmaKernel.LOGIN_VIEW;
	  storeDefaultBeans(sessionType);
  }//constructor
  ...
  /**
   * Sets the reference to the actual <tt>HttpServletRequest</tt>
   * instance.
   *
   * @param request reference to the actual incoming
   *        <tt>HttpServletRequest</tt> instance.
   */
  ...
  /**
   * Returns the reference to the actual <tt>HttpServletRequest</tt>
   * instance.
   *
  ...
  /**
   * Sets the reference to the actual <tt>HttpServletResponse</tt>
   * instance.
   *
   * @param request reference to the actual <tt>HttpServletResponse</tt>
   *        instance.
   */
   ...
   * instance.
   *
   * @return reference to the actual <tt>HttpServletResponse</tt>
   *         instance.
   */
  ...
  /**
   * Returns the reference to the actual <tt>HttpSession</tt>
   * instance.
   *
   ...
   * This is the case if it is not new, and if it has
  ...
  /**
   * Sets the reference to the actual <tt>HttpSession</tt>
   * instance.
   *
   * @param reference to the actual <tt>HttpSession</tt> instance.
   */
  ...
  /**
   * Returns the value of a request parameter from this
   * session's <tt>HttpServletRequest</tt>.
   * This is a convenience method that trims the Strings.
   *
   * @return the value of the request parameter or null
  ...
  /**
   * Returns the values of a request parameter from this
   * session's <tt>HttpServletRequest</tt>.
   * This is a convenience method that trims the Strings.
   *
   * @return the value of the request parameters or null
   *         if it doesn't have any value.
   */
  public String[] getRequestParameters(String name) {
    String[] value =
        getRequest().getParameterValues(name);
  ...
  /**
   * Redirects the the actual <tt>HttpServletRequest</tt>
   * to an abstract view.
   * <p>
   ...
   *        view.
   *
   * @see dtw.webmail.JwmaKernel#LOGIN_VIEW
   * @see dtw.webmail.JwmaKernel#ERROR_VIEW
   * @see dtw.webmail.JwmaKernel#LOGOUT_VIEW
   * @see dtw.webmail.JwmaKernel#FOLDER_VIEW
   * @see dtw.webmail.JwmaKernel#MESSAGE_VIEW
   * @see dtw.webmail.JwmaKernel#PREFERENCES_VIEW
   * @see dtw.webmail.JwmaKernel#COMPOSE_VIEW
   * @see dtw.webmail.JwmaKernel#FIRSTTIME_VIEW
   * @see dtw.webmail.JwmaKernel#CONTACTS_VIEW
   */
  public void redirect(String view) {
    //update history
    m_ViewHistory[1] = m_ViewHistory[0];
    m_ViewHistory[0] = view;

    //retrieve URL from Kernel
    view = JwmaKernel.getReference().getViewUrl(view);

    try {
      m_Response.sendRedirect(m_Response.encodeRedirectUrl(view));
    } catch (IOException ex) {
      log.error(JwmaKernel.getReference().getLogMessage("jwma.session.redirectfailed"), ex);
    }
  }//redirect
   ...
   * to the last view.
   * <p>
   * The last view is traced by a simple history mechanism
   * implemented in the <tt>redirect()</tt> method.
   */
  public void redirectToLast() {
    redirect(m_ViewHistory[1]);
  }//redirectToLast
   ...
   * to the present view.
   * <p>
   * The actual view is traced by a simple history mechanism
   * implemented in the <tt>redirect()</tt> method.
   * This method helps to acquire an updated view of the actual view.
   */
  public void redirectToActual() {
    redirect(m_ViewHistory[0]);
  }//redirectToLast


  /**
   * Stores a reference to a bean(<i>model</i>) in the
   * actual <tt>HttpSession</tt>.
   * <p>
   * All stored beans are traced, so that they can be properly
   * removed.
   *
   * @param name the unique identifier of bean to be stored as
   *        session attribute.
   * @param bean the reference to the bean to be stored as session
   *        attribute.
   */
  ...
  /**
   * Stores the default beans of any session.
   * This is done directly, to bypass the mechanism
   * that traces the other stored beans.
   * On cleanup, the default beans remain referenceable
   * within the session.
   */
  private void storeDefaultBeans(String sessionType) {
    //the session bean
	if(sessionType.equalsIgnoreCase("email")){
		m_WebSession.putValue("jwma.emailSession", this);
	} else {
		m_WebSession.putValue("jwma.internalMessageSession", this);
	}
    //the HtmlHelper bean
    m_WebSession.putValue("jwma.htmlhelper", m_HtmlHelper);
    //a reference to the i18n classloader
   // m_WebSession.putValue("jwma.resourceloader", JwmaKernel.getReference().getResourceClassLoader());
   // m_WebSession.setAttribute("jwma.resourceloader", JwmaKernel.getReference().getResourceClassLoader());
  }//storeDefaultBeans

  /**
   * Returns a reference to a bean(<i>model</i>) in the
   * actual <tt>HttpSession</tt>.
   * <p>
   ...
   * @param name the unique identifier of bean to be stored as
   *        session attribute.
   * @return the reference to the bean stored as session
   *         attribute or null.
   */
  ...
  /**
   * Removes a reference to a bean (<i>model</i>) in the
   * actual <tt>HttpSession</tt>.
   *
   * @param name the unique identifier of bean to be stored as
   *        session attribute.
  ...
  /**
   * Removes all traced bean references from the
   * actual <tt>HttpSession</tt>.
   */
  ...
  /**
   * Fetches the last login from the user's
   * preferences (to have it available for the
   * rest of the session) and then overwrites
   * the value in the preferences.
   */
   ...
   *
   * @param password completing the login for the post office of
   *        the jwma user.
   * @throws JwmaException when something goes wrong while opening or preparing
   *         the mail session.
   */
  private Store initMailSession(String password)
      throws JwmaException {

    log.debug("initMailSession()");
    //prepare for formatted log message
    //changed by mathi.. when there is no request. 
    //Object[] args = {m_Username + "@" + m_PostOffice.getAddress(), getRequestAddress()};
    ...
    try {
      ...
      if(m_MailSession == null){
	      ...
	      m_MailSession =
	          Session.getInstance(System.getProperties(), m_Authenticator);
      }
      ...
      if(!isAuthenticated()){
	      store.connect(
	          m_PostOffice.getAddress(),
	          m_PostOffice.getPort(),
	          m_Username,
	          password
	      );
      }
      ...
      if (m_MTA.isAuthenticated()) {
        transport = m_MailSession.getTransport(
            new URLName(
                m_MTA.getProtocol(),
                m_MTA.getAddress(),
                m_MTA.getPort(),
      ...
      m_JwmaTransport =
          JwmaTransportImpl.createJwmaTransportImpl(transport, m_Authenticator);
      log.debug("Created JwmaTransport wrapper instance.");

      //log to syslog
      log.info(
          MessageFormat.format(
              JwmaKernel.getReference().getLogMessage("jwma.session.login.success")
              , args
          )
      );
      return store;
      ...
      log.info(
          MessageFormat.format(
              JwmaKernel.getReference().getLogMessage("jwma.session.login.failure")
              , args
          )
      );
      if(!afe.getMessage().equals("[ALERT] Too many simultaneous connections. (Failure)")){
          ...
          throw new JwmaException(
              "session.login.authentication", true
          ).setException(afe);
      } else {
    	  //throw JwmaException
  ...
  public void endMailSession() {
    ...
    try {
      ...
      log.info(
          MessageFormat.format(
              JwmaKernel.getReference().getLogMessage("jwma.session.logout.success")
              , args
          )
      );
      ...
      log.error(
          MessageFormat.format(
              JwmaKernel.getReference().getLogMessage("jwma.session.logout.failed")
              , args
          )
          , ex
      );
  ...
  /**
   * Tests wheter a connection to the post office
   * exists, throwing a <tt>JwmaException</tt> if
   * not, thus it ensures that a connection to the
   * post office exists.
   *
   * @throws JwmaException if there is no connection to
   *         the post office.
   */
  ...
  /**
   * Sets the <tt>Locale</tt> of this <tt>Session</tt> to the one
   ...
   *
   * @param username the user's name as <tt>String</tt>.
   *
   * @return the identifier is assembled like a standard email address
   *         using the set postoffice host.
   */
  public String getUserIdentity(String username) {
    return new StringBuffer(username)
        .append('@')
        .append(m_PostOffice.getAddress())
        .toString();
  }//getUserIdentity
   ...
   * Returns the assembled user's identity as <tt>String</tt>.
   *
   * @return the identifier assembled like a standard email address
   *         using the set username and hostname.
   */
  public String getUserIdentity() {
    return new StringBuffer(m_Username)
        .append('@')
        .append(m_PostOffice.getAddress())
        .toString();
  }//getUserIdentity
  ...
  /**
   * Returns the <tt>JwmaPreferencesImpl</tt> instance
  ...
  /**
   * Saves the <tt>JwmaPreferencesImpl</tt> instance
   * of this session instance's user.
   * Note that the actual process of saving is delegated
   * to the active <tt>PreferencesPersistencePlugin</tt>
   * instance obtained from the kernel.
   *
   * @see dtw.webmail.plugin.PreferencesPersistencePlugin#savePreferences(JwmaPreferencesImpl)
   *
  ...
  public boolean savePreferences()
    ...
    try {
      JwmaKernel.getReference().getPrefsPersistencePlugin()
          .savePreferences(m_Preferences);
    } catch (Exception e) {
      throw new JwmaException("jwma.session.preferences.save");
    }
    return true;
  }//savePreferences
   ...
   * If the session is flagged as authenticated, the
   * initialization of the mail session worked successfully,
   * and the user associated with this session could be
  ...
  /**
   * Sets wheter this session is authenticated or not.
   * Private a really good reason: security.
   *
   * @param b true if authenticated, false otherwise.
   */
  ...
  /**
   * Tests wheter this session is authenticated, throwing
   * a <tt>JwmaException</tt> if not, thus ensuring an
   * authenticated session.
   *
   * @throws JwmaException if the session is not authenticated.
   */
  ...
  /**
   * Tests if this session's associated user is an
   * administrator.
   *
   * @return true if associated username represents an admin,
   *         false otherwise.
   */
  public boolean isAdministrator() {
    return m_Configuration.getAdministration().isAdmin(m_Username);
  }//isAdministrator
   ...
   * administrative user.
   *
   * @throws JwmaException if the associated username is not an
   *         admin.
   */
  public void ensureAdministrator()
    ...
    if (!isAdministrator()) {
      //throw JwmaException
      throw new JwmaException("jwma.session.administration");
    }
  }//ensureAdministrator

  /**
   * Authenticates the given user by initializing the mail session,
   * thus authenticating against the post office of this session.
   * Note that the credentials (i.e. password) are not stored
   * anywhere.
   *
   * @param username the name of this session's user as
   *        <tt>String</tt>.
   * @param password the related credentials as <tt>String</tt>.
   * @param newAccount a flag that denotes whether a jwma account
   *        already exists (false) or not (true).
   *
   * @returns true if the session was successfully authenticated,
   *          false if not.
   *
   * @throws JwmaException if initializing a mailsession or
   *         authenticating against the post office fails.
   *
   */
  public boolean authenticate(String username,
    ...
    if (isAuthenticated()) {
      ...
      /*throw new JwmaException(
          "jwma.session.multiplelogin", true
      );*/
    } else {
      ...
      if (!isAuthenticated()) {
	      store.addConnectionListener(
	        new StoreConnectionHandler(store)
	      );
      }
      ...
      if (newAccount) {
        log.debug("newAccount=true");
        m_Preferences =
            JwmaKernel.getReference()
            .getPrefsPersistencePlugin()
            .getPreferencesTemplate(this);
        log.debug("retrieved template");
        m_Preferences.setUserIdentity(getUserIdentity());
      } else {
        log.debug("newAccount=false");
        m_Preferences =
            JwmaKernel.getReference()
            .getPrefsPersistencePlugin()
            .loadPreferences(getUserIdentity());
  ...
  /**
   * Ends this session, by cleaning up user related data
   * and resources.
   * If the session is authenticated, then the new last
   * login <tt>String</tt> is stored in the users
   * preferences, before they are saved. Consequently the
   * mail session is closed, and the bean references stored
   * in this session are cleaned up.
   *
   * @see #isAuthenticated()
   * @see #savePreferences()
   */
  ...
  class StoreConnectionHandler
    ...
    /**
     * Invoked when a service connection is disconnected.
     * This probably means the user exceeded idle timeout,
     * and might want to be reconnected.
     *
     * @param e <tt>ConnectionEvent</tt> the event that was fired.
     */
